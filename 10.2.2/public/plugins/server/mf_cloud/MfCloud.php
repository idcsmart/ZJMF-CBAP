...
class MfCloud{
	...
	public function metaData(){
		return ['display_name'=>'魔方云(自定义配置)', 'version'=>'1.0.0'];
	}
  ...
  `host_prefix` varchar(20) NOT NULL DEFAULT '' COMMENT '主机名前缀',
  `host_length` int(11) NOT NULL COMMENT '主机名长度',
...
) ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARSET=utf8 COMMENT='性能限制表';",
  ...
  PRIMARY KEY (`id`),
  KEY `product_id` (`product_id`)
) ENGINE=InnoDB AUTO_INCREMENT=37 DEFAULT CHARSET=utf8 COMMENT='周期表';",
  ...
  KEY `product_id` (`product_id`)
) ENGINE=InnoDB AUTO_INCREMENT=262 DEFAULT CHARSET=utf8 COMMENT='镜像表';",
	        	...
	        	if(isset($params['custom']['vpc'])){
	        		if(isset($params['custom']['vpc']['id']) && $params['custom']['vpc']['id']>0){
	        			...
	        			if(empty($vpcNetwork) || $vpcNetwork['client_id'] != get_client_id() || $vpcNetwork['data_center_id'] != $params['custom']['data_center_id']){
	        				return ['status'=>400, 'msg'=>lang_plugins('VPC网络不存在')];
	        			}
	        		}else{
	        		...
	        		return ['status'=>400, 'msg'=>lang_plugins('VPC网络参数错误')];
	        	}
	        }
		}
