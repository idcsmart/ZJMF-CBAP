...
<div id="content" class="order-details hasCrumb" v-cloak>
  ...
  <t-card class="list-card-container">
    <ul class="common-tab">
      ...
      <li>
        <a :href="`order_notes.html?id=${id}`">{{lang.notes}}</a>
      </li>
    </ul>
    <!-- 基础信息 -->
    <div class="top-info">
      <div class="left-box">
        <div class="item">
          ...
          <a :href="`client_detail.html?client_id=${orderDetail.client_id}`" class="info aHover">{{orderDetail.client_name}}</a>
        </div>
        <div class="item">
      ...
      <div class="r-box">
        <div class="con">
          ...
          <template v-if="orderDetail.status === 'Unpaid'">
            <t-select v-model="gateway" :placeholder="lang.select+lang.pay_way" class="order-pay" @change="changePay">
          ...
          <p class="time">{{moment(orderDetail.create_time * 1000).format('YYYY-MM-DD HH:mm')}}</p>
    ...
    <t-table row-key="id" :data="orderDetail.items" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" :hide-sort-tips="true">
      ...
      <template #description="{row}">
        ...
        <template v-else>
          <a :href="`host_detail.html?client_id=${orderDetail.client_id}&id=${row.host_id}`" class="aHover" v-if="row.host_id">{{row.description}}</a>
