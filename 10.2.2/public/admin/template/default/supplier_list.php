...
<div id="content" class="supplier_list" v-cloak>
  <t-card class="list-card-container">
    <div class="add-btn">
      <t-button @click="addSupplier">添加供应商</t-button>
    </div>
    <t-table row-key="id" :data="data" size="medium" :columns="columns" @sort-change="sortChange" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" display-type="fixed-width">
      ...
      <template #name="{row}">
        <span style="color: #0052D9; cursor: pointer;" @click="goDetail(row.id)">{{row.name}}</span>
      </template>
      <template #status="{row}">
        <span>
      ...
      <template #op="{row}">
        <t-tooltip content="查看" :show-arrow="false" theme="light">
          <t-icon name="view-module" @click="goDetail(row.id)" size="18px" style="color: #0052D9; margin-right: 20px; cursor: pointer;"></t-icon>
        </t-tooltip>
        <t-tooltip content="编辑" :show-arrow="false" theme="light">
          <t-icon name="edit-1" @click="handelEdit(row)" size="18px" style="color: #0052D9; margin-right: 20px; cursor: pointer;"></t-icon>
        </t-tooltip>
        <t-tooltip content="删除" :show-arrow="false" theme="light">
          <t-icon name="delete" @click="handelDel(row.id)" size="18px" style="color: #0052D9; cursor: pointer;"></t-icon>
        </t-tooltip>
      </template>
    </t-table>
  ...
  <!-- 配置弹窗 -->
  <t-dialog header="添加/编辑供应商" :visible.sync="configVisble" :footer="false" width="650" @closed="diaClose">
    <t-form :rules="rules" ref="userDialog" :data="formData" @submit="onSubmit" :label-width="120" reset-type="initial">
      <t-form-item :label="item.title" :name="item.name" v-for="item in configData" :key="item.title">
        <!-- text -->
        <t-input style="width: calc(100% - 30px);" v-if="item.type==='text'" :disabled="(item.disableEdit && editId !='')" v-model="formData[item.name]" :placeholder="item.tip ? item.tip : lang.input+item.title"></t-input>
        <!-- password -->
        <t-input v-if="item.type==='password'" type="password" v-model="formData[item.name]" :placeholder="item.tip ? item.tip :lang.input+item.title"></t-input>
        <!-- textarea -->
        ...
        <!-- radio -->
        <t-radio-group v-if="item.type==='radio'" v-model="formData[item.name]" :options="computedOptions(item.options)">
        </t-radio-group>
        <!-- checkbox -->
        <t-checkbox-group v-if="item.type==='checkbox'" v-model="formData[item.name]" :options="item.options">
        </t-checkbox-group>
        <!-- select -->
        <t-select v-if="item.type==='select'" v-model="formData[item.name]" :placeholder="item.tip ? item.tip :lang.input+item.title">
      ...
      <div class="com-f-btn">
        ...
        <t-button theme="default" variant="base" @click="configVisble=false">取消</t-button>
      </div>
    </t-form>
  </t-dialog>
  <!-- 删除弹窗 -->
