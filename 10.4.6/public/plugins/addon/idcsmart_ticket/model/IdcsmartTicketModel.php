        ...
        try{
            ...
            if ($this->isAdmin){
                ...
                if (class_exists('server\idcsmart_common_finance\model\IdcsmartCommonSonHost')){
                    ...
                    if (!empty($host) && $host['billing_cycle']!="onetime" && $host['due_time']<=$time && $host['is_delete'] == 0){
            ...
            if ($this->isAdmin){
                ...
                add_task([
                    'type' => 'email',
                    'description' => lang_plugins('client_reply_ticket_send_mail'),
                    'task_data' => [
                        'name'=>'client_reply_ticket',//发送动作名称
                        'client_id'=>$clientId,//客户ID
                        'template_param'=>[
                            'subject' => $ticket['title'],//工单名称
                        ],
