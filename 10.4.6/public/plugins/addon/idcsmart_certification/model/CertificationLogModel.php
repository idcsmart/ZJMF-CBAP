    ...
    public function certificationList($param)
    {
        ...
        $where = function (Query $query) use($param,$ids) {
            ...
            if (!empty($param['status'])){
                $query->where('cl.status',$param['status']);
            }
...
//                ->where('oi.type','certification_person')
...
//                ->leftJoin('order o','oi.order_id=o.id')
//                ->where('oi.client_id',$clientId)
//                ->where('oi.type','certification_company')
//                ->where('oi.rel_id',0) // 表示可能存在订单，并且未进入实名资料提交这一步
//                ->order('oi.id','desc')
//                ->find();
//            if (!empty($order)){
...
//            }
    ...
    public function certificationOrder($param)
    {
        ...
        try{
            ...
            if (!empty($data['order']) && $data['order']['status']=='Paid'){
            ...
            $result = $OrderModel->createOrderBase($orderData);
