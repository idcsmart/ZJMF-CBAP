    ...
    public function orderPage($param)
    {
        ...
        if(!empty($country)){
            if(isset($param['scene']) && $param['scene'] == 'recommend'){
                // 获取推荐配置
                $recommendConfig = RecommendConfigModel::field('id,name,description,data_center_id,line_id,cpu,memory,system_disk_size,system_disk_type,data_disk_size,data_disk_type,bw,flow,peak_defence,ip_num,upgrade_range,gpu_num,ipv6_num')
                                        ->where($where)
                                        ->where('hidden', 0)
                                        ->order('order,id', 'asc')
                                        ->select()
