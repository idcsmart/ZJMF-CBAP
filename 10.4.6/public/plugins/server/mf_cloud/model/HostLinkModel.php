...
class HostLinkModel extends Model
{
    ...
    public function idcsmartCloudList($param)
    {
        ...
        $host = $this
            ->alias('hl')
            ->field('h.id,h.product_id,h.name,h.status,h.active_time,h.due_time,h.client_notes,pro.name product_name,c.'.$countryName.' country,c.iso country_code,dc.city,dc.area,hl.ip,hl.power_status,i.name image_name,ig.name image_group_name,hi.ip_num,hi.dedicate_ip,hi.assign_ip')
            ->join('host h', 'hl.host_id=h.id')
            ->leftJoin('product pro', 'h.product_id=pro.id')
            ->leftJoin('module_mf_cloud_data_center dc', 'hl.data_center_id=dc.id')
    ...
    public function adminDetail($id)
    {
        ...
        $data['config'] = ConfigModel::field('type,support_ssh_key,rand_ssh_port,rand_ssh_port_start,rand_ssh_port_end,rand_ssh_port_windows,rand_ssh_port_linux')
            ...
            if(isset($orderPage['data_center'][0]['city'][0]['area'][0]['recommend_config'][0])){
                ...
                if(empty($config['only_sale_recommend_config'])){
                    ...
                    if(!empty($buyParam['line_id'])){
                        ...
                        // 其他配置
                        if(isset($homeLineConfig['defence'][0]['value'])){
                            $buyParam['peak_defence'] = $homeLineConfig['defence'][0]['value'];
                        }
                        if(isset($homeLineConfig['gpu'][0]['value'])){
                            $buyParam['gpu_num'] = $homeLineConfig['gpu'][0]['value'];
                        }
                        ...
                        foreach($orderPage['limit_rule'] as $v){
                            ...
                            if($matchRule){
                                ...
                                if(isset($limitResult['cpu'])){
                                    $match = in_array($buyParam['cpu'], $limitResult['cpu']['value']);
                                    if($limitResult['cpu']['opt'] == 'neq'){
                                        $match = !$match;
                                    }
                                    // 不可用,在寻找可用的
                                    if(!$match){
                                        foreach($orderPage['cpu'] as $vv){
                                            $match = in_array($vv['value'], $limitResult['cpu']['value']);
                                            if($limitResult['cpu']['opt'] == 'neq'){
                                                $match = !$match;
                                            }
                                            if($match){
                                ...
                                if(isset($limitResult['memory'])){
                                    $min = intval($limitResult['memory']['min'] ?: 0);
                                    $max = intval($limitResult['memory']['max'] ?: 99999999);
                                    $match = $buyParam['memory'] >= $min && $buyParam['memory'] <= $max;
                                    if($limitResult['memory']['opt'] == 'neq'){
                                        $match = !$match;
                                    }
                                    // 不可用,在寻找可用的
                                    if(!$match){
                                        foreach($orderPage['memory'] as $vv){
                                            if($vv['type'] == 'radio'){
                                                $match = $vv['value'] >= $min && $vv['value'] <= $max;
                                                if($limitResult['memory']['opt'] == 'neq'){
                                                    $match = !$match;
                                                }
                                                if($match){
                                                ...
                                                if($limitResult['memory']['opt'] == 'eq'){
                                                    if($min >= $vv['min_value'] && $min <= $vv['max_value']){
                                                        $buyParam['memory'] = $min;
                                                        $changeParam = true;
                                                        break;
                                                    }else if($max >= $vv['min_value'] && $max <= $vv['max_value']){
                                                        $buyParam['memory'] = $vv['min_value'];
                                                        $changeParam = true;
                                                        break;
                                                    }
                                                }else if($limitResult['memory']['opt'] == 'neq'){
                                                    if($min > $vv['min_value']){
                                                        $buyParam['memory'] = $vv['min_value'];
                                                        $changeParam = true;
                                                        break;
                                                    }else if($max < $vv['max_value']){
                                                        $buyParam['memory'] = max($max + 1, $vv['min_value']);
                                                        $changeParam = true;
                                                        break;
                                                    }
                                                }
                                ...
                                if(isset($limitResult['image'])){
                                    $match = in_array($buyParam['image_id'], $limitResult['image']['id']);
                                    if($limitResult['image']['opt'] == 'neq'){
                                        $match = !$match;
                                    }
                                    // 不可用,在寻找可用的
                                    if(!$match){
                                        foreach($homeImageList['data']['list'] as $vv){
                                            foreach($vv['image'] as $vvv){
                                                $match = in_array($vvv['id'], $limitResult['image']['id']);
                                                if($limitResult['image']['opt'] == 'neq'){
                                                    $match = !$match;
                                                }
                                                if($match){
    ...
    public function hostUpdate($param)
    {
        ...
        if(!empty($hostLink)){
            ...
            if(isset($adminField['zjmf_cloud_id']) && isset($moduleAdminField['zjmf_cloud_id']) && is_numeric($moduleAdminField['zjmf_cloud_id']) && $adminField['zjmf_cloud_id'] != $moduleAdminField['zjmf_cloud_id']){
                ...
                if(!empty($update['rel_id'])){
                    ...
                    if($cloudDetail['status'] == 200){
                        ...
                        if(!isset($update['ip'])){
                            // 获取默认转发
                            if(empty($cloudDetail['data']['mainip']) && ($cloudDetail['data']['nat_acl_limit'] >= 0 || $cloudDetail['data']['nat_web_limit'] >= 0)){
                                $natAclList = $IdcsmartCloud->natAclList($update['rel_id'], ['page'=>1, 'per_page'=>1]);
                                if(isset($natAclList['data']['data']) && !empty($natAclList['data']['data'])){
                                    $cloudDetail['data']['mainip'] = $natAclList['data']['nat_host_ip'] . ':' . $natAclList['data']['data'][0]['ext_port'];
                                }
                            }
            ...
            if($ip_change){
                ...
                $res = $IdcsmartCloud->cloudModifyIpNum($id, ['num'=>(int)$moduleAdminField['ip_num'] + $baseIpNum, 'ip_group'=>$ipGroup ]);
                if($res['status'] == 200){
            ...
            $this->syncIp(['host_id'=>$param['host']['id'], 'id'=>$id], $IdcsmartCloud);
    ...
    public function currentConfig($hostId)
    {
        ...
        if(isset($configData['duration']['id'])){
            $durationId = DurationModel::where('num', $configData['duration']['num'])->where('unit', $configData['duration']['unit'])->value('id') ?? 0;
        ...
        if($hostLink['recommend_config_id'] > 0){
            ...
            // 获取当前数据盘
            $disk = DiskModel::where('host_id', $hostId)->order('is_free', 0)->select();
            foreach($disk as $v){
                $data['data_disk'][] = [
                    'rel_id'    => $v['rel_id'],
                    'size'      => $v['size'],
    ...
    public function syncIp($param, $IdcsmartCloud, $detail = NULL, $write_log = true)
    {
        ...
        if(empty($detail['data']['mainip']) && ($detail['data']['nat_acl_limit'] >= 0 || $detail['data']['nat_web_limit'] >= 0)){
            $natAclList = $IdcsmartCloud->natAclList($id, ['page'=>1, 'per_page'=>1]);
            if(isset($natAclList['data']['data']) && !empty($natAclList['data']['data'])){
