...
class LineModel extends Model
{
    ...
    public function lineCreate($param)
    {
        ...
        $this->startTrans();
        try{
            $line = $this->create($param, ['data_center_id','name','bill_type','bw_ip_group','defence_enable','defence_ip_group','ip_enable','link_clone','order','create_time','gpu_enable','gpu_name','ipv6_enable']);
    ...
    {
        $line = $this
                ->field('id,name,bill_type,bw_ip_group,defence_enable,defence_ip_group,ip_enable,link_clone,order,gpu_enable,gpu_name,ipv6_enable')
                ->find($id);
        if(empty($line)){
            return (object)[];
        }
    ...
    public function lineUpdate($param)
    {
        ...
        $this->update($param, ['id'=>$line['id']], ['name','bw_ip_group','defence_enable','defence_ip_group','ip_enable','link_clone','order','gpu_enable','gpu_name','ipv6_enable']);
    ...
    public function lineDelete($id)
    {
        ...
        try{
            ...
            $optionId = OptionModel::where('product_id', $productId)
                        ->whereIn('rel_type', [OptionModel::LINE_BW,OptionModel::LINE_FLOW,OptionModel::LINE_DEFENCE,OptionModel::LINE_IP,OptionModel::LINE_GPU,OptionModel::LINE_IPV6])
                        ->where('rel_id', $id)
                        ->column('id');
            if(!empty($optionId)){
    ...
    public function homeLineConfig($id)
    {
        ...
        if($line['ipv6_enable'] == 1){
            $data['ipv6'] = OptionModel::field('value')->where('product_id', $productId)->where('rel_type', OptionModel::LINE_IPV6)->where('rel_id', $id)->order('value', 'asc')->select()->toArray();
        }
        return $data;
    }
