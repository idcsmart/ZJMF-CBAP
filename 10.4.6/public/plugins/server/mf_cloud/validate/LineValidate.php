...
class LineValidate extends Validate
{
	protected $rule = [
        ...
        'gpu_enable'            => 'require|in:0,1',
        'gpu_name'              => 'requireIf:gpu_enable,1|length:1,100',
        'gpu_data'              => 'requireIf:gpu_enable,1|array|checkGpuData:thinkphp',
    ...
    protected $message = [
        ...
        'gpu_enable.require'            => 'mf_cloud_line_gpu_param_error',
        'gpu_enable.in'                 => 'mf_cloud_line_gpu_param_error',
        'gpu_name.requireIf'            => 'mf_cloud_line_gpu_name_require',
        'gpu_name.length'               => 'mf_cloud_line_gpu_name_length_error',
        'gpu_data.requireIf'            => 'mf_cloud_line_gpu_data_require',
        'gpu_data.array'                => 'mf_cloud_line_gpu_data_require',
    ...
    protected $scene = [
        'create' => ['data_center_id','name','bill_type','bw_ip_group','defence_enable','defence_ip_group','ip_enable','link_clone','bw_data','flow_data','defence_data','ip_data','order','gpu_enable','gpu_name','gpu_data','ipv6_enable'],
        'update' => ['id','name','bw_ip_group','defence_enable','defence_ip_group','ip_enable','link_clone','order','gpu_enable','gpu_name','ipv6_enable'],
    ];
    ...
    public function checkGpuData($value){
        $LineGpuValidate = new LineGpuValidate();
        foreach($value as $k=>$v){
            if (!$LineGpuValidate->scene('line_create')->check($v)){
                return $LineGpuValidate->getError();
            }
        }
        $optionValue = array_column($value, 'value');
        if( count($optionValue) != count( array_unique($optionValue) )){
            return 'mf_cloud_line_gpu_num_exist';
        }
        return true;
    }
