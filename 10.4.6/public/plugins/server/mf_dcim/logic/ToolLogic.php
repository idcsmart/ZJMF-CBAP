...
class ToolLogic
{
	 ...
	 * @author hh
	 * @version v1
	 * @param   string param - 要格式的参数 require
	 * @return  array
	 */
	 ...
	 * @author hh
	 * @version v1
	 * @param   string $subnetmask - 子网掩码 require
	 * @return  int
	 */
	 ...
	 * 时间 2024-02-18
	 * @title 格式化IP数量
	 * @desc 格式化IP数量,支持1_2,3_4
	 * @author hh
	 * @version v1
	 * @param   string $ip_num - IP数量
	 * @return  bool|array
	 */
	public static function formatDcimIpNum($ip_num = '')
	{
	    if(strpos($ip_num, ',') !== false){
	        $arr = explode(',', $ip_num);
	        $res = [];
	        foreach($arr as $v){
	            $r = self::formatDcimIpNum($v);
	            if($r === false){
	                return false;
	            }
	            foreach($r as $kk=>$vv){
	    ...
	    }else if(strpos($ip_num, '_') !== false){
	        $arr = explode('_', $ip_num);
	        if(count($arr) != 2 || !is_numeric($arr[0]) || !is_numeric($arr[1]) || preg_match('/[^0-9]/', $arr[0]) || preg_match('/[^0-9]/', $arr[1]) || $arr[0]<1 || $arr[0] > 10000){
	            return false;
	        }
	        return [$arr[1]=>$arr[0]];
	    }else{
	...
	public static function unzipToReserver(){
		$zip = WEB_ROOT . 'plugins/server/mf_dcim/data/abc.zip';
		$target_dir = WEB_ROOT . 'plugins/reserver/mf_dcim/';
	...
	public static function createEditLog($old, $new, $des, $no_detail = [], $after_update_value = []){
	    ...
	    foreach($new as $k=>$v){
	        ...
	        if(in_array($k, $after_update_value)){
	            $res .= lang_plugins('mf_dcim_log_modify', ['{name}'=>$des[$k], '{value}'=>$v]);
	            continue;
	        }
	        $old[$k] = $old[$k] ?? '';
	        if(isset($des[$k]) && $v != $old[$k] && !empty($des[$k])){
	            ...
	            $res .= lang_plugins('mf_dcim_log_common_modify', [
	                '{name}'=>$des[$k],
	                '{old}'=>$old[$k],
	                '{new}'=>$v,
	            ]);
	 ...
	 * 时间 2024-02-18
	 * @title 特殊的字符串分别使用多语言
	 * @desc 特殊的字符串分别使用多语言,如a_b;c_d,a,c应用多语言
	 * @author hh
	 * @version v1
	 * @param   string $str - 字符串 require
	 * @return  string
	 */
	public static function packageConfigLanguage($str)
	{
		$arr = explode(';', $str);
		$res = [];
		foreach($arr as $v){
	 ...
	 * @author hh
	 * @version v1
	 * @param   string ip_num - IP数量 require
	 * @return  string
	 */
	public static function getIpNumShow($ip_num, $nc_desc = '')
	{
		if(strpos($ip_num, '_') !== false){
            ...
            $num = $nc_desc !== '' ? $nc_desc : lang_plugins('mf_dcim_real_ip');
        }else{
            $num = $ip_num . lang_plugins('mf_dcim_indivual');
        }
        return $num;
	}
