...
class HostLinkModel extends Model
{
    ...
    * @param   string param.keywords - 关键字搜索,搜索套餐名称/主机名/IP
    ...
    */
    public function idcsmartCloudList($param)
    {
        ...
        $host = $this
            ->alias('hl')
            ->field('h.id,h.product_id,h.name,h.status,h.active_time,h.due_time,h.client_notes,pro.name product_name,c.'.$countryName.' country,c.iso country_code,dc.city,dc.area,hl.ip,hl.power_status,i.name image_name,ig.name image_group_name,ig.icon,hi.ip_num,hi.dedicate_ip,hi.assign_ip')
            ->join('host h', 'hl.host_id=h.id')
            ->leftJoin('product pro', 'h.product_id=pro.id')
            ->leftJoin('module_mf_dcim_data_center dc', 'hl.data_center_id=dc.id')
     ...
     * @return  string data.additional_ip - 附加IP(英文分号分割)
                ...
                foreach($orderPage['limit_rule'] as $v){
                    ...
                    if($matchRule){
                        ...
                        if(isset($limitResult['model_config'])){
                            $match = in_array($buyParam['model_config_id'], $limitResult['model_config']['id']);
                            if($limitResult['model_config']['opt'] == 'neq'){
                                $match = !$match;
                            }
                            // 不可用,在寻找可用的
                            if(!$match){
                                foreach($orderPage['model_config'] as $vv){
                                    $match = in_array($vv['id'], $limitResult['model_config']['id']);
                                    if($limitResult['model_config']['opt'] == 'neq'){
                                        $match = !$match;
                                    }
                                    if($match){
                        ...
                        if($lineType == 'bw' && isset($limitResult['bw'])){
                            $min = intval($limitResult['bw']['min'] ?: 0);
                            $max = intval($limitResult['bw']['max'] ?: 99999999);
                            $match = $buyParam['bw'] >= $min && $buyParam['bw'] <= $max;
                            if($limitResult['bw']['opt'] == 'neq'){
                                $match = !$match;
                            }
                            // 不可用,在寻找可用的
                            if(!$match){
                                if(isset($homeLineConfig['bw'])){
                                    foreach($homeLineConfig['bw'] as $vv){
                                        if($vv['type'] == 'radio'){
                                            ...
                                            if($limitResult['bw']['opt'] == 'eq'){
                                                if($min >= $vv['min_value'] && $min <= $vv['max_value']){
                                                    $buyParam['bw'] = $min;
                                                    $changeParam = true;
                                                    break;
                                                }else if($max >= $vv['min_value'] && $max <= $vv['max_value']){
                                                    $buyParam['bw'] = $vv['min_value'];
                                                    $changeParam = true;
                                                    break;
                                                }
                                            }else if($limitResult['bw']['opt'] == 'neq'){
                                                if($min > $vv['min_value']){
                                                    $buyParam['bw'] = $vv['min_value'];
                                                    $changeParam = true;
                                                    break;
                                                }else if($max < $vv['max_value']){
                                                    $buyParam['bw'] = max($max + 1, $vv['min_value']);
                                                    $changeParam = true;
                                                    break;
                                                }
                                            }
                        ...
                        if($lineType == 'flow' && isset($limitResult['flow'])){
                            $min = intval($limitResult['flow']['min'] ?: 0);
                            $max = intval($limitResult['flow']['max'] ?: 99999999);
                            $match = $buyParam['flow'] >= $min && $buyParam['flow'] <= $max;
                            if($limitResult['flow']['opt'] == 'neq'){
                                $match = !$match;
                            }
                            // 不可用,在寻找可用的
                            if(!$match){
                                if(isset($homeLineConfig['flow'])){
                                    foreach($homeLineConfig['flow'] as $vv){
                                        if($vv['type'] == 'radio'){
                                            ...
                                            if($limitResult['flow']['opt'] == 'eq'){
                                                if($min >= $vv['min_value'] && $min <= $vv['max_value']){
                                                    $buyParam['flow'] = $min;
                                                    $changeParam = true;
                                                    break;
                                                }else if($max >= $vv['min_value'] && $max <= $vv['max_value']){
                                                    $buyParam['flow'] = $vv['min_value'];
                                                    $changeParam = true;
                                                    break;
                                                }
                                            }else if($limitResult['flow']['opt'] == 'neq'){
                                                if($min > $vv['min_value']){
                                                    $buyParam['flow'] = $vv['min_value'];
                                                    $changeParam = true;
                                                    break;
                                                }else if($max < $vv['max_value']){
                                                    $buyParam['flow'] = max($max + 1, $vv['min_value']);
                                                    $changeParam = true;
                                                    break;
                                                }
                                            }
