...
function upgradePackagePrice(type, params) {
  if (type === 'custom') {
    ...
    return Axios.get(`/remf_cloud/${params.id}/recommend_config/price`, { params });
  }
}
// 生成升降级套餐订单
function upgradeOrder(type, params) {
  if (type === 'custom') {
    ...
    return Axios.post(`/remf_cloud/${params.id}/recommend_config/order`, params);
  }
}
// 获取是否救援系统
...
function changeSimulatePhysical(params) {
  return Axios.post(`/remf_cloud/${params.id}/simulate_physical_machine`, params);
}
