...
class CloudController
{
    ...
    public function batchOperate()
    {
        ...
        foreach ($id as $v) {
            $res = reserver_api('MfFinance', 'cloud', $action[$param['action']], ['id' => (int)$v]);
    ...
    *  @return  string list[].assign_ip - 附加IP(英文逗号分隔)
    ...
    public function list(){
        ...
        $host = HostModel::alias('h')
            ->field('h.id,h.product_id,h.name,h.status,h.active_time,h.due_time,p.name product_name,h.client_notes,hi.ip_num,hi.dedicate_ip,hi.assign_ip')
            ->leftJoin('product p', 'h.product_id=p.id')
