      ...
      if (temp.length > 0) {
        ...
        const modelArr = temp.reduce((all, cur) => {
          if (cur.result.model_config.opt === "eq") {
            ...
            const result = _temp.filter(
              (item) => !cur.result.model_config.id.includes(item)
            );
            all.push(result);
          }
          return all;
      ...
      const bwArr = temp.reduce((all, cur) => {
        if (cur.result.bw.opt === "eq") {
          all.push(
            this.createArr([
              cur.result.bw.min * 1,
              cur.result.bw.max === "" ? maxBw : cur.result.bw.max * 1,
            ])
          );
        } else {
      ...
      if (temp.length > 0) {
        ...
        const flowArr = temp.reduce((all, cur) => {
          if (cur.result.flow.opt === "eq") {
            all.push(
              this.createArr([
                cur.result.flow.min * 1,
                cur.result.flow.max === ""
                  ? maxFlow[maxFlow.length - 1]
                  : cur.result.flow.max * 1,
              ])
            );
          } else {
            let result = this.createArr([
  ...
  methods: {
    ...
    async changeConfig() {
      try {
        ...
        if (!params.config_options.image_id) {
