...
new Vue({
  ...
  methods: {
    ...
    editLimit (row, type) {
      ...
      this.limitModel = true;
      this.optType = type;
      this.checkedLimit = row.type.split(',');
      const temp = JSON.parse(JSON.stringify(row)).rule;
      const typeArr = ["memory", "system_disk", "data_disk", "bw", "flow", "ipv4_num", "ipv6_num"];
      ...
      this.limitForm = Object.assign(
        JSON.parse(JSON.stringify(this.originLimitForm)),
        { id: row.id },
        temp
      );
    },
    async submitLimit ({ validateResult, firstError }) {
      if (validateResult === true) {
        try {
          ...
          const result = this.checkedLimit.reduce((all, cur) => {
            if (Object.keys(temp).includes(cur)) {
              all[cur] = temp[cur];
            }
            return all;
          }, {});
          ...
          // 套餐
          if (this.checkedLimit.includes('recommend_config')) {
            params.rule = {
              recommend_config: {
                id: this.limitForm.recommend_config.id
              },
              duration: {
                id: this.limitForm.duration.id
              }
            };
          }
          delete params.rule.id;
          this.submitLoading = true;
