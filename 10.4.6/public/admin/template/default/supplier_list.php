...
<div id="content" class="supplier_list" v-cloak>
  <com-config>
    <t-card class="list-card-container">
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" @sort-change="sortChange" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" display-type="fixed-width">
        ...
        <template #auto_update_rate="{row}">
          <t-switch size="medium" :custom-value="[1,0]" :value="row.auto_update_rate" @change="changeStatus($event, row)"></t-switch>
          <t-tooltip :show-arrow="false" theme="light" placement="top-right">
            <template slot="content">
              <p class="tip">{{lang.upstream_text79}}：</p>
              <p class="tip">{{ row.rate_update_time>0 ? moment(row.rate_update_time * 1000).format('YYYY-MM-DD HH:mm') : '--'}}</p>
            </template>
            <t-icon name="help-circle"></t-icon>
          </t-tooltip>
        </template>
    ...
    <!-- 配置弹窗 -->
    <t-dialog :header="lang.upstream_text42" :visible.sync="configVisble" :footer="false" width="650" @closed="diaClose" class="config-dialog">
    ...
    <!-- 汇率 -->
    <t-dialog :header="lang.upstream_text81" :visible.sync="rateVisible" :footer="false" width="550" @closed="diaClose" class="config-dialog">
      <t-form :rules="rules" ref="userDialog" :data="formData" @submit="submitRate" :label-width="120" reset-type="initial">
        ...
        <t-form-item :label="lang.upstream_text79">
          <t-input :value="formData.rate_update_time>0 ? moment(formData.rate_update_time * 1000).format('YYYY-MM-DD HH:mm') : '--'" disabled></t-input>
        </t-form-item>
        ...
        <t-form-item :label="lang.upstream_text77" name="rate">
          <t-input-number v-model="formData.rate" theme="normal" :min="0.00001"
          :decimal-places="5" :placeholder="lang.upstream_text80" @blur="changeRate">
          </t-input-number>
        </t-form-item>
        <t-form-item label=" ">
