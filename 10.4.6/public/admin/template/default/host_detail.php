    ...
    <t-card class="list-card-container">
      <div class="com-h-box">
        <ul class="common-tab">
          ...
          <li v-if="$checkPermission('auth_user_detail_notification_log_sms_notification') || $checkPermission('auth_user_detail_notification_log_email_notification')">
            <a :href="`${baseUrl}/${($checkPermission('auth_user_detail_notification_log_sms_notification') ? 'client_notice_sms' : 'client_notice_email')}.htm?id=${client_id}`">{{lang.notice_log}}</a>
          </li>
        ...
        <div class="user">
          ...
          <t-button class="btn-transfer" v-if="hasTransfer && $checkPermission('auth_business_host_detail_host_transfer')" @click="handelTransfer">
          ...
          <t-row :gutter="{ xs: 8, sm: 16, md: 24, lg: 32, xl: 32, xxl: 90 }">
            ...
            <t-col :xs="12" :xl="6">
              ...
              <div class="module-opt" style="display: flex;column-gap: 20px;">
                ...
                <div class="right">
                  ...
                  <t-button @click="jumpToTicket" v-if="(hasTicket || hasNewTicket) && $checkPermission('auth_user_detail_ticket_view')">
              ...
              <template v-for="(item,index) in hostFieldList">
                ...
                <div class="config-item render">
                  <t-form-item :label="el.name" v-for="(el,ind) in item.field" :key="ind">
                    ...
                    <template v-else>
                      <t-date-picker enable-time-picker allow-input v-model="el.value" format="YYYY-MM-DD hh:mm" v-if="el.type === 'date'"></t-date-picker>
                      <t-input v-model="el.value" :disabled="el.disable" v-else></t-input>
                    </template>
                    <!-- 手动资源 -->
                    <t-button class="distribute" @click="handlerDistribute" v-if="el.key === 'manual_resource' && $checkPermission('auth_business_host_detail_dcim_host_allot')">{{lang.distribute}}
                    </t-button>
                    <!-- 机柜租用 -->
                    <t-button class="distribute" @click="handlerCabinet" v-if="el.key === 'zjmf_dcim_cabinet_id'">{{lang.distribute}}
                    </t-button>
                    <!-- DCIM机器 -->
                    <template v-if="el.key === 'zjmf_dcim_id'">
                      <!-- 分配 -->
                      <t-button class="distribute" @click="showDcim" v-if="curDcimId === '0' && $checkPermission('auth_business_host_detail_dcim_host_allot')">{{lang.distribute}}
                      </t-button>
                      <template v-else>
            ...
            <t-col :xs="24" :xl="12">
              <div class="footer-btn">
                <t-button theme="primary" type="submit" @click="updateUserInfo" :loading="isLoading" v-permission="'auth_business_host_detail_save_basic_finance_info'">{{lang.hold}}</t-button>
                <t-button theme="default" variant="base" @click="back" v-permission="'auth_business_host_detail_delete'">{{lang.delete}}</t-button>
              </div>
            </t-col>
            <!-- 内页模块 -->
    ...
    <t-dialog :header="lang.host_transfer_text11" :close-btn="false" :close-on-overlay-click="false" :visible.sync="transferVisible" width="800" :footer="false">
      <t-form :data="transferForm" ref="transferInfo" :rules="transferRules" @submit="onTransferSubmit" label-align="top">
        <t-form-item :label="lang.host_transfer_text13" name="client_id">
          ...
          <com-choose-user @changeuser="changeProUser" class="com-clinet-choose" style="position: relative;right: 0; width: 100%;">
          </com-choose-user>
    ...
    <t-dialog :header="distributeTitle" :visible.sync="resourceDialog" width="80%" :footer="false">
      <div class="left-search">
        <template v-if="distributeType === 'manual'">
          <t-input v-model="resourceForm.keywords" class="search-input" :placeholder="lang.manual_text1" @keypress.enter.native="getResourcesList" :on-clear="clearKey" clearable></t-input>
        ...
        <template v-if="distributeType === 'dcim'">
          <t-input v-model="dcimForm.ip" class="search-input" :placeholder="`${lang.input}IP`" @keypress.enter.native="getDcimList" :on-clear="clearIp" clearable></t-input>
      ...
      <t-table row-key="index" :data="distributeType === 'manual' ? resourceList : dcimList" :columns="calcColumns" :loading="resourceLoading" :hover="true">
        <template #id="{row}">
          <a :href="row.dcim_url" target="_blank" class="common-look" v-if="distributeType === 'dcim'">{{row.id}}-{{row.wltag}}</a>
        ...
        <template #status="{row}">
          <span :class="`dcim-status-${row.status}`" v-if="distributeType === 'dcim'">{{calcDcimStatus(row.status)}}</span>
          <span v-if="distributeType === 'cabinet'" :class="{'dcim-status-2': row.status === 2, 'dcim-status-3': row.status === 1}">
        ...
        <template #host_id="{row}">
          <a :href="`${baseUrl}/host_detail.htm?client_id=${row.client_id}&id=${row.host_id}`" target="_blank" class="common-look" v-if="row.host_id">{{row.host_id}}</a>
          <span v-else>--</span>
        </template>
        <template #client_id="{row}">
          <a :href="`${baseUrl}/client_detail.htm?id=${row.client_id}`" target="_blank" class="common-look" v-if="row.client_id">{{row.client_id}}</a>
          <span v-else>--</span>
        </template>
        <template #opt="{row}">
          <template v-if="distributeType === 'manual'">
              ...
              <t-tooltip :content="lang.idle" :show-arrow="false" theme="light" v-if="row.host_id">
                <a class="common-look" @click="optItem(row, 'free')">
                  <svg class="common-look">
                    <use xlink:href="/{$template_catalog}/template/{$themes}/img/icon/icons.svg#cus-free">
                    </use>
                  </svg>
                </a>
              </t-tooltip>
            ...
            <template v-else>
              <t-tooltip :content="lang.distribute" :show-arrow="false" theme="light" v-if="!row.host_id">
                <a class="common-look" @click="optItem(row, 'allot')">
                  <svg class="common-look">
                    <use xlink:href="/{$template_catalog}/template/{$themes}/img/icon/icons.svg#cus-allot">
                    </use>
                  </svg>
                </a>
              </t-tooltip>
      ...
      <t-pagination show-jumper :total="dcimTotal" v-if="dcimTotal" :current="dcimForm.page" :page-size="dcimForm.limit" :page-size-options="pageSizeOptions" :on-change="changePage" />
    </t-dialog>
    <!-- 分配/空闲 -->
