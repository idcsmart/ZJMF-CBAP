  ...
  <com-config>
    <t-card class="list-card-container">
      <div class="common-header">
        <div class="flex">
          <t-button @click="addFlow" class="add" v-permission="'auth_business_transaction_create_transaction'">{{lang.new_flow}}</t-button>
        </div>
        <!-- 右侧搜索 -->
      ...
      <!-- 高级搜索 end -->
      <t-table row-key="id" :data="calcList" size="medium" :columns="columns" :hover="hover" resizable :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" @sort-change="sortChange" :hide-sort-tips="true">
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.edit" :show-arrow="false" theme="light">
            <t-icon name="edit" size="18px" @click="updateFlow(row)" class="common-look" v-permission="'auth_business_transaction_update_transaction'"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
            <t-icon name="delete" size="18px" @click="delteFlow(row)" class="common-look" v-permission="'auth_business_transaction_delete_transaction'"></t-icon>
          </t-tooltip>
        </template>
      </t-table>
    ...
    <t-dialog :header="optTitle" :visible.sync="flowModel" :footer="false" width="600">
      <t-form :data="formData" ref="form" @submit="onSubmit" :rules="rules" v-if="flowModel">
        <t-form-item :label="lang.user" name="client_id" class="user">
          <com-choose-user :check-id="formData.client_id" :pre-placeholder="lang.example" @changeuser="changeUser" :user-id="formData.client_id">
          </com-choose-user>
        </t-form-item>
