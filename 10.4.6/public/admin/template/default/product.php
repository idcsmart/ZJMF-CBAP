...
<div id="content" class="product" v-cloak>
  <com-config>
    <t-card class="list-card-container" :class="{stop: data.status===0}">
      ...
      :tree="{ treeNodeColumnIndex: $checkPermission('auth_product_management_list_order') ? 1 : 0}" :loading="loading" :hover="hover"
      :tree-expand-and-fold-icon="treeExpandAndFoldIconRender" :before-drag-sort="beforeDragSort" :key="expandNum"
       @abnormal-drag-sort="onAbnormalDragSort" @drag-sort="changeSort" class="product-table" :row-class-name="rowName"
