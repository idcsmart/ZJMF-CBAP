  ...
  window.onload = function () {
    ...
    new Vue({
      ...
      created() {
        this.id =
        this.formData.id =
        this.moneyPage.order_id = location.href.split("?")[1].split("=")[1];
        // 所有之前跳转订单详情都是 order_details
        if (
          !this.$checkPermission("auth_business_order_detail_order_detail_view")
        ) {
          const clientAuth = [
            { auth: "auth_business_order_detail_refund_record_view", url: "order_refund" },
            { auth: "auth_business_order_detail_transaction", url: "order_flow" },
