  ...
  <com-config>
    <t-card class="list-card-container">
      <div class="common-header">
        <div class="top-btn">
          <div class="left">
            <t-button class="add" @click="toMarket" v-permission="'auth_app_list_more_app'">{{lang.more_plugins}}</t-button>
            ...
            <t-button v-if="!isNeedUpgrade && $checkPermission('auth_app_list_sync_app')" @click="getSystem" :loading="btnLoading">{{lang.sync_plugin}}</t-button>
          </div>
          <t-button class="add" @click="handleHook" :loading="hookLoading" v-permission="'auth_app_list_plugin_hook_order'">{{lang.hook_sort}}</t-button>
        </div>
      ...
      <t-table row-key="id" :data="filterPluginList" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" @sort-change="sortChange" display-type="fixed-width" :hide-sort-tips="true" :pagination="pagination" @page-change="onPageChange">
        ...
        <template #title="{row}">
          <a class="aHover" :href="`${basrUrl}${row.url}`" @contextmenu="jumpMenu($event, row); return true;" v-if="row.url" @click="jumpMenu($event, row)">
            {{row.title}}
          </a>
        ...
        <template #version="{row}">
          {{row.version}}
          <t-tooltip :content="row.error_msg ? lang.upload_text15 : lang.upgrade_plugin" :show-arrow="false" theme="light" v-if="row.isUpdate">
        ...
        <template #op="{row}">
          <template v-if="row.module === 'addon'">
            <t-tooltip :content="enableTitle(row.status)" :show-arrow="false" theme="light">
              <a class="common-look" @click="changeStatus(row)" v-if="row.status !== 3 && $checkPermission('auth_app_list_deactivate_enable_app')">
                <svg class="common-look" v-if="row.status === 0 && $checkPermission('auth_app_list_deactivate_enable_app')">
                  <use xlink:href="/{$template_catalog}/template/{$themes}/img/icon/icons.svg#cus-enable">
                  </use>
                </svg>
                <svg class="common-look" v-else-if="row.status === 1 && $checkPermission('auth_app_list_deactivate_enable_app')">
            ...
            <t-tooltip :content="installTitle(row.status)" :show-arrow="false" theme="light">
              <a class="common-look" @click="installHandler(row)" v-if="$checkPermission('auth_app_list_install_uninstall_app')">
                <svg class="common-look" v-if="row.status === 3 && $checkPermission('auth_app_list_install_uninstall_app')">
                  <use xlink:href="/{$template_catalog}/template/{$themes}/img/icon/icons.svg#cus-install">
                  </use>
                </svg>
                <svg class="common-look" v-else-if="row.status !== 3 && $checkPermission('auth_app_list_install_uninstall_app')">
    ...
    <t-dialog :header="lang.sync_plugin" :visible.sync="syncVisible" width="600" :footer="false">
      <div class="table-btn" style="text-align: right; margin: 10px 0;" v-if="$checkPermission('auth_app_list_sync_app_download_upgrade')">
        <t-button theme="primary" @click="handlerAllUpgrade" :loading="isAllLoading">{{lang.custom_tip_text3}}</t-button>
      </div>
      <t-table row-key="id" :data="syncPluginList" size="medium" :columns="pluginColumns" :hover="hover" :loading="btnLoading" :table-layout="tableLayout ? 'auto' : 'fixed'" @sort-change="sortChange" display-type="fixed-width" :hide-sort-tips="true" :max-height="500">
        ...
        <template #op="{row, rowIndex}">
          ...
          <template v-else>
            <t-tooltip :content="row.error_msg ? lang.upload_text15 : lang.download" :show-arrow="false" theme="light" v-if="row.downloaded * 1 === 0 && $checkPermission('auth_app_list_sync_app_download_upgrade')">
              <a class="common-look" @click="handlerDownload(row)" :class="{'not-allowed': row.error_msg}">
                <t-icon name="arrow-down"></t-icon>
              </a>
            </t-tooltip>
            <t-tooltip :content="row.error_msg ? lang.upload_text15 : lang.upgrade_plugin" :show-arrow="false" theme="light" v-else-if="row.downloaded * 1 === 1 && row.upgrade*1 === 1 && $checkPermission('auth_app_list_sync_app_download_upgrade')">
              <a class="common-look" @click="handlerDownload(row)" :class="{'not-allowed': row.error_msg}">
