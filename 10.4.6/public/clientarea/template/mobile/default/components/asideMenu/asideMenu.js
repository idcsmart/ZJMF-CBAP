...
const asideMenu = {
  template: ` <el-aside width="190px">
    ...
    <el-menu class="menu-top" :default-active="menuActiveId" @select="handleSelect" background-color="#1E41C9" text-color="rgba(255, 255, 255, .6)" active-text-color="#FFF">
        <template v-for="(item,index) in menu1">
            ...
            <!-- 有二级菜单 -->
            <el-submenu v-else :key="item.id" :index="item.url" :id="item.url">
                <template slot="title">
                    <i class="iconfont" :class="item.icon"></i>
                    <span class="aside-menu-text" slot="title">{{item.name}}</span>
                </template>
  ...
  methods: {
    ...
    doGetMenu() {
      getMenu().then((res) => {
        if (res.data.status === 200) {
          ...
          this.originMenu = res.data.data.menu;
    ...
    handleSelect(key) {
      ...
      // 跳转到对应路径
      const temp = this.originMenu.filter((item) => item.url === key) || [];
      const reg =
        /^(((ht|f)tps?):\/\/)([^!@#$%^&*?.\s-]([^!@#$%^&*?.\s]{0,63}[^!@#$%^&*?.\s])?\.)+[a-z]{2,6}\/?/;
      if (reg.test(key)) {
        if (temp[0]?.second_reminder === 1) {
          return window.open(`/transfer.htm?target=${encodeURIComponent(key)}`);
        } else {
