...
class Captcha
{
     ...
     * @return Response
     */
    public function create(array $custom, string $config = null, bool $api = false): Response
    {
        $this->configure($config);

        # 自定义配置 wyh 20220519新增
        ...
        $GLOBALS['captcha']=strtoupper(implode('',$text));

        foreach ($text as $index => $char) {

            $x     = $this->fontSize * ($index + 1) * 1 * ($this->math ? 1 : 1.5); # wyh 20220519 将随机数改为1,否则会出现顺序问题
            $y     = $this->fontSize + 15;
            $angle = $this->math ? 0 : mt_rand(-40, 40);

            imagettftext($this->im, $this->fontSize, $angle, floor($x), floor($y), $this->color, $fontttf, $char);
        }
