    ...
    public function systemList()
    {
        ...
        foreach($configuration as $v){
            if(in_array($v['setting'], $this->config['system'])){
                if($v['setting'] == 'lang_home_open' || $v['setting'] == 'maintenance_mode' || $v['setting'] == 'client_start_id_value' || $v['setting'] == 'order_start_id_value' || $v['setting']=='home_show_deleted_host'){
        ...
        foreach($configuration as $v){
            if(in_array($v['setting'], $this->config['login'])){
                if($v['setting']=='email_suffix'){
                    $data[$v['setting']] = (string)$v['value'];
                }else if(in_array($v['setting'], ['home_login_ip_exception_verify','home_enforce_safe_method','admin_enforce_safe_method'])){
    ...
    public function loginUpdate($param)
    {
        ...
        try {
            foreach($this->config['login'] as $v){
                if(in_array($v, ['home_login_ip_exception_verify','home_enforce_safe_method','admin_enforce_safe_method'])){
