...
/**
 * @title 模板控制器-云服务器优惠模型
 * @desc 模板控制器-云服务器优惠模型
 * @use app\common\model\CloudServerDiscountModel
 */
class CloudServerDiscountModel extends Model
{
    protected $name = 'cloud_server_discount';
    ...
    public function createDiscount($param)
    {
        ...
        try {
            ...
            # 记录日志
            active_log(lang('log_add_cloud_server_discount', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{name}' => $param['title']]), 'cloud_server_discount', $discount->id);
    ...
    public function updateDiscount($param)
    {
        ...
        if(empty($discount)){
            return ['status'=>400, 'msg'=>lang('cloud_server_discount_not_exist')];
        }
        ...
        try {
            ...
            $desc = [
                'title'         => lang('cloud_server_discount_title'),
                'description'   => lang('cloud_server_discount_description'),
                'url'           => lang('cloud_server_discount_url'),
            ];
            ...
            if(!empty($description)){
                $description = lang('log_update_cloud_server_discount', [
                    '{admin}' => 'admin#'.get_admin_id().'#'.request()->admin_name.'#',
                    '{name}'   => $discount['title'],
                    '{detail}' => implode(',', $description),
                ]);
                active_log($description, 'cloud_server_discount', $discount->id);
            }
    ...
    public function deleteDiscount($id)
    {
        ...
        try {
            # 记录日志
            active_log(lang('log_delete_cloud_server_discount', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{name}' => $discount['title']]), 'cloud_server_discount', $discount->id);
