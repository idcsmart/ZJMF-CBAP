...
class ClientModel extends Model
{
    ...
    public function clientList($param)
    {
        ...
        $adminField = $AdminFieldModel->adminFieldIndex(['view'=>'client']);
        ...
        if($param['orderby'] == 'host_active_num'){
            ...
            $costPrice = $this
                ->field('c.id,sum(o.amount) cost_price')
                ->alias('c')
                ->where($where)
                ->leftJoin('order o', 'c.id=o.client_id AND o.status="Paid"')
                ->limit($param['limit'])
                ->page($param['page'])
                ->order($param['orderby'], $param['sort'])
                ->group('c.id')
        ...
        if(isset($ids) && !empty($ids)){
            $clients = $this
                ->alias('c')
                ->field('c.id,c.username,c.email,c.phone_code,c.phone,c.status,c.create_time reg_time,c.country,c.address,c.company,c.language,c.notes,c.credit')
                ->whereIn('c.id', $ids)
            ...
            if(!isset($costPrice)){
                $costPrice = OrderModel::field('sum(amount) cost_price,client_id')->whereIn('client_id', $clientId)->where('status', 'Paid')->group('client_id')->select()->toArray();
    ...
    public function indexClient($id)
    {
        ...
        $client = $this->field('id,username,email,phone_code,phone,company,country,address,language,notes,client_notes,status,create_time register_time,last_login_time,last_login_ip,credit,operate_password,notice_open,notice_method')->find($id);
        if (empty($client)){
            return (object)[]; // 转换为对象
        }
    ...
    public function indexClient2($id)
    {
        ...
        $client['host_num'] = HostModel::where('client_id', $id)->where('is_delete', 0)->count();  // 产品数量
        $client['host_active_num'] = HostModel::where('status', 'Active')->where('client_id', $id)->where('is_delete', 0)->count(); // 已激活产品数量
        $client['unpaid_order'] = OrderModel::where('client_id', $id)->where('status', 'Unpaid')->where('is_recycle', 0)->count(); // 未支付订单
        $client['consume'] = amount_format(TransactionModel::where('client_id', $id)->where('amount', '>', 0)->sum('amount')); // 消费
