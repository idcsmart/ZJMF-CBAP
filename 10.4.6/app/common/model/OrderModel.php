...
class OrderModel extends Model
{
    ...
    public function orderList($param, $scene = '')
    {
        ...
        if($app=='home'){
            ...
            $adminField = $AdminFieldModel->adminFieldIndex(['view'=>'order']);
    ...
    public function indexOrder($id)
    {
        ...
        $order = $this->field('id,type,amount,create_time,pay_time,status,gateway_name gateway,gateway plugin_name,credit,client_id,notes,refund_amount,amount_unpaid,admin_id,is_recycle')->find($id);
        if (empty($order)){
            return (object)[]; // 转换为对象
        }

        if (plugin_method_exist($order['plugin_name'],'gateway','handle_refund')){
        ...
        if($app=='home'){
            ...
            // wyh 20240607 改
            $refundAmountAll = RefundRecordModel::where('order_id', $id)
                ->whereIn('status',['Refunded','Pending','Refunding'])
                ->sum('amount'); // 订单已退款金额
            $order['refundable_amount'] = amount_format($order['amount']-$refundAmountAll);
        }
...
//            }
     ...
     * @version v1
     * @param int id - 订单ID required
     * @param string type - 退款类型credit退款到余额,transaction退款到流水,original原支付路径 required
    ...
    public function orderRefund($param)
    {
        ...
        $param['type'] = $param['type'] ?? '';
        if(!in_array($param['type'], ['credit', 'transaction','original'])){
            return ['status' => 400, 'msg' => lang('param_error')];
        }

        if(in_array($param['type'],['transaction'])){
            $param['gateway'] = $param['gateway'] ?? '';
...
//        }
        ...
        try {
            ...
            if ($order['type']=='recharge'){
                throw new \Exception(lang('recharge_order_cannot_refund'));
            }
            ...
            if ($param['type']=='original'){
                // 获取支付接口名称
                $gateway = PluginModel::where('module', 'gateway')->where('name', $order['gateway'])->find();
                if(empty($gateway)){
            ...
            $refundAmountAll = RefundRecordModel::where('order_id', $id)
                ...
                ->sum('amount');
            if(($param['amount']+$refundAmountAll)>$order['amount']){
                throw new \Exception(lang('refund_amount_not_enough'));
            }
...
//            if(in_array($param['type'],['transaction','original'])){
...
//                'admin_id'              => $adminId,
//                'type'                  => $param['type'],
//                'transaction_id'        => in_array($param['type'],['transaction','original']) ? $transaction->id : 0,
//                'amount'                => $param['amount'],
//                'create_time'           => time(),
//                'status'                => 'Pending',
//                //'status'                => 'Refunded',
...
//            }
//
//            // TODO wyh 20240403 新增 自动退款
//            if ($param['type']=='original'){
//                $tran = TransactionModel::where('order_id', $id)->find();
//                $refundData = [
