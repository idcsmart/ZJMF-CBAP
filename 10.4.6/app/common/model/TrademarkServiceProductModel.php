...
/**
 * @title 模板控制器-商标延伸服务商品模型
 * @desc 模板控制器-商标延伸服务商品模型
 * @use app\common\model\TrademarkServiceProductModel
 */
class TrademarkServiceProductModel extends Model
{
    protected $name = 'trademark_service_product';
    ...
    public function createProduct($param)
    {
        ...
        try {
            ...
            # 记录日志
            active_log(lang('log_add_trademark_service_product', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{name}' => $param['title']]), 'trademark_service_product', $product->id);
    ...
    public function updateProduct($param)
    {
        ...
        if(empty($product)){
            return ['status'=>400, 'msg'=>lang('trademark_service_product_not_exist')];
        }
        ...
        try {
            ...
            $desc = [
                'title'         => lang('trademark_service_product_title'),
                'description'   => lang('trademark_service_product_description'),
                'price'         => lang('trademark_service_product_price'),
                'product_id'    => lang('trademark_service_product_product_id'),
            ];
            ...
            if(!empty($description)){
                $description = lang('log_update_trademark_service_product', [
                    '{admin}' => 'admin#'.get_admin_id().'#'.request()->admin_name.'#',
                    '{name}'   => $product['title'],
                    '{detail}' => implode(',', $description),
                ]);
                active_log($description, 'trademark_service_product', $product->id);
            }
    ...
    public function deleteProduct($id)
    {
        ...
        try {
            # 记录日志
            active_log(lang('log_delete_trademark_service_product', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{name}' => $product['title']]), 'trademark_service_product', $product->id);
