...
class HostModel extends Model
{
    ...
    public function hostList($param)
    {
        ...
        if($app=='home'){
            ...
            $adminField = $AdminFieldModel->adminFieldIndex(['view'=>'host']);
        ...
        $hosts = $this->alias('h')
            ->field('h.id,h.client_id,c.username client_name,c.email,c.phone_code,c.phone,c.company,h.product_id,p.name product_name,h.name,h.create_time,h.active_time,h.due_time,h.first_payment_amount,h.billing_cycle,h.billing_cycle_name,h.status,o.pay_time,h.renew_amount,h.client_notes,hi.dedicate_ip,hi.ip_num,hi.assign_ip,s.name server_name,h.notes admin_notes,h.base_price,c.status client_status,c.create_time reg_time,c.country,c.address,c.language,c.notes')
            ->leftjoin('product p', 'p.id=h.product_id')
            ->leftjoin('client c', 'c.id=h.client_id')
            ->leftjoin('order o', 'o.id=h.order_id')
            ->leftJoin('server s','s.id=h.server_id')
        ...
        foreach ($hosts as $key => $host) {
            ...
            if(isset($selectField['certification'])){
                // 实名认证字段
                $hosts[$key]['certification'] = isset($certificationHookResult[$host['client_id']]) && $certificationHookResult[$host['client_id']]?true:false;
    ...
    public function syncAccount($id)
    {
        ...
        if($res['status'] == 200){
            ...
            upstream_sync_host($id, 'update_host');
