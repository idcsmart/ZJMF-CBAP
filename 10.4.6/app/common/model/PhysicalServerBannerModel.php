...
/**
 * @title 模板控制器-物理服务器轮播图模型
 * @desc 模板控制器-物理服务器轮播图模型
 * @use app\common\model\PhysicalServerBannerModel
 */
class PhysicalServerBannerModel extends Model
{
    protected $name = 'physical_server_banner';
    ...
    public function createBanner($param)
    {
        ...
        try{
            ...
            # 记录日志
            active_log(lang('log_add_physical_server_banner', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{id}' => $banner['id']]), 'physical_server_banner', $banner->id);
    ...
    public function updateBanner($param)
    {
        ...
        if(empty($banner)){
            return ['status'=>400, 'msg'=>lang('physical_server_banner_not_exist')];
        }
        ...
        try{
            ...
            $desc = [
                'img'           => lang('physical_server_banner_img'),
                'url'           => lang('physical_server_banner_url'),
                'start_time'    => lang('physical_server_banner_start_time'),
                'end_time'      => lang('physical_server_banner_end_time'),
                'show'          => lang('physical_server_banner_show'),
                'notes'         => lang('physical_server_banner_notes'),
            ];
            ...
            if(!empty($description)){
                $description = lang('log_update_physical_server_banner', [
                    '{admin}' => 'admin#'.get_admin_id().'#'.request()->admin_name.'#',
                    '{id}'   => $banner['id'],
                    '{detail}' => implode(',', $description),
                ]);
                active_log($description, 'physical_server_banner', $banner->id);
            }
    ...
    public function deleteBanner($id)
    {
        ...
        try{
            # 记录日志
            active_log(lang('log_delete_physical_server_banner', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{id}' => $banner['id']]), 'physical_server_banner', $banner->id);
    ...
    public function showBanner($param)
    {
        ...
        try{
            ...
            # 记录日志
            if($param['show']==1){
                active_log(lang('log_show_physical_server_banner', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{id}' => $banner['id']]), 'physical_server_banner', $banner->id);
            }else{
                active_log(lang('log_hide_physical_server_banner', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{id}' => $banner['id']]), 'physical_server_banner', $banner->id);
            }
    ...
    public function orderBanner($param)
    {
        ...
        try{
            ...
            # 记录日志
            active_log(lang('log_order_physical_server_banner', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#']));
