...
class SmsLogic
{
    ...
    public function send($param)
    {
		...
		if($param['name']=='host_pending'){
			...
			if($index_product['creating_notice_sms'] && $index_product['creating_notice_sms_api']>0 && $index_product['creating_notice_sms_api_template']>0){
		...
		if($param['name']=='host_active'){
			...
			if($index_product['created_notice_sms'] && $index_product['created_notice_sms_api']>0 && $index_product['created_notice_sms_api_template']>0){
        ...
        if(!empty($param['order_id'])){
			$index_order = Db::name('order')->field('id,type,amount,create_time,status,gateway_name gateway,credit,client_id,is_recycle')->find($param['order_id']);
			if(empty($index_order) || $index_order['is_recycle']){
