...
<div id="content" class="help download" v-cloak>
  <t-card class="list-card-container left-card">
    <div class="help_card">
      ...
      <div class="help_table">
        <t-table hover row-key='id' :pagination="pagination" :data="list" @Change="changepages" @select-change="rehandleSelectChange" :columns="columns" :loading="loading" :selected-row-keys="selectedRowKeys">
          ...
          <template #filesize="slotProps">
            <div> {{slotProps.row.filesize / 1024 / 1024 >= 1 ? (slotProps.row.filesize / 1024 / 1024).toFixed(2) + "M" : (slotProps.row.filesize / 1024).toFixed(2) + "KB"}}</div>
          </template>
  ...
  <t-dialog :header="lang.attachment" placement="center" :visible.sync="visible" @Cancel="onCancel" :footer="false" @EscKeydown="onKeydownEsc" @CloseBtnClick="onClickCloseBtn" @Close="close" width="70%" @Confirm="uploadConfirm" @progress="uploadProgress">
