...
class HostLinkModel extends Model{
    ...
    public function createAccount($param){
        ...
        }else if($configData['line']['bill_type'] == 'bw' && is_numeric($configData['line']['bw_ip_group'])){
            $post['ip_group'] = $configData['line']['bw_ip_group'];
        }
    ...
    public function changePackage($param){
        ...
        if($custom['type'] == 'buy_image'){
            ...
            if(isset($newConfigData['ip'])){
                ...
                $post['id'] = $id;
                if(!empty($ipGroup)){
                    $post['ip_num'][ $ipGroup ] = $newConfigData['ip']['value'];
                }else{
                    $post['ip_num'] = $newConfigData['ip']['value'];
                }
            ...
            $post['id'] = $id;
            if(!empty($ipGroup)){
                $post['ip_num'][ $ipGroup ] = $custom['ip_data']['value'];
            }else{
                $post['ip_num'] = $custom['ip_data']['value'];
            }
            $Dcim = new Dcim($param['server']);
            $res = $Dcim->modifyIpNum($post);
            if($res['status'] == 200){
