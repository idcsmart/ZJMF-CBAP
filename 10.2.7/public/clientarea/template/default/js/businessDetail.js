  ...
  window.onload = function () {
    ...
    new Vue({
      ...
      methods: {
        ...
        // 续费
        goShop_client() {
          window.open(`https://my.idcsmart.com/shop_client.htm?activeName=1`)
        },
        // 支付成功回调
        paySuccess(e) {
          this.getAppTable()
        ...
        // 再次购买
        goShop(row) {
          window.open(`https://my.idcsmart.com/shop/shop_detail.htm?id=${row.product_id}&clientId=${row.client_id}`)

        },
        goShopDetail() {
          window.open(`https://my.idcsmart.com/shop/shop_client.htm?activeName=2`)
        },
        // 下载安装包
        async download(id) {
          try {
