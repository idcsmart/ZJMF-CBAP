                        ...
                        <tr>
                          ...
                          <td class="item-total">
                            ...
                            <el-popover placement="top-start" width="200" trigger="hover"
                              v-if="isShowLevel || (isShowPromo && item.isUseDiscountCode) || item.eventDiscount">
                              <div class="show-config-list">
                                <p v-if="isShowLevel">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}} {{
                                  (item.level_discount * (item.hasCalc ? 1: item.qty)) | filterMoney }}</p>
                            ...
                            <p class="original-price"
                              v-if="(item.level_discount * (item.hasCalc ? 1: item.qty)) != 0 || item.code_discount != 0 || item.eventDiscount!= 0">
          ...
          <div class="footer-box">
            ...
            <div class="footer-right">
              <div class="totalprice-box" v-loading="totalPriceLoading">
                <div>
                  {{lang.settlement_tip2}}：<span class="total-price">{{commonData.currency_prefix}}{{(finallyPrice *
                    1).toFixed(2) | filterMoney}}</span>
                  <el-popover placement="top-start" width="200" trigger="hover"
                    v-if="isShowLevel || (isShowPromo && isUseDiscountCode) || isShowCash || totalFullDiscount >0">
                    <div class="show-config-list">
                      <p v-if="isShowLevel">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}} {{
                        totalLevelDiscount | filterMoney }}</p>
                      <p v-if="isShowPromo && isUseDiscountCode">
                        {{lang.shoppingCar_tip_text4}}：{{commonData.currency_prefix}} {{ totalCodelDiscount |
                        filterMoney }}</p>
                      <p v-if="isShowCash && cashObj.code">{{lang.shoppingCar_tip_text5}}：{{commonData.currency_prefix}}
                        {{ cashPrice | filterMoney }}</p>
                      <p v-if="isShowFull && totalFullDiscount > 0">{{lang.goods_text4}}：{{commonData.currency_prefix}}
                        {{ totalFullDiscount | filterMoney }}</p>
                    </div>
                    <i class="el-icon-warning-outline total-icon" slot="reference"></i>
                  </el-popover>
