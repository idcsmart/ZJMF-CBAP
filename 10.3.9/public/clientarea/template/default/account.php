                ...
                <el-tab-pane :label="lang.account_menu1" name="1" v-if="showAccountController">
                    ...
                    <div class="right-name">
                      ...
                      <div class="attestation-status" v-show="attestationStatusInfo.iocnShow" @click="handelAttestation({$addon.id})">
                        ...
                        <template v-if="attestationStatusInfo.certification_company_open === 0">
                          <span class="attestation-text" v-if="attestationStatusInfo.status === 0">{{lang.account_tips20}}<span class="bule-text">{{lang.account_tips21}}<i class="el-icon-arrow-right"></i></span></span>
                          <span class="attestation-text" v-else-if="attestationStatusInfo.status === 10">{{lang.account_tips24}}</span></span>
                        </template>
                        <template v-else>
                          <span class="attestation-text" v-if="attestationStatusInfo.status === 0">{{lang.account_tips20}}<span class="bule-text">{{lang.account_tips21}}<i class="el-icon-arrow-right"></i></span></span>
                          <span class="attestation-text" v-else-if="attestationStatusInfo.status === 10">{{lang.account_tips22}}<span class="bule-text">{{lang.account_tips23}}<i class="el-icon-arrow-right"></i></span></span>
                          <span class="attestation-text" v-else-if="attestationStatusInfo.status === 20 || attestationStatusInfo.status === 30">{{lang.account_tips24}}</span>
                        </template>
                      </div>
                      {/if}
                      {/if}
                  ...
                  <div class="box-main">
                    ...
                    <div class="oauth" v-if="oauth.length > 0">
                      ...
                      <div class="oauth-box">
                        <div class="box-item el-col-7" v-for="item in oauth">
                          <div class="box-item-t">{{item.title}}</div>
                          <div class="box-item-b">
                            <el-input :disabled="true" v-model="item.showStatus">
                              <el-popconfirm v-if="item.link" :title="lang.oauth_text10" @confirm="cancelOauth(item)" slot="suffix">
                                <span slot="reference" v-if="item.link" class="a-text bule-text">{{lang.oauth_text9}}
                                </span>
                              </el-popconfirm>
                              <span slot="suffix" class="a-text bule-text" @click="bingOauth(item)" v-else>{{lang.oauth_text8}}</span>
                            </el-input>
                          </div>
                        </div>
                      </div>
                ...
                <el-tab-pane :label="lang.subaccount_text56" name="3" v-if="havePlugin('ClientCare')">
                  <div class="searchbar msg-search">
                    ...
                    <div class="msgsearch-right">
                      ...
                      <el-input v-model="msgParams.keywords" style="width: 3.2rem;" :placeholder="lang.cloud_tip_2" clearable>
                      </el-input>
                  ...
                  <div class="content_table">
                    <div class="tabledata">
                      <el-table v-loading="msgLoading" @selection-change="handleSelectionChange" :data="msgDataList" style="width: 100%;margin-bottom: .2rem;">
                        ...
                        <el-table-column :label="lang.subaccount_text58">
                          <template slot-scope="scope">
                            <span @click="goMsgDetail(scope.row.id)">
                              <span class="msg-status" :class="scope.row.read === 1 ? 'is-read' : 'no-read'">【{{scope.row.read === 1 ?
