...
<div id="content" class="order-details hasCrumb" v-cloak>
  ...
  <t-card class="list-card-container">
    ...
    <div class="top-info">
      <div class="left-box">
        ...
        <div class="item">
          ...
          <span class="btn" @click="changeCredit('add')"
            v-if="(orderDetail.amount * 1 !== orderDetail.credit * 1) && orderDetail.apply_credit_amount * 1 > 0">{{lang.app}}{{lang.credit}}</span>
          <span class="btn" @click="changeCredit('sub')"
            v-if="orderDetail.credit * 1 !== 0">{{lang.deduct}}{{lang.credit}}</span>
        </div>
        <div class="item">
    ...
    <!-- 底部描述 -->
    <t-table row-key="id" :data="orderDetail.items" size="medium" :columns="columns" :hover="hover" :loading="loading"
      :table-layout="tableLayout ? 'auto' : 'fixed'" :hide-sort-tips="true">
      ...
      <template #op="{row, rowIndex}">
        <template v-if="orderDetail.status === 'Unpaid'">
          ...
          <t-tooltip :content="lang.add" :show-arrow="false" theme="light"
            v-if="rowIndex === orderDetail.items.length -1">
