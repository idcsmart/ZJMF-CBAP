...
<div id="content" class="host hasCrumb" v-cloak>
  ...
  <t-card class="list-card-container">
    ...
    <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" :hide-sort-tips="true" @sort-change="sortChange" @select-change="rehandleSelectChange" :selected-row-keys="checkId">
      ...
      <template #name="{row}">
        {{row.name}}
        <span v-show="row.ip_num > 1" class="showIp" @click="showIp(row.id)">({{row.ip_num}})</span>
      </template>
  ...
  <!-- 批量续费弹窗 -->
  <t-dialog :header="lang.batch_renew" :visible.sync="renewVisible" :footer="false" placement="center" @close="cancelRenew" class="renew-dialog">
  ...
  <!-- 所有IP -->
  <t-dialog :header="lang.finance_search_text23" @close="ipLoading = false" :footer="false"
  :visible.sync="ipLoading" class="ip-dialog">
    <div class="ips">
