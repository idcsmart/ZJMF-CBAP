...
class CloudController{
	 ...
	 * @return  int package[].id - 灵活机型ID
	 * @return  string package[].name - 名称
	 * @return  string package[].description - 描述
	 * @return  string package[].cpu - 处理器
	 * @return  int package[].cpu_num - 处理器数量
	 * @return  string package[].memory - 内存
	 * @return  int package[].mem_num - 内存数量
	 * @return  string package[].disk - 硬盘
	 * @return  int package[].disk_num - 硬盘数量
	 * @return  int package[].bw - 带宽
	 * @return  int package[].ip_num - IP数量
	 */
    ...
    * @return  object list[].self_defined_field - 自定义字段值
	...
	*/
     ...
     * @return  string model_config.disk - 硬盘
     * @return  string model_config.gpu - 显卡
     * @return  int model_config.leave_memory - 当前机型剩余内存
     ...
     * @return  string additional_ip - 附加IP
     * @return  int package.id - 灵活机型ID(0=不是灵活机型)
     * @return  string package.name - 灵活机型名称
     * @return  string package.cpu - 处理器
     * @return  string package.memory - 内存
     * @return  string package.disk - 硬盘
     * @return  object optional_memory - 当前机器添加内存({"5":1},5是ID,1是数量)
     * @return  object optional_disk - 当前机器添加硬盘({"5":1},5是ID,1是数量)
     * @return  object optional_gpu - 当前机器添加显卡({"5":1},5是ID,1是数量)
	 */
 ...
 //        return json($result);
	 ...
	 * @title 灵活机型详情
	 * @desc 灵活机型详情
	 * @url /console/v1/product/:id/mf_dcim/package
	 * @method  GET
	 * @author hh
	 * @version v1
	 * @param   int id - 商品ID require
	...
	public function packageIndex(){
		$param = request()->param();
		$param['package_id'] = $param['package_id'] ?? 0;

		$PackageModel = new PackageModel();

		$data = $PackageModel->homePackageIndex($param['package_id']);
