...
class IdcsmartRenewModel extends Model
{
    ...
    public function cyclesFilter(HostModel $host,$cycles,$promoCode='')
    {
        ...
        foreach ($cycles as $k2=>$item2){
            ...
            # 基础价格
            $cycles[$k2]['base_price'] = bcsub($item2['base_price']??0,0,2);
            # 产品对应周期(只能用时间比较)
    ...
    public function renew($param)
    {
        ...
        try{
            ...
            $orderItems[] = [
                ...
                'amount' => $renewAmount,
                //'amount' => $amount,
                'description' => lang_plugins('host_renew_description',['{product_name}'=>$product['name'],'{name}'=>$host['name'],'{billing_cycle_name}'=>$billingCycle,'{time}'=>$beginTime. '-' . $endTime]),
            ];
    ...
    {
        $OrderModel = new OrderModel();
        if(method_exists($OrderModel, 'deleteHostUnpaidUpgradeOrder')){
    ...
    public function deleteUnpaidRenewOrder($id){
        $OrderModel = new OrderModel();
        if(method_exists($OrderModel, 'deleteUnpaidRenewOrder')){
            return $OrderModel->deleteUnpaidRenewOrder($id);
        }else{
