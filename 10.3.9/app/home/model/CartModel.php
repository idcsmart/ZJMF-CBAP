    ...
    public function indexCart()
    {
        ...
        if(!empty(self::$cartData)){
            ...
            foreach ($cartData as $key => $value) {
                ...
                $cartData[$key]['self_defined_field'] = $value['self_defined_field'] ?? (object)[];
            }
        }
    ...
    public function createCart($param)
    {
        ...
        if(isset($param['products']) && !empty($param['products'])){
            foreach ($param['products'] as $key => $value) {
                ...
                if($upstreamProduct){
                    $value['config_options']['customfield'] = $SelfDefinedFieldModel->toUpstreamId([
                        'product_id'          => $product['id'],
                        'self_defined_field'  => $value['self_defined_field'],
                    ]);
            ...
            if($upstreamProduct){
                $param['config_options']['customfield'] = $SelfDefinedFieldModel->toUpstreamId([
                    'product_id'          => $product['id'],
                    'self_defined_field'  => $param['self_defined_field'],
                ]);
