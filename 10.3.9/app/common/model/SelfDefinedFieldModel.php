...
class SelfDefinedFieldModel extends Model
{
    ...
    public function selfDefinedFieldCreate($param)
    {
        ...
        $selfDefinedField = $this->create($param, ['type','relid','field_name','is_required','field_type','description','regexpr','field_option','show_order_page','show_order_detail','show_client_host_detail','show_admin_host_detail','show_client_host_list','create_time']);
    ...
    public function selfDefinedFieldUpdate($param)
    {
        ...
        $this->update($param, ['id'=>$selfDefinedField['id'] ], ['field_name','is_required','field_type','description','regexpr','field_option','show_order_page','show_order_detail','show_client_host_detail','show_admin_host_detail','show_client_host_list','update_time']);
        ...
        foreach($desc as $k=>$v){
            if(isset($param[$k]) && $selfDefinedField[$k] != $param[$k]){
                ...
                if(in_array($k, ['is_required','show_order_page','show_order_detail','show_client_host_detail','show_admin_host_detail'])){
                    $old = $tickbox[ $old ];
                    $new = $tickbox[ $new ];
    ...
    public function selfDefinedFieldList($param)
    {
        ...
        $list = $this
                ->field('id,field_name,field_type,description,regexpr,field_option,is_required,show_order_page,show_order_detail,show_client_host_detail,show_admin_host_detail,show_client_host_list,upstream_id')
                ->where($where)
                ->order('order,id', 'asc')
                ->select()
                ->toArray();
    ...
    {
        if(isset($param['need_upstream_id'])){
     ...
     * @param   array param.self_defined_field - 自定义字段值 require
     */
     ...
     * @author hh
     * @version v1
     * @param   int order_id - 订单ID require
     */
     ...
     * @version v1
     * @param   int param.product_id - 商品ID require
     * @param   string param.type - 上游类型 require
     ...
     * @return  array self_defined_field_value - 
     */
