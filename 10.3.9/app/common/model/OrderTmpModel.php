...
class OrderTmpModel extends Model
{
    ...
    public function recharge($param)
    {
        ...
        if (check_certification_recharge() && !check_certification(get_client_id())){
        ...
        # 后台直接标记支付
        if ($this->isAdmin){
            $OrderModel->orderPaid(['id'=>$orderId, 'transaction_number' => $param['transaction_number'] ?? '']);
        }
    ...
    public function orderPayHandle($param)
    {
        ...
        if (empty($tmpOrder)){
            active_log(lang('log_pay_tmp_order_miss',['{tem_order_id}'=>$tmpOrderId]),'order_tmp',$tmpOrderId);
            return false;
        }
