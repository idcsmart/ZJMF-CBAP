...
add_hook('after_product_delete', function($param){
	...
	try{
		...
		$packageId = PackageModel::where('product_id', $param['id'])->column('id');
		...
		if(!empty($imageId)){
			HostImageLinkModel::where('image_id', $imageId)->delete();
		}
        ...
        if(!empty($packageId)){
            PackageOptionLinkModel::where('package_id', $packageId)->delete();
        }
...
add_hook('after_product_copy', function($param){
	try{
		...
		if(!empty($duration)){
			...
			$package = $PackageModel->where('product_id', $param['product_id'])->select()->toArray();
			$packageIdArr = [];
			foreach ($package as $key => $value) {
				...
				unset($value['id']);
				$value['product_id'] = $param['id'];
				$value['cpu_option_id'] = $optionIdArr[$value['cpu_option_id']] ?? 0;
				$value['mem_option_id'] = $optionIdArr[$value['mem_option_id']] ?? 0;
				$value['disk_option_id'] = $optionIdArr[$value['disk_option_id']] ?? 0;
				$r = $PackageModel->create($value);
				$packageIdArr[$id] = $r->id;
			}

			if(!empty($packageIdArr)){
				$PackageOptionLinkModel = new PackageOptionLinkModel();
				$packageOptionLink = $PackageOptionLinkModel->whereIn('package_id', array_keys($packageIdArr))->select()->toArray();
				foreach ($packageOptionLink as $key => $value) {
                    $value['package_id'] = $packageIdArr[$value['package_id']] ?? 0;
					$value['option_id'] = $optionIdArr[$value['option_id']] ?? 0;
					$PackageOptionLinkModel->create($value);
				}
			}
            ...
            foreach ($price as $key => $value) {
                ...
                if($value['rel_type']=='option'){
                    ...
                    $value['rel_id'] = $packageIdArr[$value['rel_id']] ?? 0;
                }
