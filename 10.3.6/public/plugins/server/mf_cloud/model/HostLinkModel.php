...
class HostLinkModel extends Model{
            ...
            if(!empty($recommendConfigPrice)){
                ...
                foreach($duration as $v){
                    ...
                    foreach($optionPriceArr as $relType=>$optionDurationPrice){
                        ...
                        }else if($relType == OptionModel::LINE_BW || $relType == OptionModel::LINE_FLOW){
                            if(isset($minLinePrice) && $minLinePrice == 0){
                                continue;
                            }
                            foreach($optionDurationPrice as $durationPrice){
                                if(is_null($minLinePrice)){
                                    $minLinePrice = $durationPrice[ $v['id'] ] ?? 0;
                                }else{
                                    $minLinePrice = min($minLinePrice, $durationPrice[ $v['id'] ] ?? 0);
                                }
                                if($minLinePrice == 0){
                                    break;
                                }
    ...
    public function hostTransfer($param){
        ...
        for($i = 0; $i<36; $i++){
            ...
            if($res['status'] == 200 && !in_array($res['data']['status'], [0,1])){
