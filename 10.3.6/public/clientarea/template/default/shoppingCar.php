                ...
                <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
                  <div v-for="(item,index) in showList" :key="index" class="shopping-goods" v-loading="item.isLoading">
                    <div class="table-name">
                      ...
                      <el-link type="primary" class="edit-goods" v-if="item.info" @click="goGoods(item)">{{lang.shoppingCar_editGoods}}</el-link>
                    </div>
                      ...
                      <table class="goods-table">
                        ...
                        <tbody>
                          <tr v-if="item.info">
                            ...
                            <td>
                              <el-input-number v-model="item.qty" size="small" @change="(currentValue,oldValue)=>handleChange(currentValue,oldValue,item,index)" :min="1" :max="item.stock_control === 1 ? item.stock_qty : 9999"></el-input-number>
                            ...
                            <td class="item-total" v-loading="item.priceLoading">
                              <span>{{commonData.currency_prefix}} {{((item.price * item.qty) - item.code_discount - item.level_discount * (item.hasCalc ? 1: item.qty)) > 0 ?
                                 (((item.price * item.qty) - item.code_discount - item.level_discount * (item.hasCalc ? 1: item.qty))).toFixed(2) : 0 }}</span>
                              <el-popover placement="top-start" width="180" trigger="hover" v-if="isShowLevel || (isShowPromo && item.isUseDiscountCode)">
                                <div class="show-config-list">
                                  <p v-if="isShowLevel">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}} {{ (item.level_discount * (item.hasCalc ? 1: item.qty)).toFixed(2) | filterMoney }}</p>
                              ...
                              <p class="original-price" v-if="item.code_discount != 0 || (item.level_discount * (item.hasCalc ? 1: item.qty)) != 0 ">{{commonData.currency_prefix}} {{(item.price * item.qty).toFixed(2)}}</p>
                              <div class="discount-box" v-show="item.customfield && !item.customfield.promo_code && isShowPromo">
                                <discount-code @get-discount="getDiscount(arguments)" scene='new' :product_id='item.product_id' :qty="item.qty" :amount="item.price" :billing_cycle_time="item.info.duration" :shopping_index="item.position">
                                  <discount-code>
                              </div>
                          ...
                          <tr v-else>
                            ...
                            <td class="no-goods-td">
                              ...
                              <el-button class="buy-again-btn" @click="goGoods(item)">{{lang.shoppingCar_buy_again}}</el-button>
                            </td>
                            <td></td>
                            <td></td>
                            <td class="delete-btn" @click="handelDeleteGoods(item,index)">
        ...
        <el-footer>
          <div class="footer-box">
            <div class="footer-left">
              <el-checkbox class="all-check" v-model="checkAll" @change="handleCheckAllChange">{{lang.shoppingCar_select_all}}</el-checkbox>
              <span class="delect-btn">
                <el-link type="danger" class="delect-goods" @click="deleteCheckGoods">{{lang.shoppingCar_delete_select}}</el-link>
              </span>
              <span>{{lang.shoppingCar_selected}}<span class="text-red">{{checkedCities.length}}</span>{{lang.shoppingCar_goods_text}}</span>
            </div>
            <div class="footer-right">
              <p>{{lang.shoppingCar_tip_text3}}：<span class="total-price">{{commonData.currency_prefix}}{{Number(totalPrice).toFixed(2)}}</span></p>
