      ...
      data() {
        return {
          ...
          rules: {
            ...
            product_name: [{ required: true, message: lang.select + lang.product, type: 'error' }],
            newProductName: [{ required: true, message: lang.select + lang.product, type: 'error' }],
      ...
      methods: {
        handelIframeMsg(e) {
          ...
          if (type === 'iframeBuy') {
            ...
            const subPrice = this.newBuyData.isEditPrice ? this.newBuyData.price : price * 1
            const renewPrice = this.newBuyData.custom_renew_amount_switch ? this.newBuyData.custom_renew_amount : 0
            const renewSwitch = this.newBuyData.custom_renew_amount_switch ? 1 : 0
            const subObj = { ...params, custom_order_amount: subPrice, client_id, custom_renew_amount: renewPrice, custom_renew_amount_switch: renewSwitch }
            settleOrder(subObj).then((res) => {
        ...
        selectRenew(index) {
          ...
          this.renewsParms.customfield.billing_cycle = this.renewHostList[index].billing_cycle
          this.renewsParms.customfield.custom_amount = this.renewHostList[index].price * 1
        },
        // 获取用户已开通的产品
            ...
            try {
              ...
              switch (this.formData.type) {
                ...
                case 'renew':
                  ...
                  createOrder(this.renewsParms).then((res) => {
                    this.$message.success(res.data.msg)
                    location.href = `order_details.htm?id=${res.data.data.id}`
                  }).catch((err) => {
                    this.$message.error(err.data.msg)
