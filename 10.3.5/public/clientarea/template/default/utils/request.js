...
// 这里是不需要jwt的页面 例如登录页
const noNeedJwtUrlArr = ['login.htm', 'userStatus.htm', 'goodsList.htm', 'source.htm', 'news_detail.htm', 'helpTotal.htm', 'goods.htm', 'goods_iframe.htm', 'agreement.htm', 'regist.htm', 'forget.htm']
const nowUrl = location.href.split('/').pop()
const pageRouter = nowUrl.indexOf('?') !== -1 ? nowUrl.split('?')[0] : nowUrl
if (!noNeedJwtUrlArr.includes(pageRouter) && !localStorage.getItem('jwt')) {
