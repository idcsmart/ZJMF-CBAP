      ...
      <el-container>
                ...
                <el-tabs v-model="activeIndex" @tab-click="handleClick">
                  <el-tab-pane :label="lang.finance_tab1" name="1" v-if="isShowOrderController">
                    <div class="content_table">
                      <div class="content_searchbar">
                        <div class="left_tips">
                          <el-button class="all-pay" @click="handelAllPay" :loading="allLoading" v-if="isShowCombine">{{lang.finance_btn10}}</el-button>
                        ...
                        <el-table v-loading="loading1" @selection-change="handleSelectionChange" :data="dataList1" style="width: 100%;margin-bottom: 20px;" :row-key="getRowKey" lazy :load="load" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
                          ...
                          <el-table-column prop="product_names" :label="lang.finance_label1" min-width="300" :show-overflow-tooltip="true">
                            <template slot-scope="scope">
                              ...
                              <span v-if="scope.row.product_names" class="a-text" @click="goOrderDetail(scope.row.id)">{{scope.row.product_name}}</span>
                    ...
                    <div class="content_table">
                      <div class="content_searchbar balance-searchbar">
                        ...
                        <div class="box" style="display:flex; overflow-y: auto;">
                          <el-date-picker @change="inputChange3" v-model="date" type="daterange" :range-separator=lang.finance_text18 style="width:3.5rem;margin-right:0.14rem" :start-placeholder="lang.finance_text19" value-format="timestamp" align="center" :end-placeholder="lang.finance_text20">
                          </el-date-picker>
                          <el-select v-model="params3.type" :placeholder="lang.finance_text21" style="width:2rem" @change="inputChange3">
                  ...
                  <el-tab-pane :label="lang.finance_text23" name="5" v-if="isShowContract">
                    <div class="content_table">
                      <div class="content_searchbar balance-searchbar">
                        <div class="left_tips">
                          <el-button @click="handeInfo" style="color: #fff; background-color: rgba(0, 88, 255, 1);">{{lang.finance_text24}}</el-button>
                          <el-button @click="handelApplyOrder" style="color: rgba(0, 88, 255, 1);  border: 0.01rem solid #0058FF;">{{lang.finance_text25}}</el-button>
                        </div>
                        <div class="box" style="display:flex; overflow-y: auto;">
                          <el-input v-model="params4.keywords" style="width: 3.2rem;margin-left: .2rem;" :placeholder="lang.finance_text26" @keyup.enter.native="inputChange4" clearable @clear="getContractList">
                          </el-input>
                          <el-button style="margin-left: 0.1rem; color: #fff; background-color: rgba(0, 88, 255, 1);" @Click="inputChange4">{{lang.finance_text27}}</el-button>
                        </div>
                      </div>
                      <div class="tabledata">
                          ...
                          <el-table-column :label="lang.finance_text31" width="200" align="left">
                            <template slot-scope="scope">
                              <div style="display: flex; align-items: center;">
                                ...
                                <img v-if="scope.row.status === 'complete' && scope.row.post_number" class="help-icon" @click="handelRec(scope.row)" style="width: 0.18rem;height: 0.18rem;" src="/{$template_catalog}/template/{$themes}/img/finance/icon_1.png" :alt="lang.finance_text32">
                              </div>
                            </template>
                          </el-table-column>
                          <el-table-column :label="lang.finance_label6" width="100" align="left">
                            <template slot-scope="scope">
                              <el-popover placement="top-start" trigger="hover">
                                <i slot="reference" class="el-icon-more"></i>
                                <div class="operation-box">
                                  <div class="operation-item" @click="handelSign(scope.row.order_id)" v-if="scope.row.status === 'no_sign'">{{lang.finance_text33}}</div>
                                  <div class="operation-item" @click="handelDetail(scope.row.id)" v-if="scope.row.status === 'review'">{{lang.finance_text34}}</div>
                                  <div @click="handelCancel(scope.row.id)" class="operation-item" v-if="scope.row.status === 'review'">{{lang.finance_text35}}</div>
                                  <div class="operation-item" @click="handelPreview(scope.row.id)" v-if="scope.row.status === 'complete' || scope.row.status === 'wait_mail'">{{lang.invoice_text41}}</div>
                                  <div @click="handelDownload(scope.row.id)" class="operation-item" v-if="scope.row.status === 'complete' || scope.row.status === 'wait_mail'">{{lang.finance_text36}}</div>
                                  <div @click="handelMail(scope.row.id)" class="operation-item" v-if="scope.row.status === 'complete' && !scope.row.post_number">{{lang.finance_text37}}</div>
                  ...
                  <el-tab-pane :label="lang.finance_text38" name="6" v-if="isShowCredit && creditData?.credit_limit*1 > 0">
                        ...
                        <div class="credit-item">
                          <div class="item-top" style="align-items: start;">
                            ...
                            <div class="item-r" style="text-align: right;">
                              <div>{{lang.finance_text43}}：{{commonData.currency_prefix}}{{creditData.used | formatNumber}}</div>
                              <div v-if="creditData.account?.repayment_time">{{lang.finance_text44}}：{{creditData.account?.repayment_time | formateTime3}}</div>
                            </div>
                          </div>
                          <div class="item-bottom flex-bottom">
                            <div class="item-bl">{{commonData.currency_prefix}}{{creditData.account?.amount | formatNumber}}</div>
                            <el-button class="no-btn" v-if="creditData.account?.status === 'Outstanding'">{{lang.finance_text45}}</el-button>
                            <el-button class="credit-btn" v-if="creditData.account?.status !== 'Repaid' &&  creditData.account?.status !== 'Outstanding'" @click="handelPayCredit(creditData.account?.order_id)">{{lang.finance_text46}}</el-button>
                          </div>
                        </div>
                      </div>
                      <div class="tabledata">
                          ...
                          <el-table-column :label="lang.finance_label6" width="100" align="left">
                            <template slot-scope="scope">
                              <el-popover placement="top-start" trigger="hover">
                                <i slot="reference" class="el-icon-more"></i>
                                <div class="operation-box">
                                  <div class="operation-item" @click="handelCredit(scope.row.id)">{{lang.finance_text49}}</div>
                                  <div class="operation-item" @click="handelPayCredit(scope.row?.order_id)" v-if="scope.row.status === 'Disbursed' || scope.row.status === 'Overdue'">{{lang.finance_text50}}</div>
                                </div>
                              </el-popover>
                            </template>
                          </el-table-column>
        ...
        <el-dialog width="6.8rem" :visible.sync="isShowKd" :show-close="false" @close="kdClose" class="kd-dialog">
          ...
          <div class="dialog-box">
            <div class="kd-item"><span class="kd-label">{{lang.finance_text53}}:</span><span class="kd-value">{{recData.courier_company}}</span></div>
            <div class="kd-item"><span class="kd-label">{{lang.finance_text54}}:</span><span class="kd-value">{{recData.post_number}}</span></div>
            <div class="kd-item"><span class="kd-label">{{lang.finance_text55}}:</span><span class="kd-value">{{recData.rec_address}}</span></div>
            <div class="kd-item"><span class="kd-label">{{lang.finance_text56}}:</span><span class="kd-value">{{recData.rec_phone}}</span></div>
            <div class="kd-item"><span class="kd-label">{{lang.finance_text57}}:</span><span class="kd-value">{{recData.rec_person}}</span></div>
          </div>
