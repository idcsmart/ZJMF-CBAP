...
new Vue({
  data () {
    return {
      ...
      id: '',
      tabs: 'duration', // duration,model,data_center,limit,system,other
      hover: true,
      tableLayout: false,
      delVisible: false,
      loading: false,
      ...
      dataRules: {
        ...
        host_prefix: [
          ...
          {
            pattern: /^[A-Z][a-zA-Z0-9_.]{0,9}$/, message: lang.verify8 + '1-10', type: 'warning'
          },
        ],
        host_length: [
        ...
        model_config_id: [
          { required: true, message: `${lang.select}${lang.box_title46}`, type: 'error' },
        ]
      },
      backupColumns: [ // 备份表格
        {
      ...
      memoryLoading: false,
      memoryType: '', // 内存方式
      modelColumns: [ // model表格 order_text68
        {
          colKey: 'id',
      ...
      calcRules: { // 模型配置验证
        name: [
          { required: true, message: `${lang.input}${lang.config_name}`, type: 'error' },
        ],
        group_id: [
          { required: true, message: `${lang.input}${lang.sale_group}ID`, type: 'error' },
          {
            pattern: /^[0-9]*$/, message: lang.input + lang.verify7, type: 'warning'
          },
        ],
        cpu: [
          { required: true, message: `${lang.input}${lang.mf_cpu}`, type: 'error' },
        ],
        cpu_param: [
          { required: true, message: `${lang.input}${lang.mf_cpu_param}`, type: 'error' },
        ],
        memory: [
          { required: true, message: `${lang.input}${lang.memory}`, type: 'error' },
        ],
    ...
    calcReg () { // 动态生成规则
      return (name, min, max, tag) => {
        ...
        return [
          ...
          {
            pattern: pattern, message: tag === 'ip' ? '' : lang.input + `${min}-${max}` + lang.verify18, type: 'warning'
          },
          {
            validator: val => pass(val), message: tag === 'ip' ? '' : lang.input + `${min}-${max}` + lang.verify18, type: 'warning'
          }]
      }
    },
    calcIpNum () {
  ...
  methods: {
    async autoFill(name, data) {
      try {
        const price = JSON.parse(JSON.stringify(data)).reduce(
          (all, cur) => {
            if (cur.price) {
    ...
    changeLine (e) {
      this.limitMemoryType = this.calcSelectLine.filter(item => item.id === e)[0]?.bill_type
    },
    ...
    addLineSub (type) {
      ...
      if (this.$refs['bw-item']) {
        this.bwValidator = this.$refs['bw-item'].errorClasses === 't-is-success'
      } else if (this.$refs['ip-item']) {
        this.bwValidator = this.$refs['ip-item'].errorClasses === 't-is-success'
      } else {
        this.bwValidator = true
      }
      this.optType = 'add'
          ...
          if (this.lineType === 'add') {
            ...
            switch (this.subType) {
              ...
              case 'line_ip':
                ...
                // this.lineForm.ip_data = this.lineForm.ip_data.map(item => {
                //   item.value = item.value.split(',').reduce((all,cur) => {
                //     all += cur.split('_')[0] * 1
                //     return all
                //   }, 0)
                //   return item
    ...
    async submitLine ({ validateResult, firstError }, bol = true) {
      if (validateResult === true) {
        try {
          ...
          //   }
          // }
          // if (params.ip_data.length === 0) {
          //   return this.$message.warning(lang.mf_tip24)
          // }
