  ...
  methods: {
    ...
    getManualOs() {
      manualResourceOs(this.id).then((res) => {
        if (res.data.status === 200) {
          this.osData = res.data.data.os.map((item) => {
            return {
              icon: item.name,
          ...
          this.osIcon = "/plugins/server/mf_dcim/template/clientarea/img/mf_dcim/" + this.osData[0].name + '.svg'
    ...
    osSelectGroupChange(e) {
      this.osData.map(item => {
        if (item.id == e) {
          ...
          this.osIcon = "/plugins/server/mf_dcim/template/clientarea/img/mf_dcim/" + item.name + '.svg'
