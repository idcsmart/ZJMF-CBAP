...
class HostLinkModel extends Model{
    ...
    public function getPriceCycle($productId){
        ...
        if($ProductModel['pay_type'] == 'free'){
            ...
            $option = OptionModel::alias('o')
                ...
                ->leftJoin('module_mf_dcim_duration d', 'p.duration_id=d.id')
            ...
            if(!empty($modelConfig)){
                $price = PriceModel::field('duration_id,min(price) price')->where('rel_type', 'model_config')->whereIn('rel_id', $modelConfig)->group('duration_id')->select()->toArray();
    ...
    public function adminField($param){
        ...
        if(isset($configData['bw'])){
            ...
            $data[2]['field'][] = [
                'name'      => lang_plugins('mf_dcim_out_server_bw'),
                'key'       => 'out_bw',
                'value'     => $configData['flow']['other_config']['out_bw'] ?? '',
            ];
            $data[2]['field'][] = [
                'name'      => lang_plugins('mf_dcim_in_server_bw'),
                'key'       => 'in_bw',
        ...
        if($config['data']['manual_resource']==1 && $this->isEnableManualResource()){
            ...
            $data[2]['field'][] = [
                'name'  => lang_plugins('mf_dcim_attach_ip_num'),
                'key'  => 'ip_num',
    ...
    public function hostUpdate($param){
        ...
        if(!empty($hostLink)){
            ...
            if($config['data']['manual_resource']==1 && $this->isEnableManualResource()){
                ...
                $configData['model_config']['name'] = $moduleAdminField['model_config_name'];
                $configData['model_config']['cpu'] = $moduleAdminField['model_config_cpu'];
                $configData['model_config']['cpu_param'] = $moduleAdminField['model_config_cpu_param'];
                $configData['model_config']['memory'] = $moduleAdminField['model_config_memory'];
                $configData['model_config']['disk'] = $moduleAdminField['model_config_disk'];
            ...
            if($ip_change){
                ...
                if($res['status'] == 200){
                    ...
                    if($detailRes['status'] == 200){
                        if(!$input_ip){
                            $this->where('host_id', $hostId)->update(['ip'=>$detailRes['server']['zhuip'] ?? '' ]);
