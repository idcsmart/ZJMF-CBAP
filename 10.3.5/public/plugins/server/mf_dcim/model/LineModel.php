...
class LineModel extends Model{
    ...
    public function lineCreate($param){
        ...
        $this->startTrans();
        try{
            $line = $this->create($param, ['data_center_id','name','bill_type','bw_ip_group','defence_enable','defence_ip_group','create_time','order']);
    ...
    public function lineIndex($id){
        $line = $this
                ->field('id,name,bill_type,bw_ip_group,defence_enable,defence_ip_group,order')
                ->find($id);
        if(empty($line)){
            return (object)[];
        }
    ...
    public function lineUpdate($param){
        ...
        $this->update($param, ['id'=>$line['id']], ['name','bw_ip_group','defence_enable','defence_ip_group','order']);
    ...
    public function homeLineConfig($id){
        ...
        if($line['bill_type'] == 'bw'){
            // 带宽计费
            $bw = OptionModel::field('type,value,min_value,max_value,step')->where('rel_type', OptionModel::LINE_BW)->where('rel_id', $id)->orderRaw('--value,--min_value asc')->select()->toArray();
        ...
        if($line['defence_enable'] == 1){
            $data['defence'] = OptionModel::field('value')->where('rel_type', OptionModel::LINE_DEFENCE)->where('rel_id', $id)->orderRaw('--value asc')->select()->toArray();
        }
        $ip = OptionModel::field('value')->where('rel_type', OptionModel::LINE_IP)->where('rel_id', $id)->select()->toArray();
        // 排序
        foreach($ip as $k=>$v){
            $num = $v['value'];
            ...
            $desc = $num == 'NC' ? lang_plugins('mf_dcim_real_ip') : $num .lang_plugins('mf_dcim_indivual');
            $ip[$k]['desc'] = $desc;
        }
