...
class IdcsmartCommonProductModel extends Model
{
    ...
    public function indexProduct($param)
    {
        ...
        $commonProduct = $this->field('product_id,order_page_description,allow_qty,auto_support,type,rel_id server_id')
    ...
    public function hostConfigotpion($param)
    {
        ...
        $host = $HostModel->alias('h')
            ->field('h.id,h.order_id,h.create_time,h.due_time,h.billing_cycle,h.billing_cycle_name,h.billing_cycle_time,h.renew_amount,h.first_payment_amount,p.name,h.status,h.name as host_name,h.product_id,h.client_notes')
