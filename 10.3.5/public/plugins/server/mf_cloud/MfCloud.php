...
class MfCloud{
	...
	public function metaData(){
		return ['display_name'=>'魔方云(自定义配置)', 'version'=>'1.0.3'];
	}
  ...
  `rel_type` tinyint(4) NOT NULL DEFAULT COMMENT '关联表:0=option,1=recommend_config',
  `rel_id` int(11) NOT NULL DEFAULT '0' COMMENT '关联ID',
	        ...
	        if($params['custom']['network_type'] == 'vpc'){
	        	if(isset($params['custom']['vpc'])){
	        		if(isset($params['custom']['vpc']['id']) && $params['custom']['vpc']['id']>0){
	        			...
	        			// if(empty(get_admin_id()) && (empty($vpcNetwork) || $vpcNetwork['client_id'] != get_client_id() || $vpcNetwork['data_center_id'] != $params['custom']['data_center_id'])){
