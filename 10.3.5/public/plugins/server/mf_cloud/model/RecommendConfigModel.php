...
class RecommendConfigModel extends Model{
    ...
    public function recommendConfigCreate($param){
        ...
        $this->startTrans();
        try{
            $recommendConfig = $this->create($param, ['product_id','name','description','order','data_center_id','line_id','cpu','memory','system_disk_size','system_disk_type','data_disk_size','data_disk_type','bw','flow','peak_defence','ip_num','create_time']);
    ...
    public function recommendConfigUpdate($param){
        ...
        $this->startTrans();
        try{
            $this->update($param, ['id'=>$recommendConfig['id']], ['name','description','order','data_center_id','line_id','cpu','memory','system_disk_size','system_disk_type','data_disk_size','data_disk_type','bw','flow','peak_defence','ip_num']);
    ...
    public function recommendConfigIndex($id){
        ...
        $duration = DurationModel::alias('d')
                    ...
                    ->leftJoin('module_mf_cloud_price p', 'p.rel_type='.PriceModel::REL_TYPE_RECOMMEND_CONFIG.' AND p.rel_id='.$id.' AND d.id=p.duration_id')
    ...
    public function getUpgradeRecommendConfig($current_recommend_config = []){
        ...
        if($current_recommend_config['upgrade_range'] == self::UPGRADE_DISABLE){
            ...
            $where = [
                ['product_id', '=', $current_recommend_config['product_id']],
                ['data_center_id', '=', $current_recommend_config['data_center_id']],
                ['id', '<>', $current_recommend_config['id']],
                ['hidden', '=', 0],
            ];
    ...
    public function formatRecommendConfig($recommend_config){
        ...
        $optionSystemDisk = OptionModel::where('product_id', $productId)
                        ->where('rel_type', OptionModel::SYSTEM_DISK)
                        ->where('rel_id', 0)
                        ->whereLike("other_config", '{"disk_type":"'.$recommend_config['system_disk_type'].'"%')
        ...
        if(!empty($optionSystemDisk)){
            $otherConfig = json_decode($optionSystemDisk['other_config'], true);

            $data['system_disk']['store_id'] = $otherConfig['store_id'] ?? 0;
        }
        // 匹配数据盘
        if(isset($recommend_config['data_disk_size']) && isset($recommend_config['data_disk_type'])){
            $optionDataDisk = OptionModel::where('product_id', $productId)
                        ->where('rel_type', OptionModel::DATA_DISK)
                        ->where('rel_id', 0)
                        ->whereLike("other_config", '{"disk_type":"'.$recommend_config['data_disk_type'].'"%')
                        ->where(function($query) use ($recommend_config) {
            ...
            if(!empty($optionDataDisk)){
                $otherConfig = json_decode($optionDataDisk['other_config'], true);

                $data['data_disk']['store_id'] = $otherConfig['store_id'] ?? 0;
            }
        }
        // 匹配CPU
        $optionCpu = OptionModel::where('product_id', $productId)->where('rel_type', OptionModel::CPU)->where('value', $recommend_config['cpu'])->find();
        if(!empty($optionCpu)){
            $otherConfig = json_decode($optionCpu['other_config'], true);

            $data['advanced_cpu'] = $otherConfig['advanced_cpu'] ?? 0;
            $data['cpu_limit'] = $otherConfig['cpu_limit'] ?? 0;
        }
        // 匹配线路
        ...
        if(!empty($line)){
            ...
            if($line['bill_type'] == 'bw'){
                ...
                $optionBw = OptionModel::where('product_id', $productId)
                        ...
                        ->find();
                if(!empty($optionBw)){
                    $otherConfig = json_decode($optionBw['other_config'], true);
                ...
                $optionFlow = OptionModel::where('product_id', $productId)
                            ...
                            ->find();
                if(!empty($optionFlow)){
                    $otherConfig = json_decode($optionFlow['other_config'], true);
