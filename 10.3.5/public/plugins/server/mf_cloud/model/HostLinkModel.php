...
class HostLinkModel extends Model{
    ...
    public function detail($hostId, $useCache = true){
        ...
        if(empty($data)){
            ...
            $data['recommend_config'] = $configData['recommend_config'] ?? (object)[];
            $data['system_disk'] = [
        ...
        if(isset($configData['data_disk'])){
            foreach($configData['data_disk'] as $v){
                ...
                $optionDataDisk = OptionModel::where('product_id', $productId)
                        ...
                        ->where('rel_id', 0)
                        ->whereLike("other_config", '{"disk_type":"'.($v['other_config']['disk_type'] ?? '').'"%')
                ...
                if(!empty($optionDataDisk)){
                    ...
                    $v['other_config']['store_id'] = $otherConfig['store_id'] ?? 0;
                }
                // 获取磁盘限制
                if($config['disk_limit_enable'] == 1){
                    $diskLimit = DiskLimitModel::where('product_id', $param['product']['id'])
            ...
            if($oldRcParam['data_disk']['size'] != $newRcParam['data_disk']['size']){
                if($newRcParam['data_disk']['size'] > $oldRcParam['data_disk']['size']){
                    ...
                    if(!empty($disk)){
                        ...
                        if($deleteRes['status'] == 200){
                            $description[] = lang_plugins('删除数据盘成功,名称:{name},大小:{size}', [
                                '{name}'    => $disk['name'],
                                '{size}'    => $disk['size'],
                            ]);
                            ...
                            $description[] = lang_plugins('删除数据盘失败,原因:{reason}', [
    ...
    public function getPriceCycle($productId){
        ...
        if($ProductModel['pay_type'] == 'free'){
            ...
            // 有套餐优先套餐价格
            $recommendPrice = RecommendConfigModel::alias('rc')
                            ->field('p.price,d.name')
                            ->join('module_mf_cloud_price p', 'rc.id=p.rel_id AND p.rel_type='.PriceModel::REL_TYPE_RECOMMEND_CONFIG)
            ...
            if(!empty($recommendPrice)){
                ...
                $cycle = $recommendPrice['name'];
            }else{
                // 获取cpu最低价格
                $option = OptionModel::alias('o')
                    ->field('o.*,p.duration_id,p.price,d.price_factor')
                    ->join('module_mf_cloud_price p', 'o.id=p.rel_id AND p.rel_type='.PriceModel::REL_TYPE_OPTION)
                    ->leftJoin('module_mf_cloud_duration d', 'p.duration_id=d.id')
