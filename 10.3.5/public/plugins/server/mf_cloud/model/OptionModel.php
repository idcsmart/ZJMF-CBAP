...
class OptionModel extends Model{
    ...
    public function optionIndex($id): array{
        ...
        $duration = DurationModel::alias('d')
                    ...
                    ->leftJoin('module_mf_cloud_price p', 'p.rel_type='.PriceModel::REL_TYPE_OPTION.' AND p.rel_id='.$id.' AND d.id=p.duration_id')
    ...
    public function optionDurationPrice($productId, $relType, $relId = 0, $value = 0, $whereAppend = []){
        ...
        if($type == 'radio'){
            ...
            if(!empty($optionId)){
                $match = true;
                $data = PriceModel::field('duration_id,price')->where('rel_type', PriceModel::REL_TYPE_OPTION)->where('rel_id', $optionId)->select()->toArray();
            ...
            if(!empty($option) && (($value - $option['min_value']) % $option['step'] == 0)){
                ...
                $price = PriceModel::field('duration_id,price')->where('rel_type', PriceModel::REL_TYPE_OPTION)->where('rel_id', $optionId)->select()->toArray();
                foreach($price as $v){
    ...
    public function matchOptionDurationPrice($productId, $relType, $relId = 0, $value = 0, $durationId = 0, $whereAppend = []){
        ...
        if($type == 'radio'){
            ...
            if(!empty($option)){
                $match = true;
                $price = PriceModel::where('rel_type', PriceModel::REL_TYPE_OPTION)->where('rel_id', $option['id'])->where('duration_id', $durationId)->value('price');
            }
        }else if($type == 'step'){
            $whereOption[] = ['min_value', '<=', $value];
            $whereOption[] = ['max_value', '>=', $value];
            ...
            if(!empty($option) && (($value - $option['min_value']) % $option['step'] == 0)){
                ...
                $match = true;
                $price = PriceModel::field('duration_id,price')->where('rel_type', PriceModel::REL_TYPE_OPTION)->where('rel_id', $optionId)->where('duration_id', $durationId)->value('price');
