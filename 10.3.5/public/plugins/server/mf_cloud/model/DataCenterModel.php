            ...
            if(!empty($lineId)){
                ...
                if(!empty($firstDuration)){
                    ...
                    $price = PriceModel::alias('p')
                        ...
                        ->leftJoin('module_mf_cloud_option o', 'p.rel_type='.PriceModel::REL_TYPE_OPTION.' AND p.rel_id=o.id')
    ...
    public function orderPage($param){
        ...
        if(!empty($country)){
            if(isset($param['scene']) && $param['scene'] == 'recommend'){
                // 获取推荐配置
                $recommendConfig = RecommendConfigModel::field('id,name,description,data_center_id,line_id,cpu,memory,system_disk_size,system_disk_type,data_disk_size,data_disk_type,bw,flow,peak_defence,ip_num,upgrade_range')
                                        ->where($where)
                                        ->where('hidden', 0)
                                        ->order('order,id', 'asc')
                                        ->select()
        ...
        // 获取配置
        $config = ConfigModel::field('type,support_ssh_key,support_normal_network,support_vpc_network,support_public_ip,backup_enable,snap_enable,ip_mac_bind,nat_acl_limit,nat_web_limit,ipv6_num,memory_unit,disk_limit_switch,disk_limit_num,free_disk_switch,free_disk_size,only_sale_recommend_config')
                ->where($where)
                ->find();
