...
new Vue({
  ...
  data() {
    return {
      ...
      isShowRefund: false,
      hostStatus: {
        Unpaid: { text: lang.order_text4, color: "#F64E60", bgColor: "#FFE2E5" },
        Pending: { text: lang.finance_text88, color: "#3699FF", bgColor: "#E1F0FF" },
        Active: { text: lang.finance_text142, color: "#1BC5BD", bgColor: "#C9F7F5" },
        Suspended: { text: lang.finance_text143, color: "#F99600", bgColor: "#FFE2E5" },
        Deleted: { text: lang.finance_text144, color: "#9696A3", bgColor: "#F2F2F7" },
        Failed: { text: lang.finance_text88, color: "#FFA800", bgColor: "#FFF4DE" }
      },
      isRead: false,
      isShowPass: false,
      passHidenCode: "",
  ...
  methods: {
    ...
    async getConfig() {
      ...
      try {
        ...
        if (tabVal === '1') {
          this.cycle = this.upgradeList[this.selectUpIndex]?.cycle[0]?.billingcycle
          ...
          const obj = this.upgradeList.reduce((all, cur) => {
            all[cur.id] = numType.includes(cur.option_type) ? cur.qty : cur.subid
            return all
          }, {})
    ...
    handelUpConfirm() {
      ...
      if (this.licenseActive === '1') { // 产品升级结算
        upgradeHost({ id: this.id }).then((res) => {
    ...
    getCloudDetail() {
      ...
      cloudDetail(params).then(res => {
        if (res.data.status === 200) {
          ...
          this.configDetails = []
          const temp = this.cloudData.custom_field_data.filter(item => item.showdetail).reduce((all, cur) => {
            all.push({
              name: cur.fieldname,
              sub_name: cur.value
            })
          ...
          this.configDetails = this.cloudData.config_options.concat(temp).map(item => {
            this.$set(item, 'show', false)
            return item
          })
          this.isShowPort = this.cloudData.custom_field_data.filter(item => item.fieldname === '端口' || item.fieldname === 'port').length > 0
          // 升降级展示
          ...
          if (this.cloudData.system_button.upgrade_option) {
            this.isShowOptionUpdate = !this.cloudData.system_button.upgrade_option.disabled
          }
          // 主IP
          ...
          // 处理系统图标
          const config = this.cloudData.config_options.filter(item => item.option_type === 12)
          ...
          this.osIcon = "/plugins/reserver/mf_finance/template/clientarea/img/remf_finance/" + this.osData[0]?.name + '.svg'
          this.reinstallData.osId = this.osSelectData[0]?.id
          // this.doCheckImage()
          // 根据option_type处理数据
          this.cloudData.cpu = this.cloudData.config_options.filter(item => item.option_type === 6 || item.option_type === 7 || item.option_type === 16)[0]?.sub_name
          this.cloudData.memory = this.cloudData.config_options.filter(item => item.option_type === 8 || item.option_type === 9 || item.option_type === 17)[0]?.sub_name
          this.cloudData.bw = this.cloudData.config_options.filter(item => item.option_type === 10 || item.option_type === 11 || item.option_type === 18)[0]?.sub_name
    ...
    osSelectGroupChange(e) {
      this.osData.map(item => {
        if (item.id == e) {
          this.osSelectData = this.cloudData.cloud_os.filter(item => item.group === e)
          this.osIcon = "/plugins/reserver/mf_finance/template/clientarea/img/remf_finance/" + item.name + '.svg'
          this.reinstallData.osId = this.osSelectData[0]?.id
          // this.doCheckImage()
        }
      })
    ...
    upgradeSub() {
      ...
      const type = this.licenseActive === '1' ? 'upgrade_product' : 'upgrade_config'
      upgradeOrder(type, params).then(res => {
        if (res.data.status === 200) {
    ...
    showRefund() {
      ...
      refundPage(params).then(res => {
        if (res.data.status == 200) {
          ...
          this.refundPageData.configs = this.refundPageData.config_option.data.reduce((all, cur) => {
            all.push({
              name: cur.name,
              value: this.configDetails.filter(el => el.id === cur.field.replace('configoption[', '').replace(']', '') * 1)[0]?.sub_name
            })
            return all
          }, [])
    ...
    getConfigData() {
      ...
      getOrderConfig(params).then(res => {  // 升降级配置数据
    ...
    getBackupList() {
      ...
      backupList(params).then(res => {
        ...
        if (res.data.status === 200) {
          ...
          this.dataList1 = (temp.list || []).filter(item => item.type === 'backup')
          this.dataList2 = (temp.list || []).filter(item => item.type === 'snap')
