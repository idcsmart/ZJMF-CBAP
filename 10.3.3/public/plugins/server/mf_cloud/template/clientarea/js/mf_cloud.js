  ...
  updated() {
    ...
    this.$nextTick(() => {
      this.groupSelect.forEach((item, index) => {
        ...
        item.disabled = dom.offsetWidth > dom.getElementsByClassName('safe-item')[0].offsetWidth + 30
      })
    })
  },
  destroyed() {
      ...
      try {
        ...
        if (!this.isUpdate) { // 不是回填
          this.params = {
            ...
            ip_num: '',
            duration_id: '',
            network_type: this.baseConfig.type !== 'host' ? 'normal' : (this.baseConfig.support_normal_network ? 'normal' : 'vpc'),
            name: '',
            ssh_key_id: '',
            security_group_id: '',
            security_group_protocol: [],
