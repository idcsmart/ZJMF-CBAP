    	...
    	if(empty($line)){
    		...
    		if($line['bill_type'] == 'bw'){
                ...
                if(isset($param['bw']) && !empty($param['bw']) && $param['bw'] != $configData['bw']['value']){
            		...
            		$description[] = sprintf("%s: %d => %d", lang_plugins('mf_dcim_bw'), $configData['bw']['value'], $param['bw']);
                }
                ...
                if(isset($param['flow']) && !empty($param['flow']) && $param['flow'] != $configData['flow']['value']){
            		...
            		$description[] = sprintf("%s: %d => %d", lang_plugins('mf_dcim_flow'), $configData['flow']['value'], $param['flow']);
                }
            }
            // 防护
            if($line['defence_enable'] == 1 && isset($param['peak_defence']) && !empty($param['peak_defence']) && $param['peak_defence'] != ($configData['defence']['value'] ?? 0)){
            	...
            	$description[] = sprintf("%s: %d => %d", lang_plugins('mf_dcim_peak_defence'), $configData['defence']['value'] ?? 0, $param['peak_defence']);
            }
            // 公网IP
            if(isset($param['ip_num']) && !empty($param['ip_num']) && $param['ip_num'] != ($configData['ip']['value'] ?? 0)){
            	...
            	$description[] = sprintf("%s: %d => %d", lang_plugins('mf_dcim_public_ip_num'), $configData['ip']['value'] ?? 0, $param['ip_num']);
            }
    	}
