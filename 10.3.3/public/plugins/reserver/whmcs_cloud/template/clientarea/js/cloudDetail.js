    ...
    getCloudDetail() {
      ...
      cloudDetail(params).then(res => {
        if (res.data.status === 200) {
          ...
          this.configDetails = []
          const temp = Object.values(this.cloudData.configoptions).reduce((all, cur) => {
            all.push({
              name: cur.name,
              sub_name: cur.value
            })
          ...
          const customFile = Object.keys(this.cloudData.customfields).reduce((all, cur) => {
            all.push({
              name: cur,
              sub_name: this.cloudData.customfields[cur]
            })
            return all
          }, [])
          this.configDetails = this.cloudData.config_options.concat(temp, customFile).map(item => {
          ...
          // this.osIcon = "/plugins/reserver/whmcs_cloud/template/clientarea/img/rewhmcs_cloud/" + this.osData[0].name + '.svg'
    ...
    osSelectGroupChange(e) {
      this.osData.map(item => {
        if (item.id == e) {
          ...
          this.osIcon = "/plugins/reserver/whmcs_cloud/template/clientarea/img/rewhmcs_cloud/" + item.name + '.svg'
          this.reinstallData.osId = this.osSelectData[0]?.id
          // this.doCheckImage()
        }
      })
    ...
    getDiskLIoList() {
      ...
      chartList(params).then(res => {
        if (res.data.status === 200) {
          ...
          var diskIoChart2 = echarts.init(document.getElementById('disk2-io-echart'));
