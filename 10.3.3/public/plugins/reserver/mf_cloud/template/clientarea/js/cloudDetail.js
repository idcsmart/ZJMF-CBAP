  ...
  methods: {
    ...
    async getLineDetails() {
      try {
        ...
        this.memoryName = this.cloudData.memory !== 0 ? this.cloudData.memory + this.memory_unit : this.calaMemoryList[0]?.value + this.memory_unit
        // 默认选择防御
    ...
    getImage() {
      ...
      image(params).then(res => {
        if (res.data.status === 200) {
          ...
          this.osIcon = "/plugins/server/mf_cloud/template/clientarea/img/mf_cloud/" + this.osData[0].name + '.svg'
    ...
    osSelectGroupChange(e) {
      this.osData.map(item => {
        if (item.id == e) {
          ...
          this.osIcon = "/plugins/server/mf_cloud/template/clientarea/img/mf_cloud/" + item.name + '.svg'
    ...
    addMoreDisk() {
      ...
      if ((newNum + oldNum) < max) {
        ...
        if (this.dataDiskList.length !== 0) {
          itemData.size = this.dataDiskList[0].type === 'radio' ?
            (this.dataDiskList[0]?.value || "") : obj[obj.disk_typeList[0]]?.min_value;
        ...
        this.$message({
          message: lang.mf_tip29 + this.configObj.disk_limit_num + lang.mf_tip36,
          type: 'warning'
        })
      }
    },
