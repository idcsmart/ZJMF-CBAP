    ...
    new Vue({
      data () {
        return {
          ...
          typeRules: {
            name: [
              { required: true, message: lang.input + lang.feedback_type, type: 'error', trigger: 'blur' },
              // {
              //   validator: val => val?.length <= 10, message: lang.verify8 + '1-10', type: 'warning'
              // }
            ],
            description: [
              { required: true, message: lang.input + lang.type + lang.description, type: 'error', trigger: 'blur' },
            ],
          },
          popupProps: {
      ...
      created () {
        this.getFeedbackList()
        this.getFeedbackTypeList()
        document.title = lang.feedback + '-' + localStorage.getItem('back_website_name')
      },
      mounted() {
        this.initViewer()
      },
