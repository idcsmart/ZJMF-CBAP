  ...
  window.onload = function () {
    ...
    aside && new Vue({
      ...
      methods: {
        ...
        async getVersion() {
          try {
            if (sessionStorage.isCanUpdata === 'true' || sessionStorage.isCanUpdata === 'false') {
              // 字符串转布尔值
              this.isCanUpdata = sessionStorage.isCanUpdata === 'true'
            } else {
              const res = await version()
              const systemData = res.data.data
              this.isCanUpdata = this.checkVersion(systemData.version, systemData.last_version)
            }
        ...
        async getNewVersion() {
          try {
            if (sessionStorage.pluginUpgrade === 'true' || sessionStorage.pluginUpgrade === 'false') {
              // 字符串转布尔值
        ...
        setWebTitle() {
          ...
          const url = urlArr.length > 3 ? urlArr.slice(2).join('/') : urlArr[urlArr.length - 1]
          ...
          menu.forEach(fir => {
            ...
            if (temp.length > 0) {
              let menu_id = ''
              if (location.pathname.includes('client_') && !location.pathname.includes('idcsmart_client_') && !location.pathname.includes('client_care') && !location.pathname.includes('client_custom_field')) {
                menu_id = temp.filter(e => e.url === 'client.htm')[0]?.id
              } else if (location.pathname.includes('supplier_')) {
                menu_id = temp.filter(e => e.url === 'supplier_list.htm')[0]?.id
              } else {
              ...
              temp.forEach(sec => {
                if (sec.url === url) {
                  ...
                  localStorage.lastTitle = (url === 'index.htm' ? lang.home : sec.name) + '-' + website_name
                  isSetTitle = true
                } else if (url.includes('plugin') && sec.url.split('/')[1] === url.split('/')[1] && !isSetTitle) {
                  // document.title = (url === 'index.htm' ? lang.home : sec.name) + '-' + website_name
              ...
              if (fir.url === url) {
                document.title = (url === 'index.htm' ? lang.home : fir.name) + '-' + website_name
              } else if (url.includes('plugin') && fir.url.split('/')[1] === url.split('/')[1]) {
                document.title = (url === 'index.htm' ? lang.home : fir.name) + '-' + website_name
              } else {
          ...
          /* 首页 */
          if ((location.origin + '/' + location.pathname.split('/')[1] + '/index.htm') === location.href) {
        ...
        jumpHandler(e) {
          ...
          if (
            ...
            || (e.child && str + e.child[0].url.includes('idcsmart_ticket_internal'))) {
        ...
        changeLang(e) {
          ...
          if (localStorage.getItem('backLang') !== e.value || !localStorage.getItem('backLang')) {
            if (localStorage.getItem('backLang')) {
