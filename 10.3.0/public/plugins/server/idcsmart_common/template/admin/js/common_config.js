...
new Vue({
  data () {
    return {
      ...
      dataRules: {
        ...
        server_id: [
          { required: true, message: `${lang.select}${lang.child_interface}`, type: 'error' }
        ]
      },
      /* 周期表格 */
      defaultBol: false,
      ...
      cycleRules: {
        ...
        cycle_time: [
          ...
          {
            validator: val => val > 0, message: lang.input + lang.verify16, type: 'warning'
          }
        ],
        amount: [
          { required: true, message: lang.input + lang.money, type: 'error' },
      ...
      subRules: { // 子配置验证
        ...
        option_name: [
          { required: true, message: lang.select + lang.option_value, type: 'error' },
        ],
        qty_min: [
          ...
          {
            validator: val => val >= 0, message: lang.input + lang.verify7, type: 'warning'
          }
        ],
        qty_max: [
          { required: true, message: lang.input + lang.verify7, type: 'error' },
      ...
      configRules: {
        option_name: [
          { required: true, message: lang.input + lang.option_name, type: 'error' },
          {
            validator: val => val?.length <= 20, message: lang.verify8 + '1-20', type: 'warning'
          }
        ],
        option_type: [
          { required: true, message: lang.select + lang.option_type, type: 'error' },
        ],
        fee_type: [
          { required: true, message: lang.select + lang.cost_type, type: 'error' },
        ],
        amount: [
          { required: true, message: lang.input + lang.money, type: 'error' },
          { pattern: /^\d+(\.\d{0,2})?$/, message: lang.verify5, type: 'warning' },
  ...
  methods: {
    ...
    async submitConfig () {
      try {
        const { order_page_description, allow_qty, auto_support, server_id } = this.commonConfig
