  ...
  window.onload = function () {
    ...
    new Vue({
      data() {
        return {
          ...
          rules: {
            ...
            visible_range: [{ required: true, message: lang.file_download_text33 }],
            product_id: [{ required: true, message: lang.file_download_text34 }],
          },
          formData: {
            addon_idcsmart_file_folder_id: '',
            product_id: [],
      ...
      computed: {
        ...
        calcSize() {
          return (row) => {
            return `(${row.filesize / 1024 / 1024 >= 1 ? (row.filesize / 1024 / 1024).toFixed(2) + "M" : (row.filesize / 1024).toFixed(2) + "KB"})`
          }
        }
      },
      methods: {
        ...
        async onswitch(e, el) {
          ...
          this.$message.success(el ? lang.file_download_text35 : lang.file_download_text36);
          this.getfilelist();
        },
        //编辑
        async edit(id) {
