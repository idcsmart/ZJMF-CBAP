    ...
    private function cyclesFilter(HostModel $host,$cycles,$promoCode='')
    {
        ...
        foreach ($cycles as $k2=>$item2){
            ...
            if ($flag){ # 自然月导致前一个判断可能不生效,后一个判断在周期名称相同下也不生效
                ...
                # 产品当前周期的价格以 表里数据为准(wyh 20230509改 以最大的值为主)
...
//                    $cycles[$k2]['price'] = bcsub((float)$host->renew_amount,0,2);
            ...
            $flag1 = (((float)$host->renew_amount > $cycles[$k2]['price']) && $flag)?true:false;


            // 等级折扣+优惠折扣后，取最大金额
            $cycles[$k2]['max_renew'] = $flag1;

            $cycles[$k2]['price'] = $flag1?(float)$host->renew_amount:$cycles[$k2]['price'];
            $cycles[$k2]['renew_amount'] = $flag1?(float)$host->renew_amount:$cycles[$k2]['base_price'];
        }
