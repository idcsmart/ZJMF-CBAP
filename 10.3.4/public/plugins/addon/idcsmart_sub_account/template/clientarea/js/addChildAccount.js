    ...
    new Vue({
      ...
      data() {
        return {
          ...
          rules: {
            username: [
              { required: true, message: lang.withdraw_placeholder4, trigger: "blur" },
              {
                min: 3,
      ...
      methods: {
        ...
        getCommonData() {
          ...
          document.title = this.commonData.website_name + "-" + lang.subaccount_text43;
        },
        getCountry() {
          queryCountryAPI().then(res => {
            if (res.data.status === 200) {
              this.countryList = res.data.data.list
              let china = this.countryList.find(item => item.name_zh == '中国')
          ...
          if (res.status === 200) {
            ...
            setTimeout(() => {
              this.$nextTick(() => {
                ...
                this.leftPermissionsID = this.$refs.leftTree.getCheckedKeys().concat(this.$refs.leftTree.getHalfCheckedKeys())
                this.rightPermissionsID = this.$refs.rightTree.getCheckedKeys().concat(this.$refs.rightTree.getHalfCheckedKeys())
              });
            }, 1000);
          }
        },
