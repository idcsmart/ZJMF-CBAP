      ...
      methods: {
        ...
        initTemplate() {
          tinymce.init({
            ...
            language: localStorage.getItem('backLang') === 'zh-cn' ? 'zh_CN' : 'en_US',
            min_height: 400,
            width: '100%',
        ...
        //上传失败
        handleFail({ file }) {
          this.$message.error(`${lang.help_text45} ${file.name} ${lang.help_text46}`);
        },
        //文档类型
        async gettype() {
          ...
          this.$refs.myform.validate(this.requiredRules).then((res) => {
            if (res === true) {
              ...
              if (this.id) {
                ...
                temp.cron_release_time = parseInt(new Date(temp.cron_release_time).getTime() / 1000)
