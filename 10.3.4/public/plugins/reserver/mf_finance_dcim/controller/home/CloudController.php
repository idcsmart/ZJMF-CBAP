...
class CloudController
{
        ...
        try{
            ...
            if ($result['status']==200){
                ...
                if ($result['status']==200){
                    ...
                    $res['msg'] = $result['msg'];
                    if ($RouteLogic->profit_type==1){
                        $res['data']['price'] = bcadd($result['data']['subtotal']??0, $RouteLogic->getProfitPercent()*100,2);
                    }else{
                        $res['data']['price'] = bcmul($result['data']['subtotal']??0, (1+$RouteLogic->getProfitPercent()),2);
                    }
    ...
    public function upgradeConfigPost(){
        ...
        $data = [
            ...
            'type'        => 'upgrade_config',
            'amount'      => $RouteLogic->profit_type==1?bcadd($res['data']['subtotal']??0,$RouteLogic->getProfitPercent()*100,2):bcmul($res['data']['subtotal']??0, (1+$RouteLogic->getProfitPercent()),2),//$res['data']['subtotal'],
            'description' => $res['data']['description'],
            'price_difference' => $RouteLogic->profit_type==1?bcadd($res['data']['total']??0,$RouteLogic->getProfitPercent()*100,2):bcmul($res['data']['total']??0, (1+$RouteLogic->getProfitPercent()),2),//$res['data']['total'],
            'renew_price_difference' => $RouteLogic->profit_type==1?bcadd($res['data']['total']??0,$RouteLogic->getProfitPercent()*100,2):bcmul($res['data']['total']??0, (1+$RouteLogic->getProfitPercent()),2),// $res['data']['total'],
            'upgrade_refund' => 0,
            'config_options' => [
                //'configoption' => $param['configoption']??[],
                // 取接口返回的配置
