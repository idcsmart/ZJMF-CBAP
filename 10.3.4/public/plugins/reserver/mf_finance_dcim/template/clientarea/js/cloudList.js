...
new Vue({
  ...
  data () {
    return {
      ...
      powerStatus: {
        ...
        off: { text: lang.common_cloud_text11, icon: `${url}/img/cloud/off.png` },
        operating: { text: lang.common_cloud_text12, icon: `${url}/img/cloud/operating.png` },
        fault: { text: lang.common_cloud_text86, icon: `${url}/img/cloud/fault.png` },
        suspend: { text: lang.common_cloud_text87, icon: `${url}/img/cloud/suspended.png` }
      },
      status: {
        ...
        Suspended: { text: lang.common_cloud_text91, color: "#F99600", bgColor: "#FFF4DE" },
        Deleted: { text: lang.common_cloud_text92, color: "#9696A3", bgColor: "#F2F2F7" },
        Failed: { text: lang.common_cloud_text93, color: "#3699FF", bgColor: "#E1F0FF" }
      },
      statusSelect: [
        {
          id: 1,
  ...
  methods: {
    ...
    getCommon () {
      ...
      document.title = this.commonData.website_name + '-' + lang.common_cloud_text94
    },
    // 切换分页
    sizeChange (e) {
      this.params.limit = e
        ...
        if (res.data.status === 200) {
          ...
          this.cloudData = this.cloudData.map(item => {
            this.getHostInfo(item.id).then(res => {
              if (res) {
                const os = res.config_options.filter(el => el.name_k === 'os')
                item.ip = res.host_data.dedicatedip
                item.image_group_name = os[0]?.os_group
                item.image_name = os[0]?.sub_name
              }
              item.loading = false
              this.itemKey ++
            })
