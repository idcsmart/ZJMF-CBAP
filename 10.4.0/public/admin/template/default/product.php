    ...
    <t-card class="list-card-container" :class="{stop: data.status===0}">
      <div class="common-header">
        <div class="left">
          ...
          <t-button theme="default" @click="addAgency" class="add">{{lang.immediate_agency}}</t-button>
          <t-button theme="default" class="add anget-btn" @click="handelAngenBtn">{{lang.manage_agency}}</t-button>
        </div>
      ...
      <t-enhanced-table ref="table" row-key="key" drag-sort="row-handler" :data="data" :columns="columns" :key="new Date().getTime()" :tree="{ treeNodeColumnIndex: 1 }" :loading="loading" :hover="hover" :tree-expand-and-fold-icon="treeExpandAndFoldIconRender" :before-drag-sort="beforeDragSort" @abnormal-drag-sort="onAbnormalDragSort" @drag-sort="changeSort" class="product-table" :row-class-name="rowName">
        ...
        <template #name="{row}">
          <a :href="calcHref(row)" v-if="row.qty !== undefined" class="product-name" style="cursor: pointer;" @click="editHandler(row)">{{row.name}}</a>
          <!-- <template v-else>
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.copy" :show-arrow="false" theme="light" v-if="row.key.indexOf('t') !== -1">
