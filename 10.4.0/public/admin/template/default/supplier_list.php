...
<div id="content" class="supplier_list" v-cloak>
  <com-config>
    <t-card class="list-card-container">
      <div class="add-btn">
        <t-button @click="addSupplier">{{lang.upstream_text41}}</t-button>
      </div>
      <t-table row-key="id" :data="data" size="medium" :columns="columns" @sort-change="sortChange" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" display-type="fixed-width">
        ...
        <template #name="{row}">
          <span style="color: var(--td-brand-color); cursor: pointer;" @click="goDetail(row.id)">{{row.name}}</span>
        </template>
        <template #type="{row}">
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.look" :show-arrow="false" theme="light">
            <t-icon name="view-module" @click="goDetail(row.id)" size="18px" style="color: var(--td-brand-color); margin-right: 10px; cursor: pointer;"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.invoice_text19" :show-arrow="false" theme="light">
            <t-icon name="edit-1" @click="handelEdit(row)" size="18px" style="color: var(--td-brand-color); margin-right: 10px; cursor: pointer;"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.box_content8" :show-arrow="false" theme="light">
            <t-icon name="delete" @click="handelDel(row.id)" size="18px" style="color: var(--td-brand-color); cursor: pointer;"></t-icon>
          </t-tooltip>
        </template>
      </t-table>
