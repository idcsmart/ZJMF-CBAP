  ...
  <com-config class="no-bg">
    ...
    <t-card class="list-card-container">
      <div class="com-h-box">
        ...
        <!-- 顶部右侧选择用户 -->
        <com-choose-user :cur-info="data" :check-id="id"  @changeuser="changeUser" class="com-clinet-choose">
        </com-choose-user>
      </div>
    </t-card>
    ...
    <div class="info-card">
      <t-card :class="{ stop: data.status===0}">
        ...
        <div class="header-btn">
          ...
          <t-button theme="primary" type="submit" :disabled="data.status===0" @click="loginByUser">{{lang.login_as_user}}</t-button>
        </div>
        <div class="info-box">
          <t-row align="middle">
            <t-col :span="6">
              ...
              <template v-if="hasCertification">
                ...
                <div v-if="data.certification === false">
                  <t-tooltip :content="lang.user_text10" theme="light" :show-arrow="false" placement="top-right">
                    <span style="display: flex; align-items: center;">{{lang.user_text11}}<img src="/{$template_catalog}/template/{$themes}/img/icon/no_authentication.png" alt=""></span>
                  </t-tooltip>
                </div>
      ...
      <t-card>
        ...
        <t-table row-key="1" class="ip-table" :data="data.login_logs" size="medium" :bordered="true" :columns="logColumns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'">
          <template #login_time="slotProps">
            {{ moment(slotProps.row.login_time * 1000).format('YYYY-MM-DD HH:mm:ss') }}<span v-if="slotProps.rowIndex === 0">({{lang.user_text15}})</span>
          </template>
    ...
    <t-card class="user-info">
      ...
      <t-form :data="formData" label-align="top" layout="inline" :rules="rules" ref="userInfo">
        ...
        <t-form-item :label="lang.password" name="password">
          <t-input type="password" v-model="formData.password" :placeholder="lang.password" :type="formData.password ? 'password' : 'text'" autocomplete="off"></t-input>
        </t-form-item>
    ...
    <t-card class="user-info" v-show="childList.length > 0">
      ...
      <div class="login-log chlid-box" style="margin-bottom:40px">
        ...
        <t-pagination show-jumper v-if="total" :total="total" :page-size="params.limit" :page-size-options="logSizeOptions" :on-change="changePage" />
      </div>
    </t-card>
