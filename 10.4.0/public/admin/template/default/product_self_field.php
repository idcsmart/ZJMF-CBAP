      ...
      <div class="product-self">
        ...
        <t-table hover row-key="index" :loading="loading" :data="list" :columns="columns" drag-sort="row-handler" @drag-sort="onDragSort">
          ...
          <template #op="slotProps">
            <t-icon name="edit-1" color="var(--td-brand-color)" style="margin-right: 10px;" @click="edit(slotProps.row)" class="common-look">
            </t-icon>
            <t-icon name="delete" color="var(--td-brand-color)" class="common-look" @click="deletes(slotProps.row.id)"></t-icon>
          </template>
        </t-table>
      </div>
    </t-card>
