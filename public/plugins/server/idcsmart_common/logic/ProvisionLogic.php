...
class ProvisionLogic
{
    ...
    public function execClientButton($hostid = 0, $func = ''){
        ...
        if($params['api_type'] == 'zjmf_api'){
            ...
            $module = $params['module_type'];
            # TODO wyh 20210707 授权管理 有此方法 验证授权
            if (function_exists($module . '_idcsmartauthorize')){
                $res = serverModuleIdcsmartauthorize($module);
                if ($res['status'] != 200){
    ...
    protected function execSupportFunc($name, $hostid = 0){
        $params = $this->getParams($hostid);
        $module = $params['module_type'];
        # TODO wyh 20210712 授权管理 有此方法 验证授权 执行系统默认方法也需验证
        if (function_exists($module . '_idcsmartauthorize')){
            $res = serverModuleIdcsmartauthorize($module);
            if ($res['status'] != 200){
                $result['status'] = 'error';
    ...
    public function execCustomFunc($name, $hostid = 0, $type = 'client'){
        ...
        // 	if($result['status'] == 'success'){
        // 		active_log(lang('MODULE_EXEC_SUCCESS', [$module, $name, $params['hostid']]));
        // 	}else{
        // 		active_log(lang('MODULE_EXEC_FAILED', [$module, $name, $params['hostid'], $result['msg']]));
        // 	}
        // }
        return $result;
    }
    ...
    public function afterFlowPacketPaid($hostid, $packet){
        ...
        // if(!empty($module) && !empty($params['hostid']) && $define && $name != 'status'){
        // 	if($result['status'] == 'success'){
        // 		active_log(lang('MODULE_EXEC_SUCCESS', [$module, $this->support[ucfirst($name)]['des'], $params['hostid']]));
        // 	}else{
        // 		active_log(lang('MODULE_EXEC_FAILED', [$module, $this->support[ucfirst($name)]['des'], $params['hostid'], $result['msg']]));
        // 	}
        // }
        return $result;
    }
