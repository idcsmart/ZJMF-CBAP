...
class DataCenterModel extends Model{
	...
	protected $name = 'module_idcsmart_dcim_data_center';
    ...
    public function createDataCenter($param)
    {
        ...
        $this->startTrans();
        try{
            $dataCenter = $this->create($param, ['country_id','city','product_id','create_time','order']);
    ...
    public function updateDataCenter($param)
    {
        ...
        $oldCountry = CountryModel::find($dataCenter['country_id']);
        ...
        $this->startTrans();
        try{
            $dataCenter = $this->update($param, ['id'=>$param['id']], ['country_id','city']);
        ...
        if(!empty($description)){
            $description = lang_plugins('log_modify_data_center_success', [
                '{name}'=>$oldCountry['name_zh'].$dataCenter['city'],
                '{detail}'=>$description,
            ]);
     ...
     * @author hh
     * @version v1
     * @param   DataCenterModel DataCenterModel - 数据中心模型
     * @return  string
     */
