...
new Vue({
    ...
    calcReg () { // 动态生成规则
      return (name, min, max) => {
        return [
          ...
          {
            pattern: /^[0-9]*$/, message: lang.input + `${min}-${max}` + lang.verify2, type: 'warning'
          },
          {
            validator: val => val >= min && val <= max, message: lang.input + `${min}-${max}` + lang.verify2, type: 'warning'
          }]
      }
    }
  },
  methods: {
    ...
    async handlerMemory () {
      try {
        let { id, value, type, price, min_value, max_value } = this.calcForm
        price = price.reduce((all, cur) => {
          cur.price && (all[cur.id] = cur.price)
          return all
        }, {})
