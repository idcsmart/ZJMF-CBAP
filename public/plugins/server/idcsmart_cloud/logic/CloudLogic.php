...
class CloudLogic
{
	...
	public function changeVpcNetwork($param){
		...
		if($res['status'] == 200){
			...
			set_time_limit(450);
			for($i = 0; $i<60; $i++){
				$task = $this->idcsmartCloud->taskDetail($res['data']['taskid']);
			...
			if($detail['status'] == 200){
				...
				if($detail['data']['network_type'] == 'vpc'){
					...
					$IdcsmartVpcLinkModel->saveVpcLink(['addon_idcsmart_vpc_id'=>$vpc['id'], 'server_id'=>$this->server['id'], 'vpc_network_id'=>$detail['data']['network'][0]['vpc']]);
				}
			}
			$result = [
				'status' => 200,
