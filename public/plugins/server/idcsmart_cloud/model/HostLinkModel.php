...
class HostLinkModel extends Model{
    ...
    public function detail($hostId){
        ...
        if(!empty($hostLink)){
            ...
            $cal = CalModel::field('id,name,cpu,memory,disk_size,description')->find($package['module_idcsmart_cloud_cal_id'] ?? 0);
            $bw = BwModel::alias('b')
                ->field('b.id,b.bw,b.flow,b.description,bt.name bw_type_name')
    ...
    public function calUpgradePackagePrice($param){
        ...
        $diffPrice = $priceDifference*(($diffTime < 0 ? 0 : $diffTime)/$durationPrice['duration']/24/3600);
