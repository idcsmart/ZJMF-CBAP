    ...
    <div class="index_help">
      <div class="index_item" v-for="(item,index) in list" :key="index">
        ...
        <t-select class="select_index" size="small" @Change="changetitle($event,index)" v-model="item.id">
          <t-option v-for="(it,ind) in filterInterface" :key="ind" :value="it.id" :label="it.name" :disabled="it.disabled" />
          <!-- {{it.name}} -->
          <!-- </t-option> -->
        </t-select>
        ...
        <div class="chosedocument">
          <div>
            <t-checkbox checked="item.index_hot_show?true:false" v-model="item.index_hot_show?true:false" :disabled="item.id===0" @change="hotchange($event,item.id)">按照访问热度自动显示
            </t-checkbox>
          </div>
          <div class="chosebtn" @click="mobile_file(item.id,index)">选择文档</div>
        </div>
      </div>
    </div>
    <div class="div" style="display: flex;justify-content: center;">
      <div class="chosebtn" @click="Confirmindex">保存</div>
      <t-button theme="default" variant="base" style="width: 80px;margin-left:10px" @click="backList">返回</t-button>
    </div>
  </t-card>
  <t-dialog :close-btn="false" placement="center" :visible.sync="showdialog" @Cancel="onCancel" width="40%" cancel-btn="关闭" @Confirm="Confirmindex">
    <div class="content">
      <div class="content_left">
        <div>文档分类 > <span class="blodtitle">{{dialog_name?dialog_name:dialog.name}}</span></div>
        <t-input class="inputsearchbox" v-model="params.keywords" placeholder="请输入你需要搜索的内容" @change="keywordssearch(1)">
        ...
        <t-checkbox-group class="con_chexkbox" v-model="checkgroup" @change="titlecheck($event)">
          <t-tooltip v-for="(it,ind) in searchlist" :key="it.id" v-if="it.hidden===0" class="placement top left" :content="it.title" placement="top-left" :show-arrow='false' theme="light">
            <t-checkbox class="checkboxitem" :value="it.id" :class="{checkboxitem_acitve:checkgroup.includes(it.id)}">
              {{it.title}}
            </t-checkbox>
          </t-tooltip>
        ...
        <div class="con_page">
          ...
          <span class="chevron">
            <t-pagination theme="simple" size="small" :current="params.page" :total="total" :page-size.sync="params.limit" @Change="changePage" />
          </span>
