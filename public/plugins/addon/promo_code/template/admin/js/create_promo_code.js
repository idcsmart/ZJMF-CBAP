    ...
    new Vue({
      ...
      computed: {
        ...
        calcPlaceholder () {
          switch (this.formData.type) {
            case 'percent':
              return `${lang.input}${lang.discount_ratio}`
            case 'fixed_amount':
              return `${lang.input}${lang.deduction_amount}`
            case 'replace_price':
              return `${lang.input}${lang.cover_amount}`
          }
        }
      },
      created () {
      ...
      methods: {
        ...
        async getPromo () {
          try {
            ...
            temp.products = temp.products.map(item => {
              item = 't-' + item
              return item
            })
            temp.need_products = temp.need_products.map(item => {
              item = 't-' + item
              return item
            })
            this.formData = temp
        ...
        async onSubmit ({ validateResult, firstError }) {
          if (validateResult === true) {
            try {
              ...
              params.need_products = params.need_products.map(item => {
                item = item.replace('t-', '') * 1
                return item
              }).filter(item => item)
              console.log('$$$$$', params)
        ...
        async getProductList () {
          try {
            ...
            let tempFir = firstGroup.data.data.list
            tempFir.forEach(item => {
            ...
            tempFir = tempFir.filter(item => item.children.length > 0)
            tempFir.forEach(item => {
            ...
            // 剔除一二级分组为空的数据
            tempFir = tempFir.map(item => {
              item.children = item.children.filter(ele => ele.children.length > 0)
              return item
            })
            this.productList = tempFir
          } catch (error) {
            console.log(error)
          }
        }
