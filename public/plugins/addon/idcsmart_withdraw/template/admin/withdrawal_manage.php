  ...
  <t-dialog :close-btn="false" :visible.sync="sourceModal" class="sourceModal" :footer="false">
    <div class="box">
      <div class="item">
        ...
        <div class="com-search">
          <t-input v-model="pluginKey" class="search-input" :placeholder="`${lang.input}`" @keyup.enter.native="seacrhPlugin" :on-clear="clearPluginKey" clearable>
          </t-input>
          <t-icon size="20px" name="search" @click="seacrhPlugin" class="com-search-btn" />
        </div>
        <t-table row-key="id" :columns="originColumns" :data="originList" @select-change="rehandleSelectChange" :selected-row-keys="checkId" :max-height="520">
        </t-table>
      </div>
