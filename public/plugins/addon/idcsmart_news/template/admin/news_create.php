...
<link rel="stylesheet" href="/plugins/addon/idcsmart_news/template/admin/css/common/reset.css" />
                ...
                <div id="content" class="document newscreat" v-cloak>
                    <t-card class="add_document">
                        ...
                        <div class="add_form">
                            <t-form label-align="top" :data="detialform" class="add_tform" ref="myform"
                                :rules="requiredRules">
                                <t-form-item label="文档名称" name="title" class="inlineflex">
                                    <t-input placeholder="请输入" v-model="detialform.title" style="width: 250px;" />
                                </t-form-item>
                                <t-form-item label="文档类型" name="addon_idcsmart_news_type_id" class="inlineflex">
                                    <t-select bordered style="width: 250px;"
                                        v-model="detialform.addon_idcsmart_news_type_id">
                                        <t-option v-for="(item,index) in typelist" :key="item.id" :label="item.name"
                                            :value="item.id" />
                                    </t-select>
                                </t-form-item>
                                    ...
                                    <div v-if="files && files.length" class='list-custom'>
                                        <ul>
                                            <li v-for="(item, index) in files" :key="index">
                                                {{ item.name}}
                                                <t-icon class="delfile" name="close-circle" color="#ccc"
                                                    @click="delfiles(item.name)"></t-icon>
                                            </li>
                                        </ul>
                                    </div>
                                </t-form-item>
