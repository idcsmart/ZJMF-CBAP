<?php
namespace gateway\wx_pay_h5;

use app\common\lib\Plugin;
use gateway\wx_pay_h5\validate\WxPayH5Validate;
use gateway\wx_pay_h5\lib\WxPayConfig;
use gateway\wx_pay_h5\lib\WxPayUnifiedOrder;
use gateway\wx_pay_h5\lib\WxPayApi;

require_once __DIR__ . '/lib/WxPayApi.php';
require_once __DIR__ . '/lib/WxPayData.php';
require_once __DIR__ . '/lib/WxPayConfig.php';

class WxPayH5 extends Plugin
{
    // 插件基础信息
    public $info = array(
        'name'        => 'WxPayH5', // 必填 插件标识(唯一)
        'title'       => '微信支付H5支付和PC端扫码支付', // 必填 插件显示名称
        'description' => '微信支付H5支付和PC端扫码支付', // 必填 插件功能描述
    ...
    public function WxPayH5Handle($param)
    {
        $validate = new WxPayH5Validate();
    ...
    private function setPay($param)
    {
        ...
        if($result){
            ...
            if (isset($result['mweb_url']) && $result['mweb_url']){
                ...
                return $html;
            }elseif (isset($result['code_url']) && $result['code_url']){
    ...
    {
        if($input->GetTrade_type() == "MWEB" || $input->GetTrade_type() == "NATIVE")
        {
            try{
