{include file="header"}
<!-- 页面独有样式 -->
<link rel="stylesheet" href="/{$template_catalog}/template/{$themes}/css/order.css">
</head>
...
<body>
                                ...
                                <div class="order-item">
                                    ...
                                    <div class="package-content">
                                        <div class="package-item" :class="item.id==orderData.packageId?'active':null" v-for="item in packageDataPage" :key="item.id" @click="packageItemClick(item)">
                                            <div class="money">
                                                {{commonData.currency_prefix}}{{item | showFee}}
                                            </div>
                                            <div class="package-line"></div>
                                            <span class="description" v-html="item.description">
                                ...
                                <div class="order-item bs-item" v-if="configData.backup_enable == 1">
                                    ...
                                    <div class="right" v-show="isBack">
                                        ...
                                        <el-select class="num-price-select" v-if="configData.backup_option.length>1" v-model="orderData.backId" @change="backSelectChange">
                                            <el-option v-for="item in configData.backup_option" :key='item.id' :value="item.id" :label="item.num+'个备份'+ commonData.currency_prefix +item.price"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <!-- 快照 -->
                                <div class="order-item bs-item" v-if="configData.snap_enable == 1">
                                    ...
                                    <div class="right" v-show="isSnapshot">
                                        ...
                                        <el-select class="num-price-select" v-if="configData.snap_option.length>1" v-model="orderData.snapId" @change="snapSelectChange">
                                            <el-option v-for="item in configData.snap_option" :key='item.id' :value="item.id" :label="item.num+'个快照'+ commonData.currency_prefix +item.price"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <!-- 操作系统 -->
                                    ...
                                    <div class="item-bottom">
                                        <div class="pass" v-show="isPassOrKey == 'pass'">
                                            ...
                                            <div class="text">
                                                ...
                                                <span class="text-2">长度5-15个字符；支持英文和数字；必须含有特殊字符</span>
                                            </div>
                                        </div>
                            ...
                            <div class="order-right">
                                <div class="right-main">
                                    ...
                                    <div class="order-right-item">
                                        ...
                                        <div class="row" v-show="osPrice>0">
                                            <div class="label">操作系统</div>
                                            <div class="value">{{commonData.currency_prefix+ osPrice}}</div>
                                        </div>
                                        <!-- 磁盘 -->
                                        <div class="row" v-if="isMoreDisk">
                                            <div class="label">{{moreDiskData.length}}个数据盘</div>
                                            <div class="value">{{commonData.currency_prefix+moreDiskPrice}}</div>
                                        </div>

                                        <!-- 备份 -->
                                        <div class="row" v-if="isBack">
                                            <div class="label">备份功能</div>
                                            <div class="value">{{backNum + '个备份' + commonData.currency_prefix + backPrice}}</div>
                                        </div>
                                        <!-- 快照 -->
                                        <div class="row" v-if="isSnapshot">
                                            <div class="label">快照功能</div>
                                            <div class="value">{{snapNum + '个快照' + commonData.currency_prefix + snapPrice}}</div>
                                        </div>
                                    </div>
                                    <div class="order-right-item">
                                        <div class="row">
                                ...
                                <div class="order-right-footer">
                                    ...
                                    <div class="footer-total">
                                        ...
                                        <div class="right">{{commonData.currency_prefix + totalPrice}}</div>
                                    </div>
                                    <!-- 优惠码 -->
    ...
    <script src="/{$template_catalog}/template/{$themes}/api/order.js"></script>
