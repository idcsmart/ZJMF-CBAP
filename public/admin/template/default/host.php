  ...
  <t-card class="list-card-container">
    <div class="common-header">
      ...
      <div class="com-search">
        ...
        :placeholder="`${lang.please_search}ID、${lang.product_name}、${lang.products_token}、${lang.username}、${lang.email}、${lang.phone}`" 
          @keyup.native.enter="seacrh" :on-clear="clearKey" clearable>
        </t-input>
    ...
    <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
      ...
      <template #id="{row}">
        <a :href="`host_detail.html?client_id=${row.client_id}&id=${row.id}`"
          class="aHover">{{row.id}}</a>
      </template>
      <template #status="{row}">
    ...
    <t-pagination :total="total" v-if="total" :current="params.page" :page-size="params.limit"
