...
<div id="content" class="transaction table" v-cloak>
  <t-card class="list-card-container">
    <div class="common-header">
      ...
      <div class="com-search">
        <t-input v-model="params.keywords" class="search-input"
          :placeholder="`${lang.please_search}${lang.flow_number}、${lang.order}ID、${lang.username}、${lang.email}、${lang.phone}`" 
          @keyup.enter.native="seacrh" :on-clear="clearSearch" clearable>
        </t-input>
    ...
    <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
      ...
      <template #amount="{row}">
        {{currency_prefix}}&nbsp;{{row.amount}}<span
          v-if="row.billing_cycle">/</span>{{row.billing_cycle}}
      </template>
      <template #order_id="{row}">
        <a href="javascript:;" v-if="row.order_id" @click="rowClick(row)" class="aHover">{{row.order_id}}</a>
      </template>
    ...
    <t-pagination v-if="total" :total="total" :page-size="params.limit" :current="params.page" :page-size-options="pageSizeOptions"
    ...
    <t-form :data="formData" ref="form" @submit="onSubmit" :rules="rules">
      ...
      <t-form-item :label="lang.user" name="client_id" class="user">
        <t-select v-model="formData.client_id" filterable :placeholder="lang.example" :loading="searchLoading"
          reserve-keyword :on-search="remoteMethod" clearable @clear="clearKey" :show-arrow="false">
