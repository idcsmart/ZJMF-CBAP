...
<div id="content" class="server-group table" v-cloak>
  <t-card class="list-card-container">
    <div class="common-header">
      ...
      <div class="com-search">
        <t-input v-model="params.keywords" class="search-input"
          :placeholder="`${lang.please_search}${lang.nickname}`"
    ...
    <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
      :table-layout="tableLayout ? 'auto' : 'fixed'" @sort-change="sortChange" 
      :hide-sort-tips="hideSortTips" :max-height="maxHeight">
      ...
      <template #op="{row}">
        <a class="common-look" @click="updateHandler(row)">{{lang.edit_group}}</a>
  ...
  <t-dialog :visible.sync="visible" :header="title" :on-close="close" :footer="false" width="600">
    <t-form :rules="rules" :data="formData" ref="userDialog" @submit="onSubmit" v-if="visible">
      ...
      <t-form-item :label="lang.interface" name="server_id">
        <t-select v-model="formData.server_id" :placeholder="lang.select+lang.interface" multiple :max-collapsed-num="5"
