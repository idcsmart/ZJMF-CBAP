  ...
  <t-card class="list-card-container">
    ...
    <div class="box">
      <t-form :data="formData" :label-width="80" ref="userDialog" label-width="200" @submit="onSubmit"
          ...
          <t-col :xs="12" :xl="6">
            <div class="item">
              <t-form-item :label="lang.name" name="client_id">
                <t-select v-model="formData.client_id" filterable :placeholder="lang.example" clearable
                  :loading="searchLoading" reserve-keyword :on-search="remoteMethod" clearable @clear="clearKey"
                  :show-arrow="false" :scroll="{ type: 'virtual',threshold:20 }" :popup-props="popupProps"
                  class="user-select">
                  <t-option v-for="item in userList" :value="item.id" :label="item.username" :key="item.id"
                    class="com-custom">
                    <div>
                      <p>{{item.username}}</p>
            ...
            <t-col :xs="12" :xl="6">
              <t-card bordered shadow>
                ...
                <t-form-item :label="lang.price" name="amount">
                  <t-input :placeholder="lang.input+lang.price" v-model="formData.amount"
          ...
          <t-row :gutter="{ xs: 8, sm: 16, md: 24, lg: 32, xl: 32, xxl: 90 }" v-if="formData.type==='new'">
            <t-col :xs="12" :xl="6" class="left-area">
              <div class="pro-item" v-for="(item,index) in formData.products" :key="item.key">
                ...
                <t-form-item :label="lang.choose_shop" class="price">
                  ...
                  <t-select v-model="item.product_name" :placeholder="lang.select+lang.product"
                    class="tree-select" ref="reference">
            ...
            <t-col :xs="12" :xl="6" class="config-show">
              <t-card bordered shadow>
                ...
                <div class="total">
                  <p>{{lang.total}}</p>
                  <p>{{currency_prefix}}<span
                      class="total-price">{{totalPrice.toFixed(2)}}</span>{{currency_code}}</p>
                </div>
        ...
        <!-- 升降级订单 -->
        <div class="new-order" v-if="formData.type==='upgrade'">
                ...
                <t-form-item :label="lang.choose_product" class="price">
                  <t-select v-model="shopId" :placeholder="lang.select+lang.tailorism" @change="chooseActive">
                    <t-option v-for="item in clientShopList" :value="item.id"
                      :label="item.product_name+'('+item.name+')'" :key="item.id"></t-option>
                  </t-select>
                </t-form-item>
                <!-- 升级至的商品选择 -->
                <t-form-item :label="lang.upgrade_to" class="price">
                  <t-select v-model="formData.product.product_name" :placeholder="lang.select+lang.product"
                    @change="chooseUpgrade">
          ...
          <t-row :gutter="{ xs: 8, sm: 16, md: 24, lg: 32, xl: 32, xxl: 90 }">
            <t-col :xs="12" :xl="6">
              <div class="pro-item">
                <!-- 选择已开通产品 -->
                <t-form-item :label="lang.choose_product" class="price">
                  <t-select v-model="renewIds" :placeholder="lang.select+lang.tailorism" 
                  multiple @change="chooseRenew" :min-collapsed-num="2">
                    <t-option v-for="item in clientShopList" :value="item.id"
