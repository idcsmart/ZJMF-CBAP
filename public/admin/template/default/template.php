...
<div id="content" class="template feedback" v-cloak>
  <t-card class="list-card-container">
    <ul class="common-tab">
      ...
      <li>
        <a href="info_config.html">{{lang.info_config}}</a>
      </li>
    </ul>
    <div class="box">
      <div class="com-top">
      ...
      <t-pagination :total="total" v-if="total" :page-size="params.limit" :page-size-options="pageSizeOptions" :on-change="changePage" :current="params.page" />
    </div>
  ...
  <t-dialog :header="`${lang.type_manage}`" :visible.sync="classModel" :footer="false" width="600" :close-on-overlay-click="false" class="class-dialog">
    <t-form :data="classParams" ref="classForm" @submit="submitSystemGroup" :rules="typeRules" class="cycle-form" v-if="classModel">
      <t-form-item :label="lang.feedback_type" name="name">
        <t-input v-model="classParams.name" :placeholder="`${lang.input}${lang.feedback_type}`"></t-input>
      </t-form-item>
      <t-form-item :label="`${lang.type}${lang.description}`" name="description">
        <t-input v-model="classParams.description" :placeholder="`${lang.input}${lang.type}${lang.description}`"></t-input>
