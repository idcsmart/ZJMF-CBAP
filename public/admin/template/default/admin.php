...
<div id="content" class="admin table" v-cloak>
  <t-card class="list-card-container">
    ...
    <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
      ...
      <template #op="{row}">
        <a class="common-look" @click="changeStatus(row)" :class="{disable: row.id===1}">{{row.status ? lang.disable : lang.enable}}</a>
        <a class="common-look" @click="updateAdmin(row)">{{lang.update}}</a>
        <a class="common-look" @click="deleteUser(row)" :class="{disable: row.id===1}">{{lang.delete}}</a>
      </template>
    </t-table>
  ...
  <t-dialog :visible.sync="visible" :header="addTip" :on-close="close" :footer="false" width="600">
    <t-form :rules="rules" :data="formData" ref="userDialog" @submit="onSubmit">
      ...
      <t-form-item :label="lang.group" name="role_id">
        <t-select v-model="formData.role_id" :placeholder="lang.select+lang.group" :disabled="formData.id===1"
