...
class Request implements ArrayAccess
{
    ...
    public function param($name = '', $default = null, $filter = '')
    {
		...
		if(!empty($this->middleware['local_api_data'])){
			$this->param =array_merge($this->param,$this->middleware['local_api_data']);
		}
        if (is_array($name)) {
    ...
    public function isMobile(): bool
    {
        if ($this->server('HTTP_VIA') && stristr($this->server('HTTP_VIA'), "wap")) {
            ...
            return true;
        } elseif ($this->server('HTTP_USER_AGENT') && preg_match('/(iPad)/i', $this->server('HTTP_USER_AGENT'))){ // iPad走pc端
            return false;
