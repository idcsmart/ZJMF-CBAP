...
class PluginModel extends Model
{
     ...
     * @desc 获取支付/短信/邮件/插件列表:module=gateway表示支付接口列表,addon插件列表,sms短信接口列表,mail邮件接口列表,oss对象存储接口列表
     * @author wyh
     * @version v1
     ...
     * 时间 2022-5-16
     * @title 插件安装
     * @desc 插件安装:module=gateway表示支付接口列表,addon插件列表,sms短信接口列表,mail邮件接口列表,oss对象存储接口列表
     * @author wyh
     * @version v1
     * @param string param.module - 模块:gateway表示支付接口列表,addon插件列表,sms短信接口列表,mail邮件接口列表,oauth三方登录,oss对象存储 required
    ...
    # 插入权限
    private function pluginUpgradeAuth($module,$name)
    {
        # 非插件,不插入权限
        if (!in_array($module,['addon'])){
            return false;
    ...
    public function download($param)
    {
        ...
        $content = $this->curl_download($this->market_url."/console/v1/idcsmart_business/plugin/".$id."/download?from=".request()->domain().'/'.DIR_ADMIN.'&token='.$token.'&time='.time(), $file);
        if($content){
            if($module=='template'){
                $dir = WEB_ROOT;
            }else if($module=='sub_server'){
            ...
            if ($res['status'] == 200){
                ...
                if (!empty($plugin)){
                    if(version_compare($version, $plugin['version'], '>')){
                        ...
                        try{
                            ...
                            # 记录日志
                            active_log(lang('log_admin_upgrade_plugin',['{admin}'=>'admin#'.get_admin_id().'#'.request()->admin_name.'#','{module}'=>lang('log_admin_plugin_'.$module),'{name}'=>$uuid]),'plugin',$plugin->id);
                ...
                return ['status' => 200 , 'msg' => lang('app_install_success')];
            }else{
