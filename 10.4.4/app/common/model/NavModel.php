...
class NavModel extends Model
{
    ...
    public function createPluginNav($nav,$module,$name,$type='admin')
    {
        ...
        $object = $this->create([
            'type' => $type,
            'name' => $nav['name']??'',
            'url'  => (isset($nav['url']) && !empty($nav['url']) && is_string($nav['url']))? (strpos($nav['url'], $viewSuffix) === false ? "plugin/{$url}/".$nav['url'].$viewSuffix : "plugin/{$url}/".$nav['url']):'',
            'icon' => $nav['icon']??'',
            'parent_id' => $parentId,
            'order'  => $maxOrder+1,
            'module' => $module,
        ...
        if(!empty($parentId)){
            $parent = $MenuModel->where('nav_id', $parentId)->find();
        }
        $maxOrder = $MenuModel->max('order');
        $MenuModel->create([
            'type' => $type,
            'menu_type' => 'plugin',
            'name' => lang_plugins($object['name']),
