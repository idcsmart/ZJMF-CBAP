...
class ModuleLogic
{
	 ...
	 */
	public function cartCalculatePrice(ProductModel $ProductModel, $params = [], $qty=1, $scene = 'buy', $position = 0)
	{
		$result = [];
		...
		if($ImportModule = $this->importModule($module)){
			if(method_exists($ImportModule, 'cartCalculatePrice')){
				// 获取模块通用参数
				$result = call_user_func([$ImportModule, 'cartCalculatePrice'], ['product'=>$ProductModel, 'custom'=>$params, 'qty'=>$qty, 'scene'=>$scene, 'position'=>$position]);
