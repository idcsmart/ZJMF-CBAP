...
<div id="content" class="order-details hasCrumb" v-cloak>
  <com-config>
    ...
    <t-card class="list-card-container">
      <ul class="common-tab">
        <li class="active" v-if="$checkPermission('auth_business_order_detail_order_detail_view') || $checkPermission('auth_user_detail_order_check_order')">
        ...
        <div class="left-box">
          ...
          <div class="item">
            ...
            <span class="btn" @click="changeCredit('add')" v-if="(orderDetail.amount * 1 !== orderDetail.credit * 1) && orderDetail.apply_credit_amount * 1 > 0
              && $checkPermission('auth_business_order_detail_order_detail_apply_credit') && orderDetail.is_recycle === 0">{{lang.app}}{{lang.credit}}</span>
            <span class="btn" @click="changeCredit('sub')" v-if="orderDetail.credit * 1 !== 0 && $checkPermission('auth_business_order_detail_order_detail_remove_credit') && orderDetail.is_recycle === 0">{{lang.deduct}}{{lang.credit}}</span>
          </div>
          <div class="item">
            <span class="txt">{{lang.refunded}}：</span>
            <span class="info">{{currency_prefix}}&nbsp;{{orderDetail.refund_amount}}</span>
            <span class="btn" @click="changeLog" v-permission="'auth_business_order_detail_order_detail_change_log'">{{lang.change_log}}</span>
          </div>
        ...
        <div class="r-box">
          <div class="con">
            ...
            <template v-if="orderDetail.status === 'Unpaid'">
              ...
              <P class="signPay" @click="signPay" v-if="$checkPermission('auth_business_order_detail_order_detail_paid') && orderDetail.is_recycle === 0">
                {{lang.sign_pay}}
              </P>
            </template>
            <p class="time">
            ...
            <p class="gateway">
              ...
              <template v-else>
                ...
                <!-- 混合支付 -->
                <template v-if="orderDetail.credit * 1 >0 && orderDetail.credit * 1 < orderDetail.amount * 1">
                ...
                <template v-if="orderDetail.amount*1 != 0 && orderDetail.credit==orderDetail.amount">
      ...
      <t-table row-key="id" :data="orderDetail.items" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" :hide-sort-tips="true">
        ...
        <template #op="{row, rowIndex}">
          <template v-if="orderDetail.status === 'Unpaid' && orderDetail.is_recycle === 0">
            <t-tooltip :content="lang.delete" :show-arrow="false" theme="light" v-if="row.edit && $checkPermission('auth_business_order_detail_order_detail_delete_order_item')">
              <t-icon name="delete" size="18px" @click="delteFlow(row, rowIndex)" class="common-look"></t-icon>
            </t-tooltip>
            <t-tooltip :content="lang.hold" :show-arrow="false" theme="light" v-if="row.edit && $checkPermission('auth_business_order_detail_order_detail_save_order_item')">
              <t-icon name="save" size="18px" @click="saveFlow(row)" class="common-look"></t-icon>
            </t-tooltip>
            <t-tooltip :content="lang.add" :show-arrow="false" theme="light" v-if="rowIndex === orderDetail.items.length -1 && $checkPermission('auth_business_order_detail_order_detail_create_order_item')">
      ...
      <!-- 应用/扣除余额 -->
      <t-dialog :visible.sync="visible" :header="title" :on-close="close" :footer="false" width="600" :close-on-overlay-click="false" class="apply-money">
      ...
      <t-dialog :visible="visibleLog" :header="lang.change_log" :footer="false" :on-close="closeLog" width="1000">
        <div slot="body">
          ...
          <t-pagination show-jumper v-if="logCunt" :total="logCunt" :page-size="moneyPage.limit" :page-size-options="pageSizeOptions" :on-change="changePage" />
        </div>
      </t-dialog>
    </t-card>
