  ...
  <com-config>
    ...
    <t-card class="list-card-container">
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #name="{row}">
          {{row.name}}
          <span v-if="row.ip_num > 1 && $checkPermission('auth_user_detail_host_info_host_detail')" class="showIp"
            @click="showIp(row.id)">({{row.ip_num}})</span>
          <span v-if="row.ip_num > 1 && !$checkPermission('auth_user_detail_host_info_host_detail')"
