...
<div id="content" class="host" v-cloak>
  <com-config>
    <t-card class="list-card-container">
      ...
      <div class="common-header">
        ...
        <div class="right-search">
          <div class="flex" v-show="!isAdvance">
            ...
            <com-tree-select v-show="searchType === 'product_id'" :value="params.product_id"
              @choosepro="choosePro"></com-tree-select>
            <t-button @click="seacrh">{{lang.query}}</t-button>
          </div>
          <t-button @click="changeAdvance"
            style="margin-left: 20px;">{{isAdvance ? lang.pack_up : lang.advanced_filter}}</t-button>
        </div>
      </div>
      <div class="advanced" v-show="isAdvance">
        <div class="search">
          ...
          <t-input v-model="params.keywords" class="search-input" :placeholder="lang.input" @keyup.native.enter="seacrh"
            clearable v-show="searchType !== 'product_id'">
          </t-input>
          ...
          <t-input v-model="params.username" class="search-input" :placeholder="`${lang.input}${lang.username}`"
            @keyup.native.enter="seacrh" clearable>
          </t-input>
          <t-input v-model="params.billing_cycle" class="search-input"
            :placeholder="`${lang.input}${lang.payment_cycle}`" @keyup.native.enter="seacrh" clearable>
          </t-input>
          <t-select v-model="params.status" :placeholder="lang.client_care_label29" clearable
            v-show="params.tab === 'using' || params.tab === 'overdue' || params.tab === ''">
            <t-option v-for="item in calcStatus(productStatus)" :value="item.value" :label="item.label"
              :key="item.value">
            </t-option>
          </t-select>
          ...
          <t-date-range-picker allow-input clearable v-model="range"
            :placeholder="[`${lang.due_time}`,`${lang.due_time}`]"></t-date-range-picker>
          <t-button @click="seacrh">{{lang.query}}</t-button>
        </div>
      </div>
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #product_name="{row}">
          <span class="aHover" @click="goHostDetail(row)"
            v-if="$checkPermission('auth_business_host_check_host_detail')">{{row.product_name}}</span>
        ...
        <template #name="{row}">
          ...
          <span v-if="row.ip_num > 1 && $checkPermission('auth_business_host_check_host_detail')" class="showIp"
            @click="showIp(row.id)">({{row.ip_num}})</span>
          <span v-if="row.ip_num > 1 && !$checkPermission('auth_business_host_check_host_detail')" class="showIp"
            style="cursor: inherit;">({{row.ip_num}})</span>
        </template>
        <template #id="{row}">
          <span class="aHover" @click="goHostDetail(row)"
            v-if="$checkPermission('auth_business_host_check_host_detail')">{{row.id}}</span>
    ...
    <!-- 所有IP -->
    <t-dialog :header="lang.finance_search_text23" @close="ipLoading = false" :footer="false" :visible.sync="ipLoading"
      class="ip-dialog">
      <div class="ips">
