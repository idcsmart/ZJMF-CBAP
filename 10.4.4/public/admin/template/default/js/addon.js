  ...
  window.onload = function () {
    ...
    new Vue({
      ...
      methods: {
            ...
            if (!this.pagination.keywords && this.pagination.status !== "") {
              ...
              temp = temp.filter((item) => {
                if (
                  ...
                  (this.typeObj[item.type] || lang.plugin).indexOf(
        ...
        async handlerAllUpgrade() {
          if (this.isAllLoading) {
            return;
          }
          this.isAllLoading = true;
          const allUpgrade = [];
          this.syncPluginList.forEach((item) => {
            if (item.downloaded * 1 === 1 && item.upgrade * 1 === 1 && !item.error_msg) {
              item.isLoading = true;
            ...
            if (arr.length > 0) {
              this.filterPluginList = this.data = this.data.map((item) => {
                ...
                if (arr.includes(item.name)) {
                  ...
                  item.error_msg = temp.filter(el => el.uuid === item.name)[0]?.error_msg
                }
                return item;
              });
            }
