      ...
      methods: {
        ...
        async onSubmit ({ validateResult, firstError }) {
          if (validateResult === true) {
            try {
              ...
              params.auth = params.auth.filter(item => typeof item === 'number')
              params.auth_widget = _temp.filter(item => typeof item === 'string')
        ...
        clickNode (e) {
         ...
         clickNodeWidget (e) {
          if (!e.node.expanded) {
            ...
            this.widgetExpandArr.splice(this.widgetExpandArr.indexOf(e.node.value), 1)
          }
        },
        // 删除分组
        deleteUser (row) {
        ...
        async getAuthList () {
          try {
            ...
            const temp = [
              {id: 'all', title: lang.home_widget, child: res.data.data.widget}
            ]
