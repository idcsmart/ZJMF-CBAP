  ...
  window.onload = function () {
    ...
    new Vue({
      ...
      computed: {
        calcDcimStatus () {
          return (status) => {
            return this.statusArr.filter(item => item.value === status)[0]?.label
          }
        },
      ...
      methods: {
        ...
        optItem(row, type) {
          ...
          if (type === "allot") {
            this.resourceTitle = this.distributeType === 'manual' ? lang.sure_allot_resource : lang.sure_allot_server
          } else {
            this.resourceTitle = this.distributeType === 'manual' ? lang.sure_free_resource : lang.sure_free_server;
          }
        ...
        jumpToTicket() {
          if (this.hasNewTicket) {
            return  location.href = str + `plugin/ticket_premium/client_ticket.htm?id=${this.client_id}&host_id=${this.id}`;
          }
          if (!this.hasNewTicket && this.hasTicket) {
            return  location.href = str + `plugin/idcsmart_ticket/client_ticket.htm?id=${this.client_id}&host_id=${this.id}`;
          }
            ...
            .then(async (res) => {
              ...
              try {
                ...
                tempArr.forEach((item) => {
                  if (this.hasDcimModule) { // dicm 模块只传  disable 不存在的参数
                    if (!item.disable) {
