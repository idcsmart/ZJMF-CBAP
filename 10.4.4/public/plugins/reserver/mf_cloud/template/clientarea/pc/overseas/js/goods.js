...
new Vue({
  ...
  mounted() {
    ...
    el &&
      el.addEventListener("scroll", (e) => {
        if (e.target.scrollTop > clientH / 3) {
          this.isShowTop = true;
        } else {
          this.isShowTop = false;
        }
        if (e.target.scrollTop > 10) {
          this.hasTopScroll = true;
        } else {
          this.hasTopScroll = false;
        }
      });
  ...
  computed: {
    root_name() {
      return (this.imageName.toUpperCase() || "").indexOf("WIN") !== -1
        ? "administrator"
        : "root";
    },
    calcMax() {
  ...
  methods: {
    ...
    changeDataDisk(e, index) {
      // 分单选和范围
      this.storeList[index].disk_type = e;
