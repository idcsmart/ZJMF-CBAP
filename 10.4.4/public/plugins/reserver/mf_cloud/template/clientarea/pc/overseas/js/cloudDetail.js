...
new Vue({
    ...
    return {
      ...
      powerStatusArr: {
        on: {
          ...
          icon: `${url}/img/cloud/on.svg`,
        },
        off: {
          text: lang.common_cloud_text11,
          icon: `${url}/img/cloud/off.svg`,
        },
        operating: {
          text: lang.common_cloud_text12,
          icon: `${url}/img/cloud/operating.svg`,
        },
        fault: {
          text: lang.common_cloud_text86,
          icon: `${url}/img/cloud/fault.svg`,
        },
        suspend: {
          text: lang.common_cloud_text87,
          icon: `${url}/img/cloud/suspended.svg`,
        },
      },
  ...
  methods: {
    ...
    changeManageTab() {
      ...
      switch (this.manageActive) {
        ...
        case "reinstall":
          ...
          this.osIcon =
            "/plugins/reserver/mf_cloud/template/clientarea/pc/overseas/img/mf_cloud/" +
            this.osData[0].name +
            ".svg";
    ...
    getImage() {
      ...
      image(params).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osSelectData = this.osData[0].image;
          this.reinstallData.osGroupId = this.osData[0].id;
          this.osIcon = `/plugins/reserver/mf_cloud/template/clientarea/pc/overseas/img/mf_cloud/${this.osData[0].name}.svg`;
          this.reinstallData.osId = this.osData[0].image[0].id;
          this.doCheckImage();
        }
      });
    ...
    osSelectGroupChange(e) {
      this.osData.map((item) => {
        if (item.id == e) {
          ...
          this.osIcon = `/plugins/reserver/mf_cloud/template/clientarea/pc/overseas/img/mf_cloud/${item.name}.svg`;
    ...
    paySuccess(e) {
      ...
      // 升降级
      if (this.activeName === "2" && this.manageActive === "relegation") {
        this.showUpgrade();
      }
    },
    // 取消支付回调
      ...
      try {
        ...
        if (newNum + oldNum < max) {
          ...
          if (this.dataDiskList.length !== 0) {
            ...
            itemData.disk_type =
              this.dataDiskList[0].other_config.data_disk_type;
    ...
    // 变化监听
    sliderChange(val, item, o) {
      const arr = [];
      item.selectList.forEach((i) => {
        arr.push([i.min_value, i.max_value]);
      });
      item.size = this.mapToRange(val, arr, item.min_value, val > o);
    },
    changeDataNum(val, item, o) {
      this.$nextTick(() => {
        // 数据盘数量改变计算价格
    ...
    subUnmount() {
      try {
        if (!this.del_checked) {
          return (this.errText = lang.domain_check);
        }
        this.mountLoading = true;
