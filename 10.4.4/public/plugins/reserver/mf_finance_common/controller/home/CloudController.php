...
class CloudController
{
    ...
    public function content()
    {
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                ...
                $result = $RouteLogic->curl('provision/custom/content?jwt='.$login['data']['jwt'], $data, 'GET');
            }
        }catch(\Exception $e){
            return json(['status'=>400, 'msg'=>lang_plugins('res_mf_finance_common_act_exception')]);
        }
    ...
    public function off()
    {
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                // 上游商品
                $result = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/off", [
                    'os' => $param['os']??'',
                    'code' => $param['code']??'',
                ], 'POST');
            }else{
    ...
    public function reboot()
    {
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                // 上游商品
                $result = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/reboot", [
                    'os' => $param['os']??'',
                    'code' => $param['code']??'',
                ], 'POST');
            }else{
    ...
    public function vnc()
    {
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                // 上游商品
                $result = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/vnc", [
                ], 'POST');
            }else{
                $postData = [
                    'id'        => $RouteLogic->upstream_host_id,
    ...
    public function status()
    {
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                // 上游商品
                $result = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/status", [
                ], 'GET');
            }else{
                $postData = [
                    'id'        => $RouteLogic->upstream_host_id,
     ...
     * @title 重置密码
     * @desc 重置密码
     * @url /console/v1/remf_finance_common/:id/reset_password
     * @method  POST
     * @author hh
     * @version v1
     * @param   int id - 产品ID require
     * @param   string password - 新密码 require
     * @param   string force - 是否强制关机，on是 require
     */
     ...
     * @title 救援模式
     * @desc 救援模式
     * @url /console/v1/remf_finance_common/:id/rescue
     * @method  POST
     * @author hh
     * @version v1
     * @param   int id - 产品ID require
    ...
    public function rescue()
    {
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                // 上游商品
                $result = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/rescue", [
                    'system'    => $param['system'] == 2 ? 1 : 2,
                ], 'POST');
            }else{
                $postData = [
                    'id'        => $RouteLogic->upstream_host_id,
            ...
            if($result['status'] == 200){
                $description = lang_plugins('res_mf_finance_common_log_host_start_rescue_success', [
                    '{hostname}' => $HostModel['name'],
                ]);
            }else{
                $description = lang_plugins('res_mf_finance_common_log_host_start_rescue_fail', [
                    '{hostname}' => $HostModel['name'],
                ]);
            }
            active_log($description, 'host', $HostModel['id']);
    ...
    public function syncUpgradeConfigPrice(){
        ...
        try{
            ...
            if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
                // 上游商品
                $result = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/sync_upgrade_config_price", [
     ...
     * @url /console/v1/remf_finance_common/:id/upgrade_config_page
     * @method  GET
     * @author wyh
     * @version v1
     * @param int id - 产品ID require
    ...
    public function upgradeConfigPostPage()
    {
        ...
        if (!empty($supplier) && $supplier['type']=='default' && $upstreamProduct['res_module']=='mf_finance_common'){
            // 上游商品
            $res = $RouteLogic->curl( "console/v1/remf_finance_common/{$upstreamHost['upstream_host_id']}/upgrade_config_page", [
            ], 'GET');
        }else{
