...
function getCommonDetail (id) {
  return Axios.get(`/product/${id}/remf_finance_common/order_page`)
}
// 商品详情
...
function getCommonList (params) {
  return Axios.get(`/remf_finance_common/host`, { params });
}
// 产品列表
function getCommonListDetail (id) {
  return Axios.get(`/remf_finance_common/host/${id}/configoption`);
}
...
function calculate (id) {
  return Axios.get(`/product/${id}/remf_finance_common/duration`);
}
// 获取商品折扣金额
