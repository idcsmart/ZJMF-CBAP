...
new Vue({
  ...
  computed: {
    ...
    calcImageList() {
      ...
      /* 限制只针对自定义，不支持套餐 */
      if (this.activeName === "custom" && this.limitNum) {
        let tempLimit = this.configLimitList.filter(
          (item) => item.type === "image"
        );
        if (tempLimit.length > 0) {
          // cpu限制数组
          const cpuArr = tempLimit.reduce((all, cur) => {
            all.push(...cur.cpu.split(","));
            return all;
          }, []);
          // 内存限制数组， 根据当前的cpu 筛选对应的镜像限制规则
          let memoryArr = [];
          let filterTempLimit = tempLimit.filter((item) => {
            return item.cpu == this.params.cpu;
          });
          if (filterTempLimit[0]?.memory) {
            // 单选
            memoryArr = filterTempLimit.reduce((all, cur) => {
              all.push(...(cur.memory * 1));
              return all;
            }, []);
          } else {
            memoryArr = filterTempLimit.reduce((all, cur) => {
              all.push(...this.createArr([cur.min_memory, cur.max_memory]));
              return all;
            }, []);
          }
          console.log("@@@limit", memoryArr, "----", this.params.cpu);
          /** 根据后台限制处理镜像 **/
          const imageGroup = tempLimit.map((item) => item.image_group_id);
          // 其他限制里面的可选内存交集
          // memoryArr = Array.from(new Set(memoryArr.concat(this.calaMemoryList)))
          if (
            cpuArr.includes(this.params.cpu) &&
            memoryArr.includes(this.params.memory)
          ) {
            temp =
              temp
                .map((item) => {
                  if (imageGroup.includes(item.id)) {
                    const imageId = tempLimit
                      .filter((ele) => ele.image_group_id === item.id)
                      .map((item) => item.image_id);
                    item.image = item.image.filter(
                      (el) => !imageId.includes(el.id)
                    );
                  }
                  return item;
                })
            ...
            const curImageGroup =
              temp.filter((item) => item.image_group_id === this.curImageId) ||
              [];

            let targetImage = {};
            // 重置镜像 1: 当前镜像分组无可选
            if (curImageGroup.length === 0) {
              targetImage = temp[0];
            } else {
              targetImage =
                temp.find((item) => item.id === this.curImageId) || [];
              //  this.curImage = temp.findIndex(item => item.id === this.curImageId);
            }
            ...
            // }
            this.curImage = "";
            this.imageName = targetImage.name;
            this.curImageId = targetImage.id;
            this.version = targetImage.image[0]?.name;
            this.params.image_id = targetImage.image[0]?.id;
            this.isManual = false;
          } else {
            temp = JSON.parse(JSON.stringify(this.imageList));
          }
        }
      }
