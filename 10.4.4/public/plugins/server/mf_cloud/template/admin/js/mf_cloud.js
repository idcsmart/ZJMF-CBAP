...
new Vue({
  ...
  data() {
    return {
      ...
      dataRules: {
        ...
        gpu_name: [
          { required: true, message: `${lang.input}GPU${lang.box_title46}`, type: "error" },
        ],
        memory: [
          {
            required: true,
  ...
  computed: {
    ...
    calcSystem () {
      return this.systemList.filter(item => item.image_group_id === this.lineForm.image_group_id)
    },
    calcLoading () {
      return name => {
    ...
    addLineSub(type) {
      ...
      if ( this.lineType === "update" && type === "line_gpu") {
        if (!this.lineForm.gpu_name) {
          return this.$message.warning(`${lang.input}GPU${lang.box_title46}${lang.nickname}`)
        }
      }
      if (type === "line_bw") {
