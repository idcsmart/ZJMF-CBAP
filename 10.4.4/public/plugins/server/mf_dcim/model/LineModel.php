...
class LineModel extends Model
{
        ...
        try{
            ...
            if(isset($param['ip_data']) && is_array($param['ip_data'])){
                foreach($param['ip_data'] as $v){
                    ...
                    $option = OptionModel::create($v, ['product_id','rel_type','rel_id','type','value','other_config','create_time','value_show']);
    ...
    public function homeLineConfig($id)
    {
        ...
        $ip = OptionModel::field('value,value_show')->where('rel_type', OptionModel::LINE_IP)->where('rel_id', $id)->select()->toArray();
        // 排序
        foreach($ip as $k=>$v){
            $num = $v['value'];
