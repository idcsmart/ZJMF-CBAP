...
class ResModuleLogic
{
	...
	public function cartCalculatePrice($ProductModel, $params = [], $qty=1, $scene="cal_price",$create=false)
	{
        ...
        if($supplier['type']=='whmcs'){
	        ...
	        if ($res['status']==200){
	            ...
	            if ($this->upstreamProduct['profit_type']==1){
                    ...
                    $res['data']['base_price'] = bcadd($res['data']['base_price']??0,0/*$this->upstreamProduct['profit_percent']*/,2);
                }else{
                ...
                foreach($res['data']['preview'] as $k1=>$v1){
                    if ($v1['price']>0){
                        $hasPrice++;
                    }
                }
	            ...
	            foreach($res['data']['preview'] as $k=>$v){
                    ...
                    // wyh 20240122 固定利润显示问题
                    if ($this->upstreamProduct['profit_type']==1){
                        if ($hasPrice>0){
                            $averagePrice = bcdiv($this->upstreamProduct['profit_percent'],$hasPrice,2);
                            $v['price'] = bcadd($v['price'],$averagePrice,2);
                            $res['data']['preview'][$k]['price'] = $v['price'];
                        }else{
                            // 金额都为0时，将固定利润金额加给第一个配置
                            /*if ($k==0){
                                $v['price'] = bcadd($v['price'],$this->upstreamProduct['profit_percent'],2);
                                $res['data']['preview'][$k]['price'] = $v['price'];
                            }*/
                    ...
                    if ($hasPrice==0 && $this->upstreamProduct['profit_type']==1){
        ...
        elseif ($supplier['type']=='finance'){
            ...
            if ($res['status']==200){
                ...
                if ($this->upstreamProduct['profit_type']==1){
                    ...
                    // wyh 20240202 金额显示问题
                    $res['data']['base_price'] = bcadd($res['products']['product_sale_setup_fee']+$res['products']['product_sale_price'],0/*$this->upstreamProduct['profit_percent']*/,2);
                ...
                foreach ($alloption as $k1=>$v1){
                    if (isset($v1['suboption_sale_price_total']) && $v1['suboption_sale_price_total']>0){
                        $hasPrice++;
                    }
                }
                ...
                foreach ($alloption as $k=>$v){
                    ...
                    if ($price>0){
                        ...
                        if ($this->upstreamProduct['profit_type']==1){
                            if ($hasPrice>0){
                                $averagePrice = bcdiv($this->upstreamProduct['profit_percent'],$hasPrice,2);
                                $price = bcadd($price,$averagePrice,2);
                            }else{
                                // 金额都为0时，将固定利润金额加给第一个配置
                                /*if ($k==0){
                                    $price = bcadd($v['price']??0,$this->upstreamProduct['profit_percent'],2);
                                }*/
                            }
                        }
                    }
                    ...
                    if (!empty($plugin)){
                        ...
                        $clientLevelDiscountPreview = $IdcsmartClientLevelModel->productDiscount([
                            'id' => $ProductModel['id'],
                            'amount' => $price
                        ]);
                    }
                    ...
                    $preview[] = [
                        ...
                        'price' => $price,
                        'client_level_discount_preview' => $clientLevelDiscountPreview??0
                    ];
                ...
                if (!empty($plugin)){
                    ...
                    $clientLevelDiscountRenewPrice = $IdcsmartClientLevelModel->productDiscount([
                        'id' => $ProductModel['id'],
                    ...
                    $res['data']['base_price_discount'] = $basePriceDiscount??0;
                    // 二级代理及以下给下游的客户等级折扣数据
                    $res['data']['client_level_discount'] = $clientLevelDiscount??0;
                    $res['data']['client_level_discount_renew_price'] = $clientLevelDiscountRenewPrice??0;
                }
            }
        }
        else{
	        ...
	        if ($res['status']==200){
                ...
                // 多级代理价格问题
                if (isset($res['data']['client_level_discount'])){
                    $res['data']['price'] = bcsub($res['data']['price'],$res['data']['client_level_discount'],2)>0?bcsub($res['data']['price'],$res['data']['client_level_discount'],2):0;
                }
                if (isset($res['data']['client_level_discount_renew_price'])){
                    $res['data']['renew_price'] = bcsub($res['data']['renew_price'],$res['data']['client_level_discount_renew_price'],2)>0?bcsub($res['data']['renew_price'],$res['data']['client_level_discount_renew_price'],2):0;
                }
                if (isset($res['data']['base_price_discount'])){
                    $res['data']['base_price'] = bcsub($res['data']['base_price'],$res['data']['base_price_discount'],2)>0?bcsub($res['data']['base_price'],$res['data']['base_price_discount'],2):0;
                }
                ...
                if ($this->upstreamProduct['profit_type']==1){
                    $res['data']['profit'] = bcadd(0, $this->upstreamProduct['profit_percent'], 2);
                    $res['data']['price'] = bcadd($res['data']['price_total'] ?? 0, $res['data']['profit'], 2);
                    $res['data']['renew_price'] = bcadd($res['data']['renew_price']??0,$this->upstreamProduct['profit_percent'],2);
                    $res['data']['base_price'] = bcadd($res['data']['base_price']??0,$this->upstreamProduct['profit_percent'],2);
                }else{
                    $res['data']['profit'] = bcmul($res['data']['price_total'] ?? 0, ($this->upstreamProduct['profit_percent']/100), 2);
                    $res['data']['price'] = bcadd($res['data']['price_total'] ?? 0, $res['data']['profit'], 2);
                    $res['data']['renew_price'] = bcmul($res['data']['renew_price']??0,(1+$this->upstreamProduct['profit_percent']/100),2);
                    $res['data']['base_price'] = bcmul($res['data']['base_price']??0,(1+$this->upstreamProduct['profit_percent']/100),2);
                }
	            ...
	            foreach($res['data']['preview'] as $k=>$v){
	            	if($v['price']>0){
	            	    if (isset($v['client_level_discount_preview'])){
	            	        $v['price'] = bcsub($v['price'],$v['client_level_discount_preview'],2)>0?bcsub($v['price'],$v['client_level_discount_preview'],2):0;
                        }
                        ...
                        if (!empty($plugin)){
                            ...
                            $res['data']['preview'][$k]['client_level_discount_preview'] = $clientLevelDiscountPreview??0;
                        }
	            	}
        ...
        // wyh 20240122 固定利润显示问题
        /*if (!empty($res['data']['preview'][0]) && $this->upstreamProduct['profit_type']==1){
            if ($hasPrice>0){
                $averagePrice = bcdiv($this->upstreamProduct['profit_percent'],$hasPrice,2);
                foreach ($res['data']['preview'] as $kk=>$vv){
                    if ($vv['price']>0){
                        $vv['price'] = bcadd($vv['price'],$averagePrice,2);
                        $res['data']['preview'][$kk]['price'] = $vv['price'];
                    }
                }
            }else{
                // 金额都为0时，将固定利润金额加给第一个配置
                if (isset($res['data']['preview'][0]['price'])){
                    $res['data']['preview'][0]['price'] = bcadd($res['data']['preview'][0]['price'],$this->upstreamProduct['profit_percent'],2);
                }
            }
        }*/
        return $res;
            ...
            if ($res['status']==200){
                ...
                foreach ($res['data']['host'] as $item){
                    ...
                    if ($this->upstreamProduct['profit_type']==1){
                        ...
                        $item['base_price'] = bcadd($item['price_save'], $item['profit'], 2);
                    }else{
                        $item['profit'] = bcmul($item['price'], bcadd($this->upstreamProduct['profit_percent']/100,0,2), 2);
                        $item['price'] = bcadd($item['price'], $item['profit'], 2);

                        $item['base_price'] = bcmul($item['price_save'], bcadd($this->upstreamProduct['profit_percent']/100,1,2), 2);
