...
class PluginModel extends Model
{
     ...
     * @author wyh
     * @version v1
     * @param string param.module - 模块:gateway表示支付接口列表,addon插件列表,sms短信接口列表,mail邮件接口列表,oauth三方登录 required
            ...
            foreach ($list as $key => $value) {
                if(!isset($apps[$value['type']])){
                    if($value['type']=='server'){
                        ...
                        $apps[$value['type']] = array_column($moduleList, 'name');
                        foreach ($apps[$value['type']] as $k => $v){
                            $apps[$value['type']][$k] = parse_name($v, 1); 
                        }
                    }else{
