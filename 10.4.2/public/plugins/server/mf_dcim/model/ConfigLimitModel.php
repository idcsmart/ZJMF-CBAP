...
class ConfigLimitModel extends Model
{
     ...
     * @param   array param.model_config_id - 型号配置ID require
     * @param   int param.line_id - 线路ID
     * @param   string param.min_bw - 带宽最小值 带宽线路可传
     * @param   string param.max_bw - 带宽最大值 带宽线路可传
     * @param   string param.min_flow - 流量最小值 流量线路可传
     * @param   string param.max_flow - 流量最大值 流量线路可传
    ...
    public function configLimitList($param)
    {
        ...
        $list = $this
            ->alias('cl')
            ->field('cl.id,cl.data_center_id,cl.line_id,cl.min_bw,cl.max_bw,cl.min_flow,cl.max_flow,cl.model_config_id,l.name line_name,l.bill_type,dc.country_id,dc.city,dc.area,c.iso,c.'.$countryName.' country_name')
            ->where($where)
