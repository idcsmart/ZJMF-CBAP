  ...
  methods: {
    async getPlugin () {
      try {
        ...
        const temp = res.data.data.list.filter(item => item.status === 1).reduce((all,cur) => {
          all.push(cur.name)
          return all
        },[])
    ...
    // 通用删除
    sureDelete() { // cycle, c_line, data, model, hard, limit, system, group, line_bw, line_flow, line_ip, line_defence
      switch (this.delType) {
        case "cycle":
