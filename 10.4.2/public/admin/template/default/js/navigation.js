    ...
    new Vue({
      ...
      methods: {
        ...
        itemClick (item) {
          ...
          item.second_reminder = (item.second_reminder === 1 || item.second_reminder === true) ? true : false
          // 判断是否有子导航
        ...
        confirmNewMenu () {
          ...
          let newPage = { ...JSON.parse(JSON.stringify(this.newFormData)), child: [] }
      ...
      created () {
        this.getAllIcon()
        if (!this.$checkPermission('auth_system_configuration_menu_home_menu_view')) {
          this.value = "2"
          this.getAdminMenu()
        } else {
          this.value = "1"
