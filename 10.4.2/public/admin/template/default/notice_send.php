...
<div id="content" class="notice-send table" v-cloak>
  <com-config>
    <t-card class="list-card-container">
        ...
        <div class="header-left">
          ...
          <t-form layout="inline" label-align="left" :data="formData" :rules="rules" ref="sendForm">
            <t-form-item :label="lang.sms_global_name" name="configuration.send_sms_global">
              <t-select v-model="formData.configuration.send_sms_global" class="demo-select-base" clearable :placeholder="lang.sms_global_name">
            ...
            <t-form-item :label="lang.home_sms_interface" name="configuration.send_sms">
              <t-select v-model="formData.configuration.send_sms" class="demo-select-base" clearable :placeholder="lang.home_sms_interface">
            ...
            <t-form-item :label="lang.email_interface" name="configuration.send_email">
              <t-select v-model="formData.configuration.send_email" class="demo-select-base" clearable :placeholder="lang.email_interface">
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'">
        ...
        <template #sms_global_template="{row,rowIndex}">
          <div :id="`sms_global_template${rowIndex}`" :class="{required: formData[row.name].sms_global_name && !formData[row.name].sms_global_template , none_pop: !formData[row.name].sms_global_name }">
            <t-select v-model="formData[row.name].sms_global_template" clearable :disabled="!row.sms_global_name" ref='sms_global_template' :popup-props="{ attach: `#sms_global_template${rowIndex}` }">
      ...
      <t-button @click="save" class="add" :loading="submitLoading" v-permission="'auth_system_interface_send_configuration_save_configuration'">{{lang.hold}}</t-button>
    </t-card>
  </com-config>
</div>
<!-- =======页面独有======= -->
