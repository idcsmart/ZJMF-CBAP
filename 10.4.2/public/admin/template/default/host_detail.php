      ...
      <div class="com-h-box">
        <ul class="common-tab">
          ...
          <li v-if="$checkPermission('auth_user_detail_notification_log_sms_notification') || $checkPermission('auth_user_detail_notification_log_email_notification')">
            <a :href="`${baseUrl}/${($checkPermission('auth_user_detail_notification_log_sms_notification') ? 'client_notice_sms' : 'client_notice_email')}.htm?id=${id}`">{{lang.notice_log}}</a>
          </li>
        ...
        <div class="user">
          <!-- 顶部右侧选择用户 -->
          <com-choose-user :cur-info="clientDetail" :check-id="client_id" @changeuser="changeUser" class="com-clinet-choose">
          </com-choose-user>
                ...
                <div class="left">
                  <div v-for="(item,index) in optBtns" :key="index">
                    <t-button @click="handlerMoudle(item.func)" v-if="$checkPermission(item.auth)">
                      <template v-if="$checkPermission(item.auth)">
    ...
    <t-dialog :header="distributeTitle" :visible.sync="resourceDialog" width="80%" :footer="false">
      <div class="left-search">
        ...
        <template v-if="distributeType === 'dcim'">
          <t-input v-model="dcimForm.ip" class="search-input" :placeholder="`${lang.input}IP`" @keyup.enter.native="getDcimList" :on-clear="clearIp" clearable></t-input>
      ...
      <t-table row-key="index" :data="distributeType === 'manual' ? resourceList : dcimList" :columns="distributeType === 'manual' ? resourcesColumns : dcimColumns" :loading="resourceLoading" :hover="true">
        <template #id="{row}">
