  ...
  <com-config>
    <t-card class="list-card-container">
      <div class="common-header">
        <div class="left">
          <t-button theme="default" @click="getMore" class="add" v-permission="'auth_system_interface_sms_notice_get_more_interfaces'">{{lang.get_more_interface}}</t-button>
        </div>
      </div>
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" @sort-change="sortChange" :hide-sort-tips="hideSortTips">
        ...
        <template #op="{row}">
          <t-tooltip :content="enableTitle(row.status)" :show-arrow="false" theme="light">
            <a class="common-look" @click="changeStatus(row)" v-if="row.status !== 3 && $checkPermission('auth_system_interface_sms_notice_deactivate_enable_interface')">
          ...
          <t-tooltip :content="lang.template_manage" :show-arrow="false" theme="light">
            <a class="common-look" @click="jump(row)" v-if="row.status!==3 && $checkPermission('auth_system_interface_sms_notice_sms_template_view')">
          ...
          <t-tooltip :content="lang.config" :show-arrow="false" theme="light">
            <a class="common-look" @click="handleConfig(row)" v-if="row.status!==3 && $checkPermission('auth_system_interface_sms_notice_interface_configuration')">
              <t-icon name="tools"></t-icon>
            </a>
          </t-tooltip>
          <t-tooltip :content="installTitle(row.status)" :show-arrow="false" theme="light">
            <a class="common-look" @click="installHandler(row)" v-permission="'auth_system_interface_sms_notice_install_uninstall_interface'">
