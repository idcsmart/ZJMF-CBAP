...
class OptionModel extends Model
{
     ...
     * @return  string duration[].price - 价格
     */
    public function lineGpuIndex($id)
    {
        $option = $this->optionIndex($id);
        if(empty($option) || $option['rel_type'] != OptionModel::LINE_GPU){
            return (object)[];
        }
        $data = [
            'id'        => $option['id'],
     ...
     * @param   int param.rel_type - 配置类型(0=CPU配置,1=内存配置,2=线路带宽配置,3=线路流量配置,4=线路防护配置,5=线路附加IP,6=系统盘,7=数据盘,8=线路显卡) require
     * @param   int param.rel_id 0 关联ID(rel_type=2,3,4,5,8时是线路ID)
    ...
    public function optionCreate($param)
    {
        if(in_array($param['rel_type'], [OptionModel::CPU,OptionModel::MEMORY,OptionModel::SYSTEM_DISK,OptionModel::DATA_DISK])){
            ...
            $param['rel_id'] = 0;
        }else if(in_array($param['rel_type'], [OptionModel::LINE_BW,OptionModel::LINE_FLOW,OptionModel::LINE_DEFENCE,OptionModel::LINE_IP,OptionModel::LINE_GPU])){
        ...
        try{
            ...
            }else if($param['rel_type'] == OptionModel::LINE_GPU){
     ...
     * @version v1
     * @param   int id - 配置ID require
     * @param   int rel_type - 配置类型(0=CPU配置,1=内存配置,2=线路带宽配置,3=线路流量配置,4=线路防护配置,5=线路附加IP,6=系统盘,7=数据盘,8=线路显卡) 传入了验证配置类型
     * @return  int status - 状态(200=成功,400=失败)
     * @return  string msg - 信息
     */
     ...
     * @return int rel_type - 配置类型(0=CPU配置,1=内存配置,2=线路带宽配置,3=线路流量配置,4=线路防护配置,5=线路附加IP,6=系统盘,7=数据盘,8=线路显卡)
     * @return int rel_id - 关联ID(rel_type=2,3,4,5,8时是线路ID)
     ...
     * @param   int  $relType   - 配置类型(0=CPU配置,1=内存配置,2=线路带宽配置,3=线路流量配置,4=线路防护配置,5=线路附加IP,6=系统盘,7=数据盘,8=线路显卡) require
     * @param   int  $relId     - 关联ID(rel_type=2,3,4,5,8时是线路ID) require
