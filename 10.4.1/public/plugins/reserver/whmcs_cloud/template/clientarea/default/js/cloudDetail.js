    ...
    getCloudDetail() {
      ...
      cloudDetail(params).then((res) => {
        if (res.data.status === 200) {
          ...
          // this.osIcon = "/plugins/reserver/whmcs_cloud/template/clientarea/default/img/rewhmcs_cloud/" + this.osData[0].name + '.svg'
    ...
    osSelectGroupChange(e) {
      this.osData.map((item) => {
        if (item.id == e) {
          ...
          this.osIcon =
            "/plugins/reserver/whmcs_cloud/template/clientarea/default/img/rewhmcs_cloud/" +
            item.name +
            ".svg";
          this.reinstallData.osId = this.osSelectData[0]?.id;
          // this.doCheckImage()
