...
class AppMarketController extends AdminBaseController
{
     ...
     * @return string list[].type - 应用分类addon插件,captcha验证码接口,certification实名接口,gateway支付接口,mail邮件接口,sms短信接口,server模块,template主题,oauth第三方登录,sub_server子模块,widget首页挂件 
	...
	public function getNewVersion(){
		...
		if(isset($result['status']) && $result['status']==200){
			...
			foreach ($result['data']['list'] as $key => $value) {
				...
				if(in_array($value['type'], ['server', 'sub_server', 'template'])){
     ...
     * @return string type - 应用分类addon插件,captcha验证码接口,certification实名接口,gateway支付接口,mail邮件接口,sms短信接口,server模块,template主题,oauth第三方登录,sub_server子模块,widget首页挂件 
     */
