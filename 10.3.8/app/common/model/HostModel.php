...
class HostModel extends Model
{
    ...
    public function hostList($param)
    {
        ...
        $count = $this->alias('h')
            ...
            ->count();
        $hosts = $this->alias('h')
            ->field('h.id,h.client_id,c.username client_name,c.email,c.phone_code,c.phone,c.company,h.product_id,p.name product_name,h.name,h.create_time,h.active_time,h.due_time,h.first_payment_amount,h.billing_cycle,h.billing_cycle_name,h.status,o.pay_time,h.renew_amount,h.client_notes')
            ->leftjoin('product p', 'p.id=h.product_id')
            ->leftjoin('client c', 'c.id=h.client_id')
    ...
    public function indexHost($id)
    {
        ...
        $host = $this->field('id,order_id,product_id,server_id,name,notes,first_payment_amount,renew_amount,billing_cycle,billing_cycle_name,billing_cycle_time,active_time,due_time,status,client_id,suspend_type,suspend_reason,client_notes,ratio_renew,base_price')->find($id);
        if (empty($host)){
            return (object)[]; // 转换为对象
        }
