      ...
      cloudDetail(params).then(res => {
        if (res.data.status === 200) {
          ...
          const moreData = [
            ...
            { name: `${lang.crack_success_info}`, sub_name: this.cloudData.server.crack_success_info },
            { name: `${lang.cloud_os}${lang.common_cloud_label14}`, sub_name: this.cloudData.server.osusername }
          ]
          ...
          const temp = Object.values(this.cloudData.configoptions).reduce((all, cur) => {
            ...
            return all
          }, []).concat(moreData, customFile).map(item => {
            this.$set(item, 'show', false)
            return item
          })
          this.configDetails = temp
          this.main_ip = this.cloudData.server.main_ip
          this.netDataList = this.cloudData.ip.subnet_ip || []
