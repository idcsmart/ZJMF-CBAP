  ...
  computed: {
    ...
    totalPrice() {
      const pirce =
        ...
        Number(this.clDiscount) -
        Number(this.code_discount) -
        Number(this.eventData.discount);
      return pirce <= 0 ? 0 : formatNuberFiexd(pirce);
    },
    calcDes() {
      return (val) => {
        const temp = val
  ...
  methods: {
    ...
    async changeConfig(bol = false) {
      try {
        ...
        this.original_price =
          (this.onePrice * this.orderData.qty).toFixed(2) * 1; // 原总价
        if (this.isShowLevel) {
          // 计算折扣金额
    ...
    verifyCustomFiled() {
      try {
        ...
        const valItem = requireArr
          .filter((item) => item.regexpr)
          .find(
            (item) => !new RegExp(item.regexpr).test(this.customObj[item.id])
          );
        if (valItem) {
          this.$message.error(
            `${valItem.fieldname}${lang.common_cloud_text296}`
          );
          return false;
        }
        return true;
