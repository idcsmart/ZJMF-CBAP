...
class DataCenterModel extends Model{
    ...
    public function orderPage($param){
        ...
        // 获取型号配置
        $data['model_config'] = ModelConfigModel::field('id,name,cpu,cpu_param,memory,disk,support_optional,leave_memory,max_memory_num,max_disk_num,gpu,max_gpu_num')
                                ->where($where)
                                ->where('hidden', 0)
                                ->withAttr('name', function($value){
                                    $multiLanguage = hook_one('multi_language', [
        ...
        // 获取灵活机型
        $data['package'] = [];
        // $data['package'] = PackageModel::alias('pkg')
        //                 ->field('pkg.id,pkg.name,pkg.description,o1.value cpu,pkg.cpu_num,o2.value memory,pkg.mem_num,o3.value disk,pkg.disk_num,pkg.bw,pkg.ip_num')
     ...
     * @author hh
     * @version v1
     * @param   [type] $DataCenterModel [description]
     */
