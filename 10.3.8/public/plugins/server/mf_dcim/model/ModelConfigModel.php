...
class ModelConfigModel extends Model{
    ...
    public function modelConfigCreate($param){
        ...
        $this->startTrans();
        try{
        	$modelConfig = $this->create($param, ['product_id','name','group_id','cpu','cpu_param','memory','disk','support_optional','leave_memory','max_memory_num','max_disk_num','create_time','gpu','max_gpu_num']);
    ...
    public function modelConfigUpdate($param){
        ...
        $this->startTrans();
        try{
        	$this->update($param, ['id'=>$modelConfig['id']], ['name','group_id','cpu','cpu_param','memory','disk','support_optional','leave_memory','max_memory_num','max_disk_num','update_time','gpu','gpu_max_num']);
    ...
    public function modelConfigIndex($id){
        $modelConfig = $this->field('id,product_id,name,group_id,cpu,cpu_param,memory,disk,gpu,support_optional,leave_memory,max_memory_num,max_disk_num,max_gpu_num')->find($id);
        if(empty($modelConfig)){
            return (object)[];
        }
     ...
     * @author hh
     * @version v1
     * @param   int prev_model_config_id - 前一个机型ID(0=表示置顶)
     * @param   int id - 当前机型ID
     */
