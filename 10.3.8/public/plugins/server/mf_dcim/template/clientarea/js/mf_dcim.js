...
new Vue({
  ...
  computed: {
    ...
    calcIpUnit() {
      return (val) => {
        // if (val) {
        //   return this.ipData.filter(item => val === item.value)[0]?.desc
        // }
        if (val === "NC") {
    ...
    calcConfigDisabled() {
      return (item) => {
        ...
        if (
          this.filterConfig.mem_max_num === 0 &&
          this.filterConfig.mem_max === 0
        ) {
          return false;
        }
        // 只针对未选中项
        ...
        if (
          ...
          this.filterConfig.max_memory_num !== 0
        ) {
          return true;
        }
        // 数量超出
    ...
    calcMemMax() {
      ...
      return (item) => {
        ...
        if (
          this.filterConfig.max_memory_num === 0 &&
          this.filterConfig.max_disk_num === 0
        ) {
          return Infinity;
        } else if (this.filterConfig.mem_max_num === 0) {
          return allNum;
        } else if (this.filterConfig.mem_max === 0) {
          return slotNum;
        } else {
  ...
  methods: {
        ...
        if (this.isUpdate && this.isInit) {
          ...
          typeArr.forEach((type) => {
            this.filterConfig[type] = this.filterConfig[type].map((item) => {
              if (Object.keys(this.backfill[type]).includes(String(item.id))) {
                item.checked = true;
              } else {
                item.checked = false;
              }
          ...
          this.params.optional_gpu = Array.from(
            Object.keys(this.backfill.optional_gpu)
          ).reduce((all, cur) => {
            all.push({
              id: cur,
              num: this.backfill.optional_gpu[cur],
    ...
    async changeConfig() {
      try {
        ...
        this.totalPrice = res.data.data.price * 1 + res.data.data.discount * 1;
        ...
        // }
        this.levelNum = res.data.data.discount * 1;
