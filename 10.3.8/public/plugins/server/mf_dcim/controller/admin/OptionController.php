...
use server\mf_dcim\validate\GpuValidate;
...
class OptionController{
	 ...
	 * @title 显卡配置列表
	 * @desc 显卡配置列表
	 * @url /admin/v1/mf_dcim/gpu
	 * @method  GET
	 * @author hh
	 * @version v1
     * @param   int product_id - 商品ID
     * @return  int list[].id - 通用配置ID
     * @return  string list[].value - 显卡
     * @return  string list[].price - 价格
     * @return  string list[].duration - 周期
     * @return  int count - 总数量
	 */
	public function gpuList(){
		$param = request()->param();
		$param['rel_type'] = OptionModel::GPU;
		$param['orderby'] = 'order';
		$field = 'id,value';
     ...
     * @return  string duration[].price - 价格
	 */
	public function gpuIndex(){
		$param = request()->param();

		$OptionModel = new OptionModel();

		$data = $OptionModel->gpuIndex((int)$param['id']);
	 ...
	 * @version v1
	 * @param   int product_id - 商品ID require
     * @param   string value - 显卡 require
     * @param   int order - 排序 require
     * @param   object price - 周期价格(如{"5":"12"},5是周期ID,12是价格)
     * @return  int id - 通用配置ID
	 */
	public function gpuCreate(){
		$param = request()->param();

		$GpuValidate = new GpuValidate();
		if (!$GpuValidate->scene('create')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($GpuValidate->getError())]);
        }
        $param['rel_type'] = OptionModel::GPU;
        $param['rel_id'] = 0;
     ...
     * @param   object price - 周期价格(如{"5":"12"},5是周期ID,12是价格)
	 */
	public function gpuUpdate(){
		$param = request()->param();

		$GpuValidate = new GpuValidate();
		if (!$GpuValidate->scene('update')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($GpuValidate->getError())]);
        }
	 ...
	 * @param   int id - 通用配置ID require
	 */
	public function gpuDelete(){
		$param = request()->param();

		$OptionModel = new OptionModel();

		$result = $OptionModel->optionDelete((int)$param['id'], OptionModel::GPU);
		return json($result);
	}
