...
new Vue({
  ...
  computed: {
    calcProtocol() {
      return (protocol) => {
        return this.protocolArr.filter(item => item.value === protocol)[0]?.label
      };
    },
    calcNat() {
    ...
    renewParams: {
      handler() {
        ...
        if (this.isShowPromo && this.renewParams.customfield.promo_code) {
          // n: 算出来的价格
          n =
            (this.renewParams.base_price * 1000 - this.renewParams.clDiscount * 1000 - this.renewParams.code_discount * 1000) / 1000 > 0
              ?
              (this.renewParams.base_price * 1000 - this.renewParams.clDiscount * 1000 - this.renewParams.code_discount * 1000) / 1000
              :
              0;
        } else {
          //  n: 算出来的价格
  ...
  data() {
    return {
      ...
      natRules: {
        name: [
          { required: true, message: `${lang.placeholder_pre1}${lang.security_label1}`, trigger: 'blur' }
        ],
        domain: [
          { required: true, message: `${lang.placeholder_pre1}${lang.domain}`, trigger: 'blur' }
        ],
        int_port: [
          { required: true, message: `${lang.placeholder_pre1}${lang.int_port}`, trigger: 'blur' }
        ],
        protocol: [
  ...
  methods: {
    ...
    getImage() {
      ...
      image(params).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osIcon =
            "/plugins/server/mf_cloud/template/clientarea/img/mf_cloud/" +
            this.osData[0].name +
            ".svg";
    ...
    async submitConnect() {
      try {
        if (!this.connectCheck && this.powerStatus === 'off' && this.connectType === 'mf_cloud_ip') {
        ...
        this.loading5 = true
        const res = await handlerConnectResource(this.connectType, this.connectWay, params)
        this.loading5 = false
