...
<div id="content" class="host-detail hasCrumb" v-cloak>
  ...
  <t-card class="list-card-container">
    <div class="com-h-box">
      ...
      <div class="user">
        <t-select class="pro-select" v-if="this.clientList" v-model="client_id" :popup-props="popupProps" filterable
          :filter="filterMethod" @change="changeUser" :loading="searchLoading" reserve-keyword
        ...
        <t-row :gutter="{ xs: 8, sm: 16, md: 24, lg: 32, xl: 32, xxl: 90 }">
            ...
            <div class="item">
              <t-form-item :label="lang.product" name="product_id">
                ...
                <t-popup attach="#myPopup" placement="bottom-left" :visible="visibleTree" v-else style="width: 100%">
                  <div id="myPopup">
                    <t-input :value="calcName(formData.product_id)" :placeholder='lang.tailorism'
                  ...
                  <template slot="content" class="test">
                    ...
                    <t-tree :data="calcProduct" :activable="true" :expand-on-click-node="true" ref="tree"
                      :keys="{value: 'key', label:'name'}" @click="onClick" hover :expand-mutex="true"
                      id="product-tree">
                    </t-tree>
                  </template>
                </t-popup>
          ...
          <t-col :xs="24" :xl="12" style="margin-top: 20px;">
            ...
            <div class="config-item">
              ...
              <t-form-item :label="lang.open_time" name="active_time" :rules="[{ validator: checkTime}]">
                <t-date-picker mode="date" format="YYYY-MM-DD HH:mm:ss" enable-time-picker
                  v-model="formData.active_time" @change="changeActive" />
              </t-form-item>
              <t-form-item :label="lang.due_time" name="due_time" :rules="[{ validator: checkTime1}]">
                <t-date-picker mode="date" format="YYYY-MM-DD HH:mm:ss" enable-time-picker v-model="formData.due_time"
                  @change="changeActive" :disabled="disabled" />
              </t-form-item>
            </div>
          </t-col>
            ...
            <template v-for="(item,index) in hostFieldList">
              ...
              <div class="config-item render">
                <t-form-item :label="el.name" v-for="(el,ind) in item.field" :key="ind">
                  ...
                  <t-button class="distribute" @click="handlerDistribute"
                    v-if="el.key === 'manual_resource'">{{lang.distribute}}</t-button>
                </t-form-item>
          ...
          <t-col :xs="24" :xl="12">
            <div class="footer-btn">
              <t-button theme="primary" type="submit" @click="updateUserInfo"
                :loading="isLoading">{{lang.hold}}</t-button>
  ...
  <!-- 产品转移弹窗 -->
  <t-dialog :header="lang.host_transfer_text11" :close-btn="false" :close-on-overlay-click="false"
    :visible.sync="transferVisible" width="800" :footer="false">
    <t-form :data="transferForm" ref="transferInfo" :rules="transferRules" @submit="onTransferSubmit" label-align="top">
      <t-form-item :label="lang.host_transfer_text13" name="client_id">
        <t-select v-if="this.clientList" v-model="transferForm.client_id" :popup-props="popupProps" filterable
          :filter="filterMethod" :loading="searchLoading" reserve-keyword :on-search="remoteMethod">
          <t-option v-for="item in clientList" :value="item.id" :label="calcShow(item)" :key="item.id">
            #{{item.id}}-{{item.username ? item.username : (item.phone? item.phone: item.email)}}
            <span v-if="item.company">({{item.company}})</span>
          </t-option>
  ...
  <!-- 停用 -->
  <t-dialog :header="lang.deactivate" :close-btn="false" :close-on-overlay-click="false" :visible.sync="suspendVisible"
    width="600" :footer="false">
  ...
  <!-- 手动资源分配 -->
  <t-dialog :header="`${lang.manual_resources}${lang.distribute}`" :visible.sync="resourceDialog" width="80%"
    :footer="false">
    <div class="left-search">
      <t-input v-model="resourceForm.keywords" class="search-input" :placeholder="lang.manual_text1"
        @keyup.enter.native="getResourcesList" :on-clear="clearKey" clearable></t-input>
    ...
    <t-table row-key="index" :data="resourceList" :columns="resourcesColumns" :loading="resourceLoading">
