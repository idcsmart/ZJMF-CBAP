...
class OrderModel extends Model
{
    ...
    public function indexOrder($id)
    {
        ...
        $order = $this->field('id,type,amount,create_time,pay_time,status,gateway_name gateway,credit,client_id,notes,refund_amount,amount_unpaid,admin_id')->find($id);
        if (empty($order)){
            return (object)[]; // 转换为对象
        }
    ...
    public function createUpgradeOrder($param)
    {
        ...
        try {
            ...
            $hookDiscountResults = hook("client_discount_by_amount",['client_id'=>$host['client_id'],'product_id'=>$host['product_id'],'amount'=>$result['data']['renew_price']]);
            foreach ($hookDiscountResults as $hookDiscountResult){
                if ($hookDiscountResult['status']==200){
                    $renewPrice = $result['data']['renew_price']-($hookDiscountResult['data']['discount']??0);
                }
            }
            $hookPromoCodeResults = hook('apply_promo_code',['host_id'=>$host->id,'price'=>$result['data']['renew_price'],'scene'=>'upgrade','duration'=>$host['billing_cycle_time']]);
     ...
     * @return string msg - 提示信息
     */
    public function batchDeleteOrder($param)
    {
        $id = $param['id']??[];
        ...
        $hosts = HostModel::where('status', 'Pending')->whereIn('order_id', $id)->select()->toArray();
        if (!empty($hosts)){
            return ['status'=>400, 'msg'=>lang('hosts_under_activation_in_the_order')];
        }
    ...
    public function hostOrderHandle($id)
    {
        ...
        $host = $HostModel->alias('h')
            ->field('h.status,h.billing_cycle,h.billing_cycle_name,h.name,h.billing_cycle_time,p.auto_setup,p.creating_notice_sms,p.creating_notice_mail,h.order_id,p.name as product_name,h.active_time')
