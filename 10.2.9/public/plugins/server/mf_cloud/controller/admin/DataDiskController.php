...
/**
 * @title 魔方云(自定义配置)-数据盘配置
 * @desc 魔方云(自定义配置)-数据盘配置
 * @use server\mf_cloud\controller\admin\DataDiskController
 */
class DataDiskController{

	/**
	 * 时间 2023-01-31
	 * @title 添加数据盘配置
	 * @desc 添加数据盘配置
	 * @url /admin/v1/mf_cloud/data_disk
	 * @method  POST
	 * @author hh
	 * @version v1
	...
	public function create(){
        ...
        $param['rel_type'] = OptionModel::DATA_DISK;
     ...
     * @return  string list[].other_config.store_id - 存储ID
	 ...
	 * @title 修改数据盘配置
	 * @desc 修改数据盘配置
	 * @url /admin/v1/mf_cloud/data_disk/:id
	 * @method  PUT
	 * @author hh
	 * @version v1
	...
	public function delete(){
		...
		$result = $OptionModel->optionDelete((int)$param['id'], OptionModel::DATA_DISK);
		return json($result);
	}
	...
	public function index(){
		...
		$data = $OptionModel->diskIndex((int)$param['id'], OptionModel::DATA_DISK);
	 ...
	 * @title 添加数据盘性能限制
	 * @desc 添加数据盘性能限制
	 * @url /admin/v1/mf_cloud/data_disk_limit
	 * @method  POST
	 * @author hh
	 * @version v1
     * @param   int product_id - 商品ID require
	...
	public function diskLimitCreate(){
		...
		$result = $DiskLimitModel->diskLimitCreate($param, DiskLimitModel::DATA_DISK);
		return json($result);
	}
	...
	public function diskLimitList(){
		...
		$data = $DiskLimitModel->diskLimitList($param, DiskLimitModel::DATA_DISK);
	 ...
	 * @title 修改数据盘性能限制
	 * @desc 修改数据盘性能限制
	 * @url /admin/v1/mf_cloud/data_disk_limit/:id
	 * @method  PUT
	 * @author hh
	 * @version v1
     * @param   int id - 配置ID require
	...
	public function diskLimitUpdate(){
		...
		$result = $DiskLimitModel->diskLimitUpdate($param, DiskLimitModel::DATA_DISK);
		return json($result);
	}
	...
	public function diskLimitDelete(){
		...
		$result = $DiskLimitModel->diskLimitDelete((int)$param['id'], DiskLimitModel::DATA_DISK);
		return json($result);
	}
	 ...
	 * @title 获取数据盘类型
	 * @desc 获取数据盘类型
	 * @url /admin/v1/mf_cloud/data_disk/type
	 * @method  GET
	 * @author hh
	 * @version v1
     * @param   int product_id - 商品ID
