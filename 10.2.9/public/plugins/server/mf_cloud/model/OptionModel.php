...
class OptionModel extends Model{
        ...
        try{
            ...
            if($param['rel_type'] == OptionModel::CPU){
                $insert['other_config'] = json_encode([
                    ...
                    // 'ipv6_num' => $param['other_config']['ipv6_num'] ?? '',
                ]);
                ...
                if($line['bill_type'] != 'bw'){
                    throw new \Exception("线路不是带宽计费,不能添加带宽计费");
                }
                $type = $this
                    // ->where('product_id', $productId)
                    ->where('rel_type', $param['rel_type'])
                ...
                if($line['bill_type'] != 'flow'){
                    throw new \Exception("线路不是流量计费,不能添加流量计费");
                }
                $insert['other_config'] = json_encode([
            ...
            if($type == 'radio'){
                ...
                if(in_array($param['rel_type'], [OptionModel::SYSTEM_DISK, OptionModel::DATA_DISK])){
                    $whereSame[] = ['other_config', '=', json_encode(['disk_type'=>$param['other_config']['disk_type'] ?? ''])];
                }
                $same = $this
                        ->where($whereSame)
                        ->find();
        ...
        if(!empty($list)){
            ...
            if(!empty($firstDuration)){
                $price = PriceModel::alias('p')
                    ->field('p.option_id,p.price,o.other_config,o.rel_type')
                    ->whereIn('p.option_id', $id)
                    ...
                    if(isset($v['type'])){
                        if($v['type'] == 'step'){
                            ...
                            if($param['rel_type'] == OptionModel::SYSTEM_DISK || $param['rel_type'] == OptionModel::DATA_DISK){
                                ...
                                $disk_type = $otherConfig['disk_type'] ?? '';
                            }
    ...
    public function optionUpdate($param){
        ...
        if($option['rel_type'] == OptionModel::CPU){
            if($option['value'] != $param['value']){
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('cpu', $option['value'])->find();
            ...
            if($option['type'] == 'radio'){
                if (!$MemoryValidate->scene('radio')->check($param)){
                    return ['status'=>400, 'msg'=>lang_plugins($MemoryValidate->getError())];
                }
                if($option['value'] != $param['value']){
                    $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('memory', $option['value'])->find();
                    if(!empty($recommendConfig)){
                        return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                    }
                }
                ...
                // 反向查找,正在使用该规则的推荐配置
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('memory', '>=', $option['min_value'])->where('memory', '<=', $option['max_value'])->select()->toArray();
                if(!empty($recommendConfig)){
                    foreach($recommendConfig as $v){
                        // 判断新的范围能否继续使用
                        if($param['max_value']>=$v['memory'] && $param['min_value']<=$v['memory'] && ($v['memory']-$param['min_value'])%$param['step'] == 0){
            ...
            if($option['type'] == 'radio'){
                if (!$LineBwValidate->scene('radio')->check($param)){
                    return ['status'=>400, 'msg'=>lang_plugins($LineBwValidate->getError())];
                }
                if($option['value'] != $param['value']){
                    $recommendConfig = RecommendConfigModel::where('line_id', $option['rel_id'])->where('bw', $option['value'])->find();
                    if(!empty($recommendConfig)){
                        return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                    }
                }
                ...
                $recommendConfig = RecommendConfigModel::where('line_id', $option['rel_id'])->where('bw', '>=', $option['min_value'])->where('bw', '<=', $option['max_value'])->select()->toArray();
                if(!empty($recommendConfig)){
                    
                    foreach($recommendConfig as $v){
                        // 判断新的范围能否继续使用
                        if($param['max_value']>=$v['bw'] && $param['min_value']<=$v['bw'] && ($v['bw']-$param['min_value'])%$param['step'] == 0){
            ...
            if($option['value'] != $param['value']){
                $recommendConfig = RecommendConfigModel::where('line_id', $option['rel_id'])->where('flow', $option['value'])->find();
                if(!empty($recommendConfig)){
                    return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                }
            }
        }else if($option['rel_type'] == OptionModel::LINE_DEFENCE){
            if($option['value'] != $param['value']){
                $recommendConfig = RecommendConfigModel::where('line_id', $option['rel_id'])->where('peak_defence', $option['value'])->find();
                if(!empty($recommendConfig)){
                    return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                }
            }
            ...
            // 推荐配置没有该项
        }else if($option['rel_type'] == OptionModel::SYSTEM_DISK){
            $param['other_config']['disk_type'] = $param['other_config']['disk_type'] ?? '';
            $diskValidate = new diskValidate();
            ...
            if($option['type'] == 'radio'){
                if (!$diskValidate->scene('radio')->check($param)){
                    return ['status'=>400, 'msg'=>lang_plugins($diskValidate->getError())];
                }
                if($option['value'] != $param['value'] || $param['other_config']['disk_type'] != $otherConfig['disk_type']){
                    $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('system_disk_size', $option['value'])->where('system_disk_type', $otherConfig['disk_type'])->find();
                    if(!empty($recommendConfig)){
                        return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                    }
                }
                ...
                // 反向查找,正在使用该规则的推荐配置
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('system_disk_size', '>=', $option['min_value'])->where('system_disk_size', '<=', $option['max_value'])->where('system_disk_type', $otherConfig['disk_type'])->select()->toArray();
                if(!empty($recommendConfig)){
                    if($param['other_config']['disk_type'] != $otherConfig['disk_type']){
                        return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                    }
                    foreach($recommendConfig as $v){
                        // 判断新的范围能否继续使用
                        if($param['max_value']>=$v['system_disk_size'] && $param['min_value']<=$v['system_disk_size'] && ($v['system_disk_size']-$param['min_value'])%$param['step'] == 0){
        ...
        }else if($option['rel_type'] == OptionModel::DATA_DISK){
            $param['other_config']['disk_type'] = $param['other_config']['disk_type'] ?? '';
            $diskValidate = new diskValidate();
            ...
            if($option['type'] == 'radio'){
                ...
                if($option['value'] != $param['value'] || $param['other_config']['disk_type'] != $otherConfig['disk_type']){
                    $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('data_disk_size', $option['value'])->where('data_disk_type', $otherConfig['disk_type'])->find();
                    if(!empty($recommendConfig)){
                        return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                    }
                }
                ...
                // 反向查找,正在使用该规则的推荐配置
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('data_disk_size', '>=', $option['min_value'])->where('data_disk_size', '<=', $option['max_value'])->where('data_disk_type', $otherConfig['disk_type'])->select()->toArray();
                if(!empty($recommendConfig)){
                    if($param['other_config']['disk_type'] != $otherConfig['disk_type']){
                        return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
                    }
                    foreach($recommendConfig as $v){
                        // 判断新的范围能否继续使用
                        if($param['max_value']>=$v['data_disk_size'] && $param['min_value']<=$v['data_disk_size'] && ($v['data_disk_size']-$param['min_value'])%$param['step'] == 0){
        ...
        $oldPrice = PriceModel::field('duration_id,price')->where('option_id', $option->id)->select()->toArray();
        ...
        try{
            ...
            PriceModel::where('option_id', $option->id)->delete();
            if(!empty($priceArr)){
                $PriceModel = new PriceModel();
                $PriceModel->insertAll($priceArr);
            }
        ...
        if($option['rel_type'] == OptionModel::CPU){
            ...
            $des['store_id'] = lang_plugins('储存ID');
    ...
    public function optionDelete($id, $rel_type = null){
        ...
        if($option['rel_type'] == OptionModel::CPU){
            ...
            if($option['type'] == 'radio'){
                ...
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('memory', '>=', $option['min_value'])->where('memory', '<=', $option['max_value'])->find();
            }
            if(!empty($recommendConfig)){
                return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
            }
            // 如果是最后一个内存配置,检查是否有
            $count = $this->where('product_id', $productId)->where('rel_type', OptionModel::MEMORY)->count();
            if($count == 1){
                $configLimit = ConfigLimitModel::where('product_id', $productId)->count();
                if(!empty($configLimit)){
                    return ['status'=>400, 'msg'=>lang_plugins('mf_cloud_cannot_delete_this_config_for_config_limit_exist')];
                }
            }
        }else if($option['rel_type'] == OptionModel::LINE_BW){
            $where = [];
            $where[] = ['line_id', '=', $option['rel_id']];
            if($option['type'] == 'radio'){
                $where[] = ['bw', '=', $option['value']];
            }else{
                $where[] = ['bw', '>=', $option['min_value']];
                $where[] = ['bw', '<=', $option['max_value']];
            }
            ...
            if($option['type'] == 'radio'){
                ...
                // 反向查找,正在使用该规则的推荐配置
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('system_disk_size', '>=', $option['min_value'])->where('system_disk_size', '<=', $option['max_value'])->where('system_disk_type', $otherConfig['disk_type'])->find();
            }
            if(!empty($recommendConfig)){
                return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
            }
            ...
            if($option['type'] == 'radio'){
                ...
                // 反向查找,正在使用该规则的推荐配置
                $recommendConfig = RecommendConfigModel::where('product_id', $productId)->where('data_disk_size', '>=', $option['min_value'])->where('data_disk_size', '<=', $option['max_value'])->where('data_disk_type', $otherConfig['disk_type'])->find();
            }
            if(!empty($recommendConfig)){
                return ['status'=>400, 'msg'=>lang_plugins('config_conflict_please_edit_recommend_config')];
            }
        ...
        try{
            ...
            PriceModel::where('option_id', $id)->delete();
    ...
    public function optionIndex($id): array{
        ...
        $duration = DurationModel::alias('d')
                    ...
                    ->leftJoin('module_mf_cloud_price p', 'p.option_id='.$id.' AND d.id=p.duration_id')
     ...
     */
    public function optionDurationPrice($productId, $relType, $relId = 0, $value = 0, $whereAppend = []){
        $data = [];
        $match = false;
        ...
        foreach($whereAppend as $k=>$v){
            $whereOption[] = [$k, 'LIKE', rtrim($v, '}').'%' ];
        }
        ...
        if($type == 'radio'){
            ...
            if(!empty($optionId)){
                $match = true;
                $data = PriceModel::field('duration_id,price')->where('option_id', $optionId)->select()->toArray();
            ...
            if(!empty($option) && (($value - $option['min_value']) % $option['step'] == 0)){
                ...
                foreach($whereAppend as $k=>$v){
                    $wherePrice[] = ['o.'.$k, 'LIKE', rtrim($v, '}').'%' ];
                }

                $price = OptionModel::alias('o')
                        ->field('o.min_value,o.max_value,p.option_id,p.duration_id,p.price')
                        ->where($wherePrice)
                        ->leftJoin('module_mf_cloud_price p', 'o.id=p.option_id')
                        ->order('o.min_value', 'asc')
                        ->group('p.option_id,p.duration_id')
                        ->select();
     ...
     * @param   int  $value     - 当前值 require
     */
    public function matchOptionDurationPrice($productId, $relType, $relId = 0, $value = 0, $durationId = 0, $whereAppend = []){
        $match = false;  // 配置是否匹配
        $price = null;   // 价格
        ...
        if($type == 'radio'){
            ...
            if(!empty($option)){
                $match = true;
                $price = PriceModel::where('option_id', $option['id'])->where('duration_id', $durationId)->value('price');
            }
        }else if($type == 'step'){
            $whereOption[] = ['min_value', '<=', $value];
            $whereOption[] = ['max_value', '>=', $value];
            ...
            if(!empty($option) && (($value - $option['min_value']) % $option['step'] == 0)){
                ...
                $match = true;
                $price = PriceModel::field('duration_id,price')->where('option_id', $optionId)->where('duration_id', $durationId)->value('price');
