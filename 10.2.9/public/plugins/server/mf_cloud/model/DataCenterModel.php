...
class DataCenterModel extends Model{
    ...
    public function dataCenterList($param)
    {
        ...
        $dataCenter = $this
                ->alias('dc')
                ->field('dc.id,dc.city,dc.area,dc.country_id,dc.cloud_config,dc.cloud_config_id,c.name_zh country_name')
                ->where($where)
    ...
    public function deleteDataCenter($id)
    {
        ...
        $description = lang_plugins('log_delete_data_center_success', ['{name}'=>$CountryModel['name_zh'].$dataCenter['city'].$dataCenter['area'] ]);
        active_log($description, 'product', $dataCenter['product_id']);
    ...
    public function orderPage($param){
        ...
        // 获取配置
        $config = ConfigModel::field('host_prefix,host_length,support_ssh_key,support_normal_network,support_vpc_network,support_public_ip,backup_enable,snap_enable,ip_mac_bind,nat_acl_limit,nat_web_limit,ipv6_num,memory_unit')
                ->where($where)
                ->find();
