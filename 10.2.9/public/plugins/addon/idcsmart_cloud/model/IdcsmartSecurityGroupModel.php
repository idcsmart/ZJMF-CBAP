...
class IdcsmartSecurityGroupModel extends Model
{
    ...
    public function createIdcsmartSecurityGroup($param)
    {
        ...
        try {
            ...
            $idcsmartSecurityGroup = $this->create([
                'client_id' => $clientId,
                'type' => $param['type']  ?? 'host',
                'name' => $param['name'] ?? '',
                'description' => $param['description'] ?? '',
                'create_time' => time()
            ]);

            $this->createDefaulRules($idcsmartSecurityGroup->id, $param['type']  ?? 'host');
    ...
    public function deleteIdcsmartSecurityGroup($id)
    {
        ...
        try {
            ...
            IdcsmartSecurityGroupRuleLinkModel::destroy(function($query) use($ruleIds){
                $query->whereIn('addon_idcsmart_security_group_rule_id', $ruleIds);
            });
            IdcsmartSecurityGroupRuleModel::destroy(function($query) use($id){
                $query->where('addon_idcsmart_security_group_id', $id);
            });
            /*IdcsmartSecurityGroupLinkModel::destroy(function($query) use($id){
                $query->where('addon_idcsmart_security_group_id', $id);
            });*/
            IdcsmartSecurityGroupHostLinkModel::destroy(function($query) use($id){
                $query->where('addon_idcsmart_security_group_id', $id);
            });
