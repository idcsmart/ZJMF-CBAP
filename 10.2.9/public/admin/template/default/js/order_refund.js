    ...
    new Vue({
      data() {
        return {
          ...
          rules: {
            amount: [
              { required: true, message: lang.input + lang.Refund + lang.money, type: 'error' },
              {
            ...
            gateway: [{ required: true, message: lang.select + lang.gateway, type: 'error' }],
            type: [{ required: true, message: lang.select + lang.type, type: 'error' }],
            transaction_number: [
      ...
      created() {
        this.id = this.params.id = this.formData.id = location.href.split('?')[1].split('=')[1];
        this.currency_prefix = JSON.parse(localStorage.getItem('common_set')).currency_prefix || '¥';
        document.title = lang.refund_record + '-' + localStorage.getItem('back_website_name');
      },
    }).$mount(template)
