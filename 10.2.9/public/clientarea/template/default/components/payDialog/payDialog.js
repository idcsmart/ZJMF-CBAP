const payDialog = {
  template:
    ...
    <el-dialog custom-class='pay-dialog' :visible.sync="isShowZf" :show-close="false" @close="zfClose">
      <div class="pc-pay">
        <div class="dia-title">
          <div class="title-text">订单提交成功，请尽快付款！订单号：ID-{{zfData.orderId}}</div>
          <div class="title-text">应付金额 <span class="pay-money">{{ commonData.currency_prefix }}<span class="font-26">{{ Number(zfData.amount).toFixed(2)}}</span></span></div>
        </div>
        <div class="dia-content">
          <div class="pay-top">
            <div class="pay-type" ref="payListRef">
              <div class="type-item" v-for="item in gatewayList" @click="handelSelect(item)" :class="zfData.gateway === item.name ? 'active' : ''" :key="item.id">
                ...
                <span class="type-dec" v-if="isShowCredit && item.name === 'CreditLimit'">可用信用额{{ commonData.currency_prefix}}{{creditData.remaining_amount}}</span>
              </div>
            </div>
            <!-- 左右按钮 -->
          ...
          <div class="credit-tip" v-if="zfData.gateway == 'CreditLimit'">提示：您正在使用信用支付，如需退款，请先还清对应周期账单，否则无法成功退款，请谨慎选择！</div>
          <div class="use-blance">
            <el-checkbox v-model="zfData.checked" @change="useBalance" :disabled="isCz "></el-checkbox>
            <span class="blance-text"  @click="handelBalance">使用余额支付<span class="blance-tip">（当前余额{{ commonData.currency_prefix}} {{(balance * 1).toFixed(2)}}）</span></span>
          </div>
          <div class="QR-box">
            <div class="qr-text" v-if="zfData.gateway !== 'UserCustom'">
              <span v-if="zfData.gateway === 'UserCustom'">线下支付：</span>
              <span v-else>扫码支付：</span>
            </div>
            <div class="qr-content" v-loading="payLoading || loading" v-show="isShowPay && isShowimg && zfData.gateway !== 'UserCustom'" v-html="payHtml"></div>
          ...
          <div v-if="zfData.gateway === 'UserCustom'" class="custom-text">
            <div class="qr-content" v-loading="payLoading || loading" v-show="isShowPay" v-html="payHtml"></div>
            <i class="el-icon-document-copy" v-if="isShowPay && payHtml" @click="copyText(payHtml)"></i>
          </div>
        </div>
        <div class="dia-fotter">
          <el-button class="confirm-btn" @click="handleOk" v-loading="doPayLoading" :disabled="(balance * 1 < zfData.amount *1) && zfData.gateway !== 'CreditLimit'" v-if="!isShowPay || zfData.gateway === 'CreditLimit'">确认支付</el-button>
          <el-button class="def-btn" v-if="isShowPay && zfData.gateway === 'UserCustom'">请线下支付</el-button>
          <el-button class="def-btn" v-if="isShowPay && zfData.gateway !== 'UserCustom' && zfData.gateway !== 'CreditLimit'">请扫码支付</el-button>
          <el-button class="cancel-btn" @click="zfClose">取消</el-button>
        </div>
      </div>
      <div class="mobile-pay">
        <div class="dia-title">
          <div class="title-text">订单提交成功，请尽快付款！</div>
          <div class="title-text font-26">应付:<span class="pay-money">{{ commonData.currency_prefix }}<span class="font-26">{{ Number(zfData.amount).toFixed(2)}}</span></span></div>
        </div>
        <div class="dia-content">
          <div class="order-id">订单号：ID-{{zfData.orderId}}</div>
          <div class="pay-top">
            <div class="pay-type" ref="payListRef">
              <div class="type-item" v-for="item in gatewayList" @click="handelSelect(item)" :class="zfData.gateway === item.name ? 'active' : ''" :key="item.id">
                <img :src="item.url" v-if="item.url" alt="">
