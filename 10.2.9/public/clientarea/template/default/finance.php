    ...
    <el-container>
      ...
      <el-container>
            ...
            <div class="content_box">
              <div class="content_tab">
                <el-tabs v-model="activeIndex" @tab-click="handleClick">
                    ...
                    <div class="content_table">
                      ...
                      <div class="tabledata">
                        <el-table v-loading="loading1" @selection-change="handleSelectionChange" :data="dataList1" style="width: 100%;margin-bottom: 20px;" :row-key="getRowKey" lazy :load="load" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
                          <el-table-column type="selection" width="80" v-if="isShowCombine" :reserve-selection="true"></el-table-column>
                          ...
                          <el-table-column prop="product_names" :label="lang.finance_label1" min-width="300" :show-overflow-tooltip="true">
                            <template slot-scope="scope">
                              ...
                              <el-tooltip placement="top" v-if="scope.row.description">
                                ...
                                <span style="cursor: pointer;" class="a-text" @click="goOrderDetail(scope.row.id)">{{scope.row.product_name}}</span>
                              </el-tooltip>
                      ...
                      <div class="tabledata">
                        <el-table v-loading="loading2" :data="dataList2" style="width: 100%;margin-bottom: .2rem;">
                          ...
                          <el-table-column prop="order_id" width="130" :label="lang.finance_label7" align="left">
                            <template slot-scope="scope">
                              <div class="order_id">
                                <a v-if="scope.row.order_id !== '--'" class="a-text" @click="goOrderDetail(scope.row.order_id)">{{scope.row.order_id}}</a>
                          ...
                          <el-table-column prop="gateway" width="300" :label="lang.finance_label5" align="left"></el-table-column>
                          <el-table-column prop="transaction_number" width="300" :label="lang.finance_label9" align="center" :show-overflow-tooltip="true">
                            <template slot-scope="scope">
                            ...
                            <div class="mob-item-row mob-item-row2">
                              <span class="mob-item-row2-name" :title="item.product_name">
                                ...
                                <span class="row2-name-text">
                                  <a v-if="item.order_id !== '--'" class="a-text" @click="goOrderDetail(item.order_id)">{{item.order_id}}</a>
                                  <span v-else class="a-text" @click="goOrderDetail(item.order_id)">{{item.order_id}}</span>
                                </span>
                              </span>
                              <span>
                                <span>{{ commonData.currency_prefix + item.amount}}</span>
                    ...
                    <div class="content_table">
                      ...
                      <div class="tabledata">
                        <el-table v-loading="loading3" :data="dataList3" style="width: 100%;margin-bottom: .2rem;">
                          ...
                          <el-table-column prop="notes" :label="lang.finance_label10" align="left" :show-overflow-tooltip="true">
                          </el-table-column>
                  ...
                  <el-tab-pane label="合同管理" name="5" v-if="isShowContract">
                    <div class="content_table">
                      <div class="content_searchbar balance-searchbar">
                        <div class="left_tips">
                          <el-button @click="handeInfo" style="color: #fff; background-color: rgba(0, 88, 255, 1);">甲方信息管理</el-button>
                          <el-button @click="handelApplyOrder" style="color: rgba(0, 88, 255, 1);  border: 0.01rem solid #0058FF;">申请合同</el-button>
                        </div>
                        <div class="box" style="display:flex; overflow-y: auto;">
                          <el-input v-model="params4.keywords" style="width: 3.2rem;margin-left: .2rem;" placeholder="请输入合同ID、产品标识、编号" @keyup.enter.native="inputChange4" clearable @clear="getContractList">
                          </el-input>
                          <el-button style="margin-left: 0.1rem; color: #fff; background-color: rgba(0, 88, 255, 1);" @Click="inputChange4">搜索</el-button>
                        </div>
                      </div>
                      <div class="tabledata">
                        <el-table v-loading="loading5" :data="dataList5" style="width: 100%;margin-bottom: .2rem;">
                          <el-table-column prop="id" label="合同ID" width="200" align="left">
                          </el-table-column>
                          <el-table-column label="产品内容" align="left" :show-overflow-tooltip="true">
                            <template slot-scope="scope">
                              <span v-if="scope.row.base_contract === 1">基础合同</span>
                          ...
                          <el-table-column label="状态" width="200" align="left">
                            <template slot-scope="scope">
                              <div style="display: flex; align-items: center;">
                                <span :class="scope.row.status === 'no_sign'  ? 'has-border': '' " class="contract-status" :style="{'color':contractStatusObj[scope.row.status].color,'background':contractStatusObj[scope.row.status].background}">{{contractStatusObj[scope.row.status].label}}</span>
                                <el-popover placement="top-start" trigger="hover">
                                  <div slot="reference" style="display: flex; align-items: center;">
                                    <svg v-if="scope.row.status === 'reject' || scope.row.status === 'cancel'" t="1681982176042" class="help-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="8315" width="18" height="18">
                                      <path d="M511.333 63.333c-247.424 0-448 200.576-448 448s200.576 448 448 448 448-200.576 448-448-200.576-448-448-448z m271.529 719.529c-35.286 35.287-76.359 62.983-122.078 82.321-47.3 20.006-97.583 30.15-149.451 30.15-51.868 0-102.15-10.144-149.451-30.15-45.719-19.337-86.792-47.034-122.078-82.321-35.287-35.286-62.983-76.359-82.321-122.078-20.006-47.3-30.15-97.583-30.15-149.451s10.144-102.15 30.15-149.451c19.337-45.719 47.034-86.792 82.321-122.078 35.286-35.287 76.359-62.983 122.078-82.321 47.3-20.006 97.583-30.15 149.451-30.15 51.868 0 102.15 10.144 149.451 30.15 45.719 19.337 86.792 47.034 122.078 82.321 35.287 35.286 62.983 76.359 82.321 122.078 20.006 47.3 30.15 97.583 30.15 149.451s-10.144 102.15-30.15 149.451c-19.337 45.719-47.034 86.792-82.321 122.078z" fill="#0058FF" p-id="8316"></path>
                                      <path d="M642.045 285.629c-26.482-39.772-72.632-61.676-129.945-61.676-45.43 0-81.73 14.938-107.891 44.4-21.679 24.415-34.958 57.378-39.469 97.974-3.153 28.378-0.747 50.163-0.462 52.553 2.091 17.549 18.02 30.084 35.56 27.991 17.549-2.09 30.081-18.011 27.991-35.56-0.019-0.161-1.845-16.636 0.52-37.916 2.077-18.688 7.877-44.708 23.717-62.547 13.679-15.406 33.317-22.895 60.034-22.895 35.722 0 62.235 11.462 76.675 33.147 15.268 22.93 14.215 52.064 6.398 70.765-4.475 10.704-25.708 30.276-42.77 46.002-35.924 33.111-73.07 67.349-73.07 108.723v61.457c0 17.673 14.327 32 32 32s32-14.327 32-32V546.59c0-0.684 0.354-7.103 12.607-21.925 10.498-12.696 25.413-26.444 39.837-39.739 24.981-23.025 48.576-44.774 58.443-68.379 8.323-19.912 11.834-42.319 10.153-64.799-1.794-24.014-9.516-46.877-22.328-66.119z" fill="#0058FF" p-id="8317"></path>
                                      <path d="M512.099 702.965m-40 0a40 40 0 1 0 80 0 40 40 0 1 0-80 0Z" fill="#0058FF" p-id="8318"></path>
                                    </svg>
                                  </div>
                                  <span v-if="scope.row.status === 'reject'">{{scope.row.reason}}</span>
                                </el-popover>
                                <img v-if="scope.row.status === 'complete' && scope.row.post_number" class="help-icon" @click="handelRec(scope.row)" style="width: 0.18rem;height: 0.18rem;" src="/{$template_catalog}/template/{$themes}/img/finance/icon_1.png" alt="快递信息">
                              </div>
                            </template>
                          </el-table-column>
                          <el-table-column :label="lang.finance_label6" width="100" align="left">
                            <template slot-scope="scope">
                              <el-popover placement="top-start" trigger="hover">
                                <i slot="reference" class="el-icon-more"></i>
                                <div class="operation-box">
                                  <div class="operation-item" @click="handelSign(scope.row.order_id)" v-if="scope.row.status === 'no_sign'">签订合同</div>
                                  <div class="operation-item" @click="handelDetail(scope.row.id)" v-if="scope.row.status === 'review'">查看</div>
                                  <div @click="handelCancel(scope.row.id)" class="operation-item" v-if="scope.row.status === 'review'">取消</div>
                                  <div @click="handelDownload(scope.row.id)" class="operation-item" v-if="scope.row.status === 'complete' || scope.row.status === 'wait_mail'">下载</div>
                                  <div @click="handelMail(scope.row.id)" class="operation-item" v-if="scope.row.status === 'complete' && !scope.row.post_number">邮递</div>
                                  <div class="operation-item" v-if="scope.row.status === 'reject' || scope.row.status === 'cancel'">--</div>
                                </div>
                              </el-popover>
                            </template>
                          </el-table-column>
                        </el-table>
                        <pagination :page-data="params4" @sizechange="sizeChange4" @currentchange="currentChange4"></pagination>
                      </div>
                    </div>
                  </el-tab-pane>
                  <el-tab-pane label="信用额" name="6" v-if="isShowCredit && creditData?.credit_limit*1 > 0">
                    <div class="credit-content">
                      <div class="credit-top">
                        <div class="credit-item">
                          <div class="item-top">
                            <div class="item-l">授信总额({{commonData.currency_suffix}})</div>
                            <div class="item-r">{{creditData.end_time | formateTime3}}到期</div>
                          </div>
                          <div class="item-bottom">
                            <div class="item-bl">{{commonData.currency_prefix}}{{creditData.credit_limit | formatNumber}}</div>
                          </div>
                        </div>
                        <div class="credit-item">
                          <div class="item-top">
                            <div class="item-l">剩余额度({{commonData.currency_suffix}})</div>
                            <div class="item-r">
                              <span class="label-box" :class="creditData.status !== 'Active' ? 'no-active' : 'is-active'">{{credit_status[creditData.status]}}</span>
                            </div>
                          </div>
                          <div class="item-bottom">
                            <div class="item-bl">{{commonData.currency_prefix}}{{creditData.remaining_amount | formatNumber}}</div>
                          </div>
                        </div>
                        <div class="credit-item">
                          <div class="item-top" style="align-items: start;">
                            <div class="item-l">本期账单待还({{commonData.currency_suffix}})</div>
                            <div class="item-r" style="text-align: right;">
                              <div>全部待还：{{commonData.currency_prefix}}{{creditData.used | formatNumber}}</div>
                              <div v-if="creditData.account?.repayment_time">还款截止：{{creditData.account?.repayment_time | formateTime3}}</div>
                            </div>
                          </div>
                          <div class="item-bottom flex-bottom">
                            <div class="item-bl">{{commonData.currency_prefix}}{{creditData.account?.amount | formatNumber}}</div>
                            <el-button class="no-btn" v-if="creditData.account?.status === 'Outstanding'">未出账</el-button>
                            <el-button class="credit-btn" v-if="creditData.account?.status !== 'Repaid' &&  creditData.account?.status !== 'Outstanding'" @click="handelPayCredit(creditData.account?.order_id)">立即还款</el-button>
                          </div>
                        </div>
                      </div>
                      <div class="tabledata">
                        <el-table v-loading="loading6" :data="dataList6" style="width: 100%;margin-bottom: .2rem;">
                          <el-table-column label="出账周期" width="500" align="left">
                            <template slot-scope="scope">
                              {{scope.row.start_time | formateTime2}}- {{scope.row.end_time | formateTime2}}
                            </template>
                          </el-table-column>
                          <el-table-column label="消费额度" align="left" :show-overflow-tooltip="true">
                            <template slot-scope="scope">
                          ...
                          <el-table-column label="状态" width="200" align="left">
                            <template slot-scope="scope">
                              <span class="contract-status" :style="{'color':creditStatusObj[scope.row.status].color,'background':creditStatusObj[scope.row.status].background}">{{creditStatusObj[scope.row.status].label}}</span>
                            </template>
                          </el-table-column>
                          <el-table-column :label="lang.finance_label6" width="100" align="left">
                            <template slot-scope="scope">
                              <el-popover placement="top-start" trigger="hover">
                                <i slot="reference" class="el-icon-more"></i>
                                <div class="operation-box">
                                  <div class="operation-item" @click="handelCredit(scope.row.id)">消费记录</div>
                                  <div class="operation-item" @click="handelPayCredit(scope.row?.order_id)" v-if="scope.row.status === 'Disbursed' || scope.row.status === 'Overdue'">还款</div>
                                </div>
                              </el-popover>
                            </template>
                          </el-table-column>
                        </el-table>
                        <pagination :page-data="params6" @sizechange="sizeChange6" @currentchange="currentChange6"></pagination>
                      </div>
                    </div>
                  </el-tab-pane>
                </el-tabs>
        ...
        <el-dialog width="6.8rem" :visible.sync="isShowKd" :show-close="false" @close="kdClose" class="kd-dialog">
          <div class="dialog-title">快递信息</div>
          <div class="dialog-dec">您的合同已发出，请注意查收</div>
          <div class="dialog-box">
            <div class="kd-item"><span class="kd-label">快递:</span><span class="kd-value">{{recData.courier_company}}</span></div>
            <div class="kd-item"><span class="kd-label">单号:</span><span class="kd-value">{{recData.post_number}}</span></div>
            <div class="kd-item"><span class="kd-label">地址:</span><span class="kd-value">{{recData.rec_address}}</span></div>
            <div class="kd-item"><span class="kd-label">电话:</span><span class="kd-value">{{recData.rec_phone}}</span></div>
            <div class="kd-item"><span class="kd-label">姓名:</span><span class="kd-value">{{recData.rec_person}}</span></div>
          </div>
          <div class="dialog-fotter">
            <el-button @click="kdClose">关闭</el-button>
          </div>
        </el-dialog>
        <!-- 甲方信息管理弹窗 -->
        <el-dialog width="6.8rem" :visible.sync="isShowInfoDia" :show-close="false" @close="infoClose" class="info-dialog">
          <div class="dialog-title">甲方信息管理</div>
          <div class="dialog-dec">
            <p>合同签订后具有法律效力！</p>
            <p>请仔细核对您的甲方信息，确认信息的真实和完整。</p>
          </div>
          <div class="dialog-box">
            <el-form :model="infoFormData" class="info-form" :rules="infoRules" ref="infoForm" label-position="top">
              <div class="certification-info" v-if="isCertification">
                <div class="kd-item"><span class="kd-label">甲方名称:</span>
                ...
                <div class="kd-item"><span class="kd-label">证件号码:</span>
              ...
              <el-form-item label="甲方名称" prop="name" v-if="!isCertification">
                <el-input v-model="infoFormData.name" placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item label="证件号码" prop="id_number" v-if="!isCertification">
                <el-input v-model="infoFormData.id_number" placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item label="联系电话" prop="contact_phone">
                <el-input v-model="infoFormData.contact_phone" placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item label="联系邮箱" prop="contact_email">
                <el-input v-model="infoFormData.contact_email" placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item label="联系地址" prop="contact_address">
                <el-input v-model="infoFormData.contact_address" placeholder="请输入"></el-input>
              </el-form-item>
            </el-form>
          </div>
          <div class="dialog-fotter">
            <el-button class="save-btn" @click="saveInfoData">保存</el-button>
            <el-button class="cancel-btn" @click="infoClose">取消</el-button>
          </div>
        </el-dialog>
        <!-- 取消申请弹窗 -->
        <el-dialog width="4.5rem" :visible.sync="isShowCancel" :show-close="false" @close="cancelClose" class="cancel-dialog">
          <div class="dialog-title">取消申请</div>
          <div class="dialog-dec">撤回申请后，若要再次申请，需重新盖章</div>
          <div class="dialog-fotter">
            <el-button class="save-btn" @click="saveCancel">确认</el-button>
            <el-button class="cancel-btn" @click="cancelClose">取消</el-button>
          </div>
        </el-dialog>
        <!-- 申请纸质合同弹窗 -->
        <el-dialog width="6.8rem" :visible.sync="isShowMailDia" :show-close="false" @close="MailClose" class="mail-dialog">
          <div class="dialog-title">申请纸质合同</div>
          <div class="dialog-dec">
          ...
          <div class="dialog-box">
            <el-form :model="mailFormData" class="info-form" :rules="mailRules" ref="mailForm" label-position="top">
              <el-form-item label="收件人姓名" prop="rec_person">
                <el-input v-model="mailFormData.rec_person" placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item label="收件人地址" prop="rec_address">
                <el-input v-model="mailFormData.rec_address" placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item label="收件人电话" prop="rec_phone">
                <el-input v-model="mailFormData.rec_phone" placeholder="请输入"></el-input>
              </el-form-item>
            </el-form>
          </div>
          <div class="dialog-fotter">
            <div class="fotter-left">
              快递费用：<span class="price-blue">￥20.00</span>
            </div>
            <div style="display: flex;">
              <el-button class="save-btn" @click="saveMailData">确认申请</el-button>
              <el-button class="cancel-btn" @click="MailClose">取消</el-button>
            </div>
          </div>
        </el-dialog>
        <!-- 消费记录弹窗 -->
        <el-dialog width="12rem" :visible.sync="isShowCreditDia" :show-close="false" @close="creditClose" class="mail-dialog creat-dia">
          ...
          <div class="dialog-box" style="margin-top: 0.2rem;">
            <el-table v-loading="loading7" :data="dataList7" style="width: 100%;margin-bottom: 20px;" :row-key="getRowKey" lazy :load="load" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
              <el-table-column prop="id" label="ID" width="90" align="left">
                <template slot-scope="scope">
                  <span class="a-text" @click="goOrderDetail(scope.row.id)"> {{scope.row.product_names ? scope.row.id : '--'}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="product_names" :label="lang.finance_label1" min-width="250" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    ...
                    <div v-if="scope.row.gateway">
                      ...
                      <div v-if="scope.row.credit > 0">
                        ...
                        <div v-else>
                          <el-popover placement="top" trigger="hover" popper-class="tooltip">
                            <i class="el-icon-s-finance" style="color: #F99600;font-size: 0.35rem;"></i>
                            <span style="color: #F99600;"> {{commonData.currency_prefix + scope.row.credit + commonData.currency_suffix}}</span>
                            <span slot="reference" class='gateway-pay'>{{lang.finance_text5}}</span>
                          </el-popover>
                          <span>{{scope.row.gateway ? '+'+scope.row.gateway:''}}</span>
                        </div>
          ...
          <div class="dialog-fotter">
            <div></div>
            <el-button class="save-btn" @click="creditClose">关闭</el-button>
          </div>
        </el-dialog>
      </el-container>
    </el-container>
