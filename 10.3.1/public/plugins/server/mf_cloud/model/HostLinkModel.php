...
class HostLinkModel extends Model{
    ...
    public function createAccount($param){
        ...
        $support_nat = $post['network_type'] == 'vpc' && (isset($configData['nat_acl_limit']) || isset($configData['nat_web_limit']));
        if($support_nat){
            ...
            if(!empty($securityGroupHostLink)){
                ...
                if(!empty($securityGroupLink)){
                    ...
                    if(!isset($securityGroupCreateRes['data']['id'])){
                        return ['status'=>400, 'msg'=>lang_plugins('无法创建安全组')];
                    }
    ...
    public function changePackage($param){
        ...
        if($custom['type'] == 'buy_image'){
            ...
            $supportNat = $configData['network_type'] == 'vpc' && (isset($configData['nat_acl_limit']) || isset($configData['nat_web_limit']));
            $baseIpNum = 1;
            if($supportNat){
                $baseIpNum = 0;
            }
