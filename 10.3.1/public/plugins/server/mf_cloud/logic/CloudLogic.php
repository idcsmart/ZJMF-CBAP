...
class CloudLogic
{
	...
	public function reinstall($param){
		...
		if($res['status'] == 200){
			...
			$HostLinkModel->detail($this->hostModel['id'], false);
		}else{
			$result = [
				'status'=>400,
	...
	public function diskList()
	{
		...
		foreach($diskList as $k=>$v){
			...
			if(empty($v['name'])){
				...
				$diskList[$k]['name'] = $diskName[ $v['id'] ] ?? '磁盘'.$k;
			}
	...
	public function detail()
	{
		...
		if(isset($detail['data'])){
			...
			HostLinkModel::where('host_id', $this->hostModel['id'])->update(['password'=>aes_password_encode($detail['data']['rootpassword']) ]);
		}
    ...
    public function calDiskPrice(&$param){
        ...
        if(isset($param['add_disk']) && !empty(array_filter($param['add_disk']))){
        	...
        	if($diskNum + count($param['add_disk']) > 16){
            	return ['status'=>400, 'msg'=>lang_plugins('mf_cloud_over_max_disk_num', ['{num}'=>16])];
        	}
        	$OptionModel = new OptionModel();

            foreach($param['add_disk'] as $k=>$v){
            	$whereAppend = ['other_config'=>json_encode(['disk_type'=>$v['type']])];

                $optionDurationPrice = $OptionModel->matchOptionDurationPrice($productId, OptionModel::DATA_DISK, 0, $v['size'], $duration['id'], $whereAppend);
    ...
    public function calResizeDiskPrice($param){
		...
		$OptionModel = new OptionModel();
       	foreach($param['resize_data_disk'] as $k=>$v){
       		...
       		$whereAppend = ['other_config'=>json_encode(['disk_type'=>$disk['type']])];
			...
			// 获取当前配置价格
            $currentOptionDurationPrice = $OptionModel->matchOptionDurationPrice($productId, OptionModel::DATA_DISK, 0, $disk['size'], $duration['id'], $whereAppend);
            if($optionDurationPrice['match']){
    ...
    public function changeVpcNetwork($param){
        ...
        if($result['status'] == 200){
        	$cache = cache('MODULE_MF_CLOUD_DETAIL_'.$this->hostModel['id']);
        	if(!empty($cache)){
        		$cache['vpc_network'] = [
        			'id' => $vpcNetwork['id'],
        			'name' => $vpcNetwork['name'],
        		];
        		cache('MODULE_MF_CLOUD_DETAIL_'.$this->hostModel['id'], $cache, 3600);
        	}
        }
	 ...
	 * @return  string memory_usable - 可用内存(‘-’代表获取不到)
	 * @return  string memory_usage - 内存使用率(‘-1’代表获取不到)
	 */
    ...
    public function calCommonConfigPrice($param){
        ...
        if($param['memory'] != $configData['memory']['value']){
            ...
            $description[] = sprintf("内存: %d => %d", $configData['memory']['value'], $param['memory']);
        }
        // 检查之前的线路是否还存在
    	$line = LineModel::where('id', $configData['line']['id'])->find();
    	if(empty($line)){
    ...
    public function loginToClient(){
		...
		if($res['status'] == 200){
			...
			$data = [
				'username' => $res['data']['mainip'] ?: $res['data']['hostname'],
				'password' => $res['data']['panel_pass'],
			];

			$res = curl($url . '/index.php?path=token', $data, 30, 'POST');
			if(!empty($res['error'])){
				$result = [
                    'status' => 400,
                    'msg'    => lang_plugins('暂时无法登录')
                ];
                return $result;
			}
			if($res['http_code'] >= 200 && $res['http_code'] < 300){
                ...
                $result = [
                    ...
                    'msg'    => lang_plugins('success_message'),
                    'data'   => [
                        'url'=> $url . '/#/login?token='.urlencode($token),
                    ],
                ];

                $description = lang_plugins('实例{hostname}登录控制面板成功', ['{hostname}'=>$this->hostModel['name']]);
                active_log($description, 'host', $this->hostModel['id']);
			}else{
                $result = [
                    'status' => 400,
