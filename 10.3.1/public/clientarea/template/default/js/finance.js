    ...
    new Vue({
      ...
      data() {
        return {
          ...
          mailRules: {
            rec_person: [{ required: true, message: lang.finance_text111, trigger: 'blur' }],
            rec_address: [{ required: true, message: lang.finance_text112, trigger: 'blur' }],
            rec_phone: [{ required: true, message: lang.finance_text113, trigger: 'blur' }],
          },
          infoRules: {
            name: [{ required: true, message: lang.finance_text114, trigger: 'blur' }],
            id_number: [{ required: true, message: lang.finance_text115, trigger: 'blur' }],
            contact_phone: [{ required: true, message: lang.finance_text116, trigger: 'blur' }],
            contact_email: [{ required: true, message: lang.finance_text117, trigger: 'blur' }],
            contact_address: [{ required: true, message: lang.finance_text118, trigger: 'blur' }],
          },
          cancelId: ''
      ...
      methods: {
        ...
        // 去实名认证
        handelAttestation() {
          location.href = `/plugin/${this.pluginId('IdcsmartCertification')}/authentication_select.htm`
        },
        // 获取插件Id
        pluginId(name) {
        ...
        // 申请合同
        handelApplyContract() {
          location.href = `/plugin/${this.pluginId('EContract')}/applyContract.htm`
        },
        // 下载电子合同
        ...
        async handelPreview(id) {
          try {
            const res = await downloadContract(id) // 返回的是文件流
            const blob = new Blob([res.data], { type: res.headers['content-type'] })
            const href = window.URL.createObjectURL(blob); //创建下载的链接
            // 新窗口打开预览
            window.open(href)
        ...
        handelDetail(id) {
          location.href = `/plugin/${this.pluginId('EContract')}/contractDetail.htm?id=${id}`
        },
        handelSign(id) {
          location.href = `/plugin/${this.pluginId('EContract')}/signContract.htm?id=${id}`
        },
        handelCancel(id) {
          this.cancelId = id
        ...
        saveMailData() {
          this.$refs.mailForm.validate((valid) => {
            if (valid) {
              mailContract(this.mailFormData).then((res) => {
                this.MailClose()
        ...
        saveCancel() {
          cancelContrat(this.cancelId).then((res) => {
            this.cancelClose()
        ...
        saveInfoData() {
          this.$refs.infoForm.validate((valid) => {
            if (valid) {
              editPartInfo(this.infoFormData).then((res) => {
                this.infoClose()
                this.$message.success(res.data.msg)
              }).catch((err) => {
                this.$message.error(err.data.msg)
            ...
            if (res.data.status === 200) {
              ...
              list.map((item) => {
                ...
                if (item.product_names.length > 2) {
                  product_name = item.product_names[0] + "、" + item.product_names[1] + " " + lang.finance_text121 + item.product_names.length + lang.finance_text122;
                } else {
        ...
        getCommon() {
          ...
          document.title = this.commonData.website_name + "-" + lang.finance_text123;
