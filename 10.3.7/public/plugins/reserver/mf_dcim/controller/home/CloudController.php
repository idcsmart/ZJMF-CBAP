	...
	*/
	public function list(){
        ...
        $host = HostModel::alias('h')
            ->field('h.id,h.name,h.status,h.active_time,h.due_time,h.client_notes,p.name product_name,h.client_notes')
            ->leftJoin('product p', 'h.product_id=p.id')
            ->join('upstream_product up', 'p.id=up.product_id AND up.res_module="mf_dcim"')
            ->where($where)
 		...
 		try{
			...
			if($result['status'] == 200){
				if(isset($result['data']['discount']) && !empty($result['data']['discount'])){
                    // 加上等级折扣
                    $result['data']['price'] = bcadd($result['data']['price'], $result['data']['discount'], 2);
                    $result['data']['price_difference'] = bcadd($result['data']['price_difference'], $result['data']['discount'], 2);
                    $result['data']['renew_price_difference'] = bcadd($result['data']['renew_price_difference'], $result['data']['discount'], 2);
