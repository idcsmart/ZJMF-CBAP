    ...
    async getLineDetails() {
      try {
        ...
        if (this.lineDetail.bw) {
          ...
          if (this.packageObj.id > 0) { // 灵活机型带宽不可变小
        ...
        if (this.lineDetail.ip) {
          ...
          this.ipName = this.lineDetail.ip.filter(item => item.value == this.params.ip_num)[0]?.desc
          // 循环生成IP可选数组
    ...
    getManualOs() {
      manualResourceOs(this.id).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osIcon =
            "/plugins/server/mf_dcim/template/clientarea/img/mf_dcim/" +
            this.osData[0].name +
            ".svg";
