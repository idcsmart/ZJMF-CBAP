                    ...
                    <table class="goods-table">
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td class="item-total">
                            <span>{{commonData.currency_prefix}} {{(((item.price * item.qty)*1000 - item.code_discount*1000 - item.level_discount * (item.hasCalc ? 1: item.qty) * 1000) / 1000) > 0 ?
                              (((item.price * item.qty)*1000 - item.code_discount*1000 - item.level_discount * (item.hasCalc ? 1: item.qty) * 1000) / 1000).toFixed(2) : 0.00}}</span>
                            <el-popover placement="top-start" width="180" trigger="hover" v-if="isShowLevel || (isShowPromo && item.isUseDiscountCode)">
                              <div class="show-config-list">
                                <p v-if="isShowLevel">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}} {{ (item.level_discount * (item.hasCalc ? 1: item.qty)) | filterMoney }}</p>
                            ...
                            <p class="original-price" v-if="(item.level_discount * (item.hasCalc ? 1: item.qty)) != 0 || item.code_discount != 0">{{commonData.currency_prefix}} {{(item.price * item.qty).toFixed(2)}}</p>
                ...
                <div>
                  ...
                  <el-popover placement="top-start" width="180" trigger="hover" v-if="isShowLevel || (isShowPromo && isUseDiscountCode) || isShowCash">
                    <div class="show-config-list">
                      ...
                      <p v-if="isShowCash && cashObj.code">{{lang.shoppingCar_tip_text5}}：{{commonData.currency_prefix}} {{ cashPrice | filterMoney }}</p>
                    </div>
                    <i class="el-icon-warning-outline total-icon" slot="reference"></i>
                  </el-popover>
