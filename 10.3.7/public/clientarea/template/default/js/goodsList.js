    ...
    new Vue({
      ...
      computed: {
        ...
        totalMoneyCalc() {
          ...
          const firstDiscount = this.carList.filter(item => this.checkList[0] === item.positions)
        ...
        calcDomainList () { // 处理可购买的域名带价格才展示
          return data => {
            return data.filter(item => {
              return (item.avail === 0) || (item.avail === 1 && item.priceArr.length > 0)
            })
          }
        },
        showOriginal () {
      ...
      methods: {
        ...
        getCarList() {
          ...
          cartList().then((res) => {
            ...
            this.carList.forEach((item) => {
              domainPrice({ name: item.config_options.domain, host_id: this.host_id })
                .then((res) => {
                  item.priceArr = res.data.data || [];
                  item.showPrice = res.data.data[0]?.buyprice || 0;
                })
        ...
        handelDomainSearch() {
          ...
          this.domainList = [];
          domainSearch({ domain: this.domainInput, suffix: this.selectSuffix, host_id: this.host_id })
            .then((res) => {
              if (res.data.status === 200) {
