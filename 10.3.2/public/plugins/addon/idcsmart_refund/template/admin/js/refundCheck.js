    ...
    new Vue({
      data () {
        return {
          ...
          rules: {
            reject_reason: [{ required: true, message: lang.input + lang.dismiss_the_reason, type: 'error' }],
          },
          delVisible: false,
          delId: ''
        }
      ...
      methods: {
        ...
        formatDate (date, judge) {
          const str1 = [date.getFullYear(), this.formatDateAdd0(date.getMonth() + 1), this.formatDateAdd0(date.getDate())].join('/');
        ...
        async getList () {
          ...
          list.data.data.list.forEach(item => {
            ...
            item.create_time = item.create_time ? this.formatDate(new Date(item.create_time), true) : ""
            item.due_time = Number(item.due_time) * 1000;
            item.due_time = item.due_time ? this.formatDate(new Date(item.due_time)) : "";
            if (!item.admin_name) {
              item.admin_name = "-"
            }
          });
