...
class ConfigModel extends Model{
    ...
    public function saveConfig($param){
        ...
        $this->update($param, ['product_id'=>$param['product_id']], ['type','node_priority','ip_mac_bind','support_ssh_key','rand_ssh_port','support_normal_network','support_vpc_network','support_public_ip','backup_enable','snap_enable','reinstall_sms_verify','reset_password_sms_verify','niccard','cpu_model','ipv6_num','nat_acl_limit','nat_web_limit']);
        if($clearData['clear']){
            if(isset($clearData['recommend_config_id'])){
                RecommendConfigModel::whereIn('id', $clearData['recommend_config_id'])->delete();
            }
            if(isset($clearData['line_id'])){
    ...
    public function isClear($productId, $newType){
        ...
        if($config['type'] == 'host'){
            if($newType == 'lightHost'){
                // 是否有VPC的推荐配置
                $recommendConfig = RecommendConfigModel::field('id,name')->where('product_id', $productId)->where('network_type', 'vpc')->select()->toArray();
                if(!empty($recommendConfig)){
                    $result = [
                        'clear' => true,
                        'recommend_config_id' => array_column($recommendConfig, 'id'),
                        'desc'  => lang_plugins('mf_cloud_switch_type_will_delete') . lang_plugins('mf_cloud_recommend_config') . ':' . implode(',', array_column($recommendConfig, 'name')),
                    ];
                }
                ...
                $recommendConfig = RecommendConfigModel::field('id,name')
                                ->where('product_id', $productId)
                                ->where(function($query) use ($flowLine) {
                                    $query->whereOr('network_type', 'vpc');
                                    if(!empty($flowLine)){
                                        $query->whereOr('line_id', 'IN', array_column($flowLine, 'id'));
                                    }
                                })
                                ->select()
                                ->toArray();

                if(!empty($recommendConfig) || !empty($flowLine)){
                    $desc = lang_plugins('mf_cloud_switch_type_will_delete');
                    if(!empty($recommendConfig)){
                        $desc .= lang_plugins('mf_cloud_recommend_config') . ':' . implode(',', array_column($recommendConfig, 'name')).',';
                    }
                    if(!empty($flowLine)){
            ...
            if($newType == 'host'){
                ...
                if(!empty($flowLine)){
                    $recommendConfig = RecommendConfigModel::field('id,name')
                                ->where('product_id', $productId)
                                ->where('line_id', 'IN', array_column($flowLine, 'id'))
                                ->select()
                                ->toArray();
