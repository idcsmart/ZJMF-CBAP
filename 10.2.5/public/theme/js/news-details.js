    ...
    function getDetail() {
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#announce-type').attr('href', `./news-classify.html?id=${news.addon_idcsmart_news_type_id}&title=${news.type}`)
                ...
                titleList.forEach((item, i) => {
                    $(`#newsBox`).append(`      
                        <div class="news-item">
                            ...
                            <a class="font-ell1 link-hover" href="news-details.html?id=${item.id}">${item.title}</a>
                        </div>
                    `)
                })
            }
