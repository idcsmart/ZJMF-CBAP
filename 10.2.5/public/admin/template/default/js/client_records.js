    ...
    new Vue({
      data () {
        return {
          ...
          delId: '',
          rules: {
            content: [{ required: true, message: lang.input + lang.client_info, type: 'error', trigger: 'blur' }]
          },
          recordFrom: {
            id: '',
            content: '',
      ...
      mounted () {
        document.title = lang.info_records + '-' + localStorage.getItem('back_website_name')
      ...
      methods: {
        ...
        uploadSuccess (val) {
          ...
          if (this.optType === 'add') {
            ...
            const index = this.recordsList.findIndex(item => item.id === this.delId)
        ...
        async sureDelUser () {
          try {
            ...
            this.getRecordsList()
            this.totalUpdate = false
            this.recordsList.splice(this.recordsList.findIndex(item => item.id === this.delId), 1)
          } catch (error) {
            this.delVisible = false
        ...
        scrollBottom () {
          ...
          if (document.querySelector('.loading').getBoundingClientRect().top < clientHeight) {
            if (this.scrollDisabled) {
              this.loadingText = lang.no_more_data
              return
            } else {
              this.loadingText = lang.loading
              if (this.recordsList.length === this.recordsTotal && this.recordsTotal !== 0) {
                return
              }
              if (this.isLoading) {
                return
        ...
        async getRecordsList () {
          try {
            ...
            if (this.recordsParams.page > 1) {
              this.recordsList = Array.from(new Set(this.recordsList.concat(res.data.data.list)))
            } else {
              this.recordsList = res.data.data.list
            }
            this.recordLoading = false
            if (this.recordsTotal !== 0 && this.recordsList.length < this.recordsTotal) {
              this.scrollDisabled = false
            } else {
              this.scrollDisabled = true
            }
            this.isLoading = false
            this.recordsList = this.recordsList.map(item => {
              item.edit = false
              return item
      ...
      created () {
        this.id = this.recordsParams.id = location.href.split('?')[1].split('=')[1] * 1
        this.getClintList()
        this.getPlugin()
        this.getUserDetail()
        this.getRecordsList()
