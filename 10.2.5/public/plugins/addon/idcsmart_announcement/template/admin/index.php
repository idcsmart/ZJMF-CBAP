...
<div id="content" class="help news" v-cloak>
      ...
      <div class="help_table">
        <t-table hover row-key="index" :pagination="pagination" :loading="loading" :data="list" :columns="columns" @Change="changepages">
          ...
          <template #create_time="slotProps">
            <span v-if="slotProps.row.create_time" style="width: 200px;">{{ getLocalTime(slotProps.row.create_time) }}</span>
          </template>
  ...
  <t-dialog :header="lang.idcsmart_announcement" placement="center" :visible.sync="visible" :on-cancel="onCancel" :on-esc-keydown="onKeydownEsc" :on-close-btn-click="onClickCloseBtn" :on-close="close" width="70%" :footer="false">
