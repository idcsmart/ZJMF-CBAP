  ...
  <t-card class="list-card-container">
    ...
    <t-form label-align="top" :data="detialform" class="add_tform" ref="myform" :rules="requiredRules" layout="inline">
      ...
      <t-form-item :label="lang.order_text8" name="client_id" class="form-item-left">
        <t-select class="select-496" v-model="detialform.client_id" filterable :placeholder="lang.order_text8" :loading="searchLoading" reserve-keyword :on-search="remoteMethod" clearable @clear="clearKey" :show-arrow="false" :scroll="{ type: 'virtual',threshold:20 }" :popup-props="popupProps" @change="clientChange" class="user-select">
      ...
      <t-form-item :label="lang.order_text10" name="host_ids">
        <t-select class="select-496" :max="1" :scroll="{ type: 'virtual',threshold:20 }" :popup-props="popupProps" @change="hostChange" :min-collapsed-num="2" multiple :placeholder="lang.order_text10" clearable v-model="detialform.host_ids" :options="hostOptions" :keys="{ label: 'showName', value: 'id'}"></t-select>
      </t-form-item>
    </t-form>
    <div class="text-box">
