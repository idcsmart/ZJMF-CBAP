  ...
  window.onload = function () {
    ...
    new Vue({
      data () {
        return {
          ...
          forwardFormRules: {
            ...
            admin_id: [{
              required: true, message: lang.order_designated_person + lang.isRequired,
              type: 'error'
            }],
            notes: [{
              required: true, message: lang.order_designated_reson + lang.isRequired,
              type: 'error'
            }],
            ticket_type_id: [{
              required: true, message: lang.order_designated_type + lang.isRequired,
              type: 'error'
            }],
          },
          total: 100,
          ...
          turnInsideFormRules: {
            ...
            content: [{
              validator: (val) => !val || (val.length <= 3000), message: lang.verify3 + '3000',
              type: 'warning'
            }]
          },
          // 指定部门下拉框数据（管理员分组列表数据）
      ...
      methods: {
        ...
        changeUser (id) {
          this.id = id;
          location.href = `${this.baseUrl}/plugin/idcsmart_ticket/client_ticket.html?id=${this.id}`;
        },
        // 选择部门
        changeType (type) {
        ...
        rowClick (e) {
          // location.href = `ticket_detail.html?id=${e.row.id}`;
        },
        // 工单-转发
        ...
        forwardFormSubmit ({ validateResult, firstError }) {
          if (validateResult === true) {
            ...
            forwardTicket(data.id, params).then(result => {
        ...
        goAddorder (row) {
          if (row) {
            location.href = str + `/plugin/idcsmart_ticket_internal/ticket_internal_add.html?id=${row.id}&client_id=${row.client_id}&host_ids=${row.host_ids}`
          } else {
