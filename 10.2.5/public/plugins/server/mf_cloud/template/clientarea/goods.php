            ...
            <template v-if="isLogin">
              ...
              <el-form-item :label="lang.security_group">
                ...
                <div class="group-box">
                  <!-- 新增安全组 -->
                  <div class="select">
                    <el-checkbox v-model="item.check" v-for="(item,index) in groupSelect" :key="index" :ref="`safe${index}`">
                      <el-tooltip placement="top" effect="light" trigger="hover" :disabled="item.disabled" class="safe-tip" :content="item.name">
            ...
            <div class="new">{{commonData.currency_prefix}}<span>{{calcTotalPrice}}</span>
              <el-popover placement="top" width="200" trigger="hover" v-if="levelNum || discount" popper-class="level-pup">
                <div class="show-config-list">
                  <p v-if="levelNum">{{lang.shoppingCar_tip_text2}}：{{commonData.currency_prefix}} {{ levelNum | filterMoney }}</p>
                  <p v-if="discount">{{lang.shoppingCar_tip_text4}}：{{commonData.currency_prefix}} {{ discount | filterMoney }}</p>
                </div>
