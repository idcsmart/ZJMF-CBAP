        	    ...
        	    if ($res['status']==200){
        	        ...
        	        if (!empty($res['invoiceid'])){
                        ...
                        $cartData = [
                            ...
                            'billingcycle' => $upstreamHost['upstream_configoption']['billingcycle'] ?? '',
                        ...
                        if ($res['status']==200){
                            ...
                            if ($res['status']==200){
                                if (isset($res['data']['hostid'][0]) && !empty($res['data']['hostid'][0])){
                                    $upstreamHost->save([
                                        'upstream_host_id' => $res['data']['hostid'][0]
                                    ]);
                                }

                                $invoiceId = $res['data']['invoiceid']??0;
	 ...
	 public function renew(HostModel $HostModel)
	 {
        ...
        if($supplier['type']=='whmcs'){
        	$data = [
	            ...
	            'billingcycle' => $HostModel['billing_cycle_name']??''
	        ];
        ...
        elseif ($supplier['type']=='finance'){
            $renewData = [
                'billingcycles' => $HostModel['billing_cycle_name']??'',
	...
	public function cartCalculatePrice($ProductModel, $params = [], $qty=1)
	{
        ...
        elseif ($supplier['type']=='finance'){
            ...
            if ($res['status']==200){
                ...
                $res['data']['duration'] = $res['products']['duration']??0; // 老财务添加数据
                $description = '';
                ...
                foreach ($alloption as $k=>$v){
                    $preview[] = [
                        'name' => $v['option_name'],
                        'value' => $v['suboption_name'],
                        'price' => bcmul($v['suboption_price_total']??0,(1+$this->upstreamProduct['profit_percent']/100),2),
                    ];
                    $description .= $v['option_name'].': '.$v['suboption_name'].',价格:'.(bcmul($v['suboption_price_total']??0,(1+$this->upstreamProduct['profit_percent']/100),2))."\r\n";
                }
            ...
            if ($res['status']==200){
                ...
                foreach ($cycles as $cycle){
                    $hosts[] = [
                        ...
                        'price' => bcmul($cycle['amount'], (1+$this->upstreamProduct['profit_percent']/100), 2),
                        'billing_cycle' => $cycle['billingcycle'],
                    ];
                }
